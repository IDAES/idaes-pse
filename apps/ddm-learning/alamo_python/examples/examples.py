...
def sixcamel(*x):
    x1,x2 = x
    t1 = np.multiply(4.0-2.1*np.power(x1,2)+np.divide(np.power(x1,4),3.0),np.power(x1,2))
...
def branin(*x):
    ...
    x1,x2 = x
    pi = 3.14159
    z = (x2-(5.1/(4*pi**2))*x1**2+(5/pi)*x1-6)**2 + 10*(1-(1/(8*pi))*np.cos(x1)+10)+np.random.normal(0,0.1)
    return z
