...
##############################################################################
def allcard(xdata,zdata,xval,zval,opdict):
    ...
    kwargs=opdict
    trans=list(['linfcns','expfcns','logfcns','sinfcns','cosfcns','monomialpower','multi2power','multi3power','ratiopower'])
    ...
    # split trianing and validationd ata before looping through cc
    tlist=list([])
    vlist=list([])
    ndlist = range(ndata)
    if (cvfold == 'loo'):
...
#        res = alamopy.doalamo(xdata,zdata,kwargs.values())
        ...
        if ( ccon > 1):
            if (float(rmse['val']) >= float(rmseold)):
                sys.stdout.write('              Problem name   : '+kwargs['almname']+'\n')
                sys.stdout.write('  rMSEval : '+str(rmse['val'])+'    MSEold : '+str(rmseold)+'\n')
                if ( cvfold == 'valset'):
                    sys.stdout.write('   Ntrain : '+str(len(tlist))+'    Nval : '+str(len(vlist))+'\n')
                else:
                    sys.stdout.write('   Ntrain : '+str(len(tlist[0]))+'    Nval : '+str(len(vlist[0]))+'\n')
                ...
                os.system('cp '+'oldalmsim.sv '+kwargs['almname'].split('.')[0]+'alm.py')
                endt = time.time()
