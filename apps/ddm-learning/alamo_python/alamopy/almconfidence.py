    ...
    if(type(data['model']) == type({})):
        ...
        # This section is not currently in compliance with custom basis functions
            ...
            for j in range(nlinterms):
                ...
                thisterm=thisterm[-1]
                thislam = lambdify([symbols(data['xlabels'])], parse_expr(thisterm.replace('^','**')), "numpy")
            ...
            for j in range(nlinterms):
                data['conf_inv'][okey].append('B'+str(j+1)+' : '+str(coeffs[j])+'+/-'+str(ci[j]))
    else:
        model=data['model']
        model.split('=')[1]
        out = model.split('=')[0]
        ...
        data['conf_inv']=list()
        for j in range(nlinterms):
            data['conf_inv'].append('B'+str(j+1)+' : '+str(coeffs[j])+'+/-'+str(ci[j]))
