...
def almwriter(data, debug, vargs, kwargs):
    ...
    with open(data['stropts']['almname'], 'w') as a:
        for arg in data['opts'].keys():
            if arg == 'sigma' and data['opts'][arg] < 0 : continue
            elif (arg == 'modeler' and data['opts'][arg] == 6):
                a.write(arg+' '+str(data['opts'][arg])+'\n')
                a.write('builder 0\n')
                a.write('enumall 1\n')
                a.write('enum1 1\n')          
            else: a.write(arg+' '+str(data['opts'][arg])+'\n')

        for arg in list(['xlabels','zlabels']):
            a.write(arg+' '+str(data['lstopts'][arg])[1:-1].replace(",",'').replace("'",'')+'\n')
        ...
        if ('xmin' in kwargs.keys()):
            ...
            for arg in data['set4'].keys():
                a.write(arg+' '+str(data['set4'][arg])[0:-1].replace(",",'')+'\n')
    ...
    if ('almopt' in data['stropts'].keys()):
        catfile('almtemp',str(data['stropts']['almname']),str(data['stropts']['almopt']))
