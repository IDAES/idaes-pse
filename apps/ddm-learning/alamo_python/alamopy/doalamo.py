...
def addBasisGroup(type_of_function, input_indices="", powers=""):
    ...
    if type_of_function == "CONST":
        ...
        if powers == "":
            powers = "-1111"
        group_list.append("%i %s %s %s" % (group_index, 
                                           type_of_function, input_indices, powers))
...
def addBasisConstraint(group_id, output_id, constraint_type, intParam):
    ...
    basis_constraint_list.append("%i %i %s %i" % (group_id, output_id,
                                                  constraint_type, intParam))
...
def writeCustomALAMOOptions(kwargs):
    global custom_fxn_list, surface_constraint_list, \
        extrapxmax, extrapxmin, group_list, basis_constraint_list
    name = "almopt.txt"
...
def checkForSampledData(data, debug):
    ...
    lf2 = lf.split('\n')
    lf2 = lf2[1:-1]
    sys.stdout.write('Updating number of training points from ' 
                     + str(data['opts']['ndata']) + ' to ' + str(len(lf2)) + '\n')
...
def expandOutput(xdata, zdata, vargs, data, debug):
    ...
    if debug['expandoutput']:
        ...
        for key in list(['model', 'f(model)', 'ssr', 'R2', 'size', 'rmse', 
                         'nbas', 'totaltime', 'olrtime', 'miptime', 'clrtime', 
                         'othertime', 'version', 'status', 'madp', 'numolr', 
                         'nummip', 'numclr', 'ninputs']):
