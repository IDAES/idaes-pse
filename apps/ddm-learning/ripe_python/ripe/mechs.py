...
##############################################################################
_all__ = ['powerlawp5','powerlaw2','powerlaw3','powerlaw4',
          'avrami2','avrami3','avrami4','avrami5','randomnuc','ptompkins',
          'jander','antijander','valensi','parabolic','gb3d',
          'zlt','grain','massact','massactm']
...
def jander(*data):
    pd = data[0]
    return 3.0 * ((1.0-pd)**(1.0/3.0)) * (1.0/(1.0+pd)**((-1.0/3.0)-1.0))

def antijander(*data):
    pd = data[0]
    return ((3.0/2.0)*(1.0-pd)**(2.0/3.0)) * (1.0/(1.0+pd)**((-1.0/3.0)-1.0))
...
def gb3d(*data):
    pd = data[0]
    return (3.0/2.0)*(1.0-pd)**(4.0/3.0) * 1.0/((1.0-pd)**((-1.0/3.0)-1.0))
...
def getmechs(kwargs):
    ...
    if 'stoich' not in inkeys:
        ...
        if type(stoich) == type(0) or type(stoich) == type(0.0):
            stoich = [kwargs['stoich']]
