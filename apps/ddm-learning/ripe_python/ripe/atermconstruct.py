            ...
            for tempi in range(len(mechs)):
                ...
                for mspec in mechline[1]:
                    for h_ind in list(mechline[0]):
                        ...
                        if mspec == 'massact' or mechline[2] == True:
...
def checkargs(kwargs,inkeys,npc,nobs,ns):
    ...
    for key in list(set(inkeys)-set(['t','Tref'])):
        ...
        if type(kwargs[key]) == type(0.0):
            kwargs[key] = [[kwargs[key]]] * npc
        elif type(kwargs[key]) == type(0):
...
def normalizefeatures(aterm,fixarray):
    ...
    for k in range(nm):
        for l in range(nh):#L
            if fixarray[l,k] == 1:
                maxval = np.max([x for x in np.ndarray.flatten(aterm[:,:,k,l])])# if x > 0.0])
                minval = np.min([x for x in np.ndarray.flatten(aterm[:,:,k,l])])# if x > 0.0])
