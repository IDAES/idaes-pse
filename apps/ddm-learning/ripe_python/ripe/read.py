...
def read(infile):
    ...
    data, rspace, debug = ripe.data, ripe.rspace, ripe.debug
    ...
    llist = ['mechanisms','clc','slabels','pclabels','slabels','pclabels','noise','adsspec','adslabel','revlabel','revspec']
    ...
    while ln < len(rd):
        ...
        if( inopt not in rspace['inopt'] ):
            sys.stdout.write("Error : I don't understand the input file at line : "+str(ln+1)+"\n")
            exit
...
#            continue
    ...
    # read validation conc data
            ...
            if ('pclabels' in data['otherdata']):
                if ('x0' in data['otherdata']['pclabels']):
                    if('npc' in data['otherdata']):
                        ...
                        sys.stdout.write('Must specify number of process conditions before c0data')
        ...
        # Initilize the data dictionary                                               
            ...
            t1=0
            for i in range(ln+1,ln+len(data['otherdata']['adslabel'])*len(data['otherdata']['adsspec'])+1):
                data['procdata']['adata'][data['otherdata']['adslabel'][t1]][data['otherdata']['slabels'][i-ln-1]]=str(rd[i])
                if ( i-ln-i == len(data['otherdata']['adsspec'])):
                    t1=t1+1
            ln=ln+1+len(data['otherdata']['adslabel'])*len(data['otherdata']['adsspec'])
    # Take care of species not included in adsorption                                 
        ...
        # Initilize the data dictionary                                               
            ...
            for i in range(ln+1,ln+len(data['otherdata']['revspec'])+1):
                data['procdata']['revdata'][data['otherdata']['revlabel'][0]][data['otherdata']['revspec'][i-ln-1]]=str(rd[i])
...
def readtimeseries(rd,ln,dlm,st1,st2):
    ...
    data[st1]['time']=np.array(np.zeros([data['otherdata']['timeseries'],data['otherdata']['ntimedata']]))
    for s in data['otherdata']['slabels']:
        data[st2][s]=np.zeros([data['otherdata']['timeseries'],data['otherdata']['ntimedata']])
        t1=0
        t2=0
        for i in range(ln+1,ln+1+data['otherdata']['ntimedata']*data['otherdata']['npc']):
            rd2=rd[i].split(dlm)
            if (len(rd2) == 1):
                rd2=rd[i].split('\t')
                while ('' in rd2): rd2.remove('')
                ...
                t3=0
                for s in data['otherdata']['slabels']:
                    # Check for true model speicified to debug
                    if debug['true']['use'] == False:
                        data[st2][s][t1][t2] = float(rd2[t3+1])
                    else:
                        data[st2][s][t1][t2] = 1-exp(-debug['true']['k']*exp(-debug['true']['e']/(8.314*data[st1]['T'][t1]))*data[st1]['time'][t1][t2])
                    t3=t3+1
                t2=t2+1
