...
def constructmodel(riperes,**kwargs):
    ...
    def ripesim(data):
        ...
        model.conc0 = pyo.Param(model.s, initialize = dict(((s),data[s-1]) for s in model.s))
        if ptype > 1:
            model.T = pyo.Param(initialize = data[ns])
            model.E = pyo.Param(model.r, initialize = dict(((r), acte[r-1]) for r in model.r))
                                
        model.nu = pyo.Param(model.r, model.s, initialize = dict(((r,s), stoichs[r-1][s-1]) for r in model.r for s in model.s))                                
        model.conc = pyo.Var(model.s, domain = pyo.NonNegativeReals, initialize = 1.0)
        try:
            model.flow = pyo.Param(model.s, initialize = dict(((s),data[ns+s]) for s in model.s))
        except:
        ...
        model.k = pyo.Param(model.r, initialize = dict(((r), prek[r-1]) for r in model.r))
        ...
        def prates_2(model,r):
            ...
            return model.rate[r] == model.k[r] * pyo.exp(-(model.E[r]/(gc*model.T))) * mechs[r-1](*indata)
        def prates_3(model,r):
            indata = [model.conc[i] for i in model.s] + [data[ns]]
            return model.rate[r] == model.k[r] * pyo.exp(-(model.E[r]/(gc))*(1/(model.T) - 1/(Tref))) * mechs[r-1](*indata)
        if ptype == 1:
            trule = prates_1
        elif ptype == 2:
            trule = prates_2
        ...
        def balance(model,s):
            return 100.0*model.dum[s] == model.flow[s] * (1.0/ model.vol) * (model.conc0[s] - model.conc[s]) + sum([model.nu[i,s]*model.rate[i] for i in model.r])
        ...
        conres  = []
        for i in range(ns):
            conres.append(results.Solution.Variable['conc['+str(i+1)+']']['Value'])
        return conres
    return ripesim
...
def ems(riperes,sim,lb,ub,nspec,**kwargs):
    ...
    def check_fun(x):
        ...
        return -1.0*np.sum(np.divide(np.power(np.subtract(x[:ns],res_sim(x[ns:])),2),riperes['sigma']))
    ...
    def error_fun(x):
        ...
        sim_conc = sim(x[:])
        res_conc = res_sim(x[:])
        return -1.0*np.sum(np.power(np.divide(np.subtract(sim_conc,res_conc),sim_conc),2))
    t_targets = np.zeros([nd,1])
    if dflag == True:
        for i in range(nd):
            t_targets[i] = check_fun(alldata[i][:])

    bb = rbfopt.RbfoptUserBlackBox(ndim,np.array(lb),np.array(ub),np.array(['R']*ndim), error_fun)
    # Request that any point returned be no closer than the other closest point
    distance = 10.0
    for i in range(nd):
        for j in range(nd):
            if i != j:
                distance = max(distance, np.linalg.norm(alldata[i,ns:ns+ndim]-alldata[j,ns:ns+ndim]))
    settings = []
    t = datetime.datetime.now()
    d_mult = 1.0
    settings =  rbfopt.RbfoptSettings(nlp_solver_path=sharedata['nlp_path'], minlp_solver_path=sharedata['minlp_path'],print_solver_output=False,min_dist = d_mult*distance,algorithm='Gutmann', rand_seed= int(time.mktime(t.timetuple()))) # random seed required for RBFopt functionality
...
#        print 'check conc : ',alldata[:,ns:ns+ndim],t_targets
        alg = rbfopt.RbfoptAlgorithm(settings, bb, do_init_strategy=False,init_node_pos=alldata[:,ns:ns+ndim],init_node_val=t_targets)#,num_nodes_at_restart=nd)
    else:
