...
def doalamo(data,time,xlo,xup,rpc,rspec,sharedata):
    ...
    import sympy
    from alamopy import multos as mos
    ...
    for pc in rpc:
        for s in rspec:
            ...
            if(float(r2[pc][s]) > sharedata['pwalamo']['err']):
                ...
                cutoff = (1.0-float(sharedata['pwalamo']['err']))*np.sum((data[xlm:xum,s,pc] - np.average(data[xlm:xum,s,pc]))**2)
                temp =0
                i = xlm
                while temp < 0.5 * cutoff:
                    temp = temp + (data[i,s,pc] - amodel.subs(st,tdata[i]))**2
                    i=i+1
                if ((i-xlo) < sharedata['pwalamo']['nmin']): i=xlo+sharedata['pwalamo']['nmin']
                if ((xup-i) > sharedata['pwalamo']['nmin']): i = xup-sharedata['pwalamo']['nmin']
                [lp1, lr1, sharedata] = doalamo(data,tdata,xlo,i,[pc],[s],sharedata)
                sharedata = dopwalamo(data,tdata,lp1,lr1,xlo,i,[pc],[s],sharedata)
                [lp2, lr2, sharedata] = doalamo(data,tdata,i,xup,[pc],[s],sharedata)
                sharedata = dopwalamo(data,tdata,lp2,lr2,i,xup,[pc],[s],sharedata)
            else:
                sharedata['pwalamo']['profiles'][pc][s].append(profile[pc][s])
                sharedata['pwalamo']['tlo'][pc][s].append(xlo)
                sharedata['pwalamo']['tup'][pc][s].append(xup)
...
def sstargets(fdata,pc):
    ...
    for i in range(n):
        for j in range(s):
            targets[i,j] = (pc['flow'][i][j] / pc['vol'][i]) * (fdata[i,j] - pc['x0'][i][j])
    return targets
...
def dynamictargets(data,fdata,pc,sharedata):
    ...
    while tempn >= ntimedata:
        [prof, r2, sharedata] = doalamo(data[tempn-ntimedata:tempn,:,:],pc['t'][tempn-ntimedata:tempn],1,ntimedata,range(int(pc['npc'])),range(int(spec)),sharedata)
        for l in list(['profiles','tlo','tup','etrack']):
            for prc in range(pc['npc']):
                sharedata['pwalamo'][l][prc]={}
                for s in range(spec):
                    sharedata['pwalamo'][l][prc][s] = list()
        sharedata = dopwalamo(data[tempn-ntimedata:tempn,:,:],pc['t'][tempn-ntimedata:tempn],prof, r2,1,ntimedata,range(pc['npc']),range(spec),sharedata)
