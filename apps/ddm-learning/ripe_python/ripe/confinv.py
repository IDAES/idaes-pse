...
#                tempk = s_target * (k[0] / scales['nfactor'][int(i1)-1][int(i2)-1])
    ...
    if ptype == 'arr':
        if 'Tref' not in pc.keys():
            sr = ripe.kinforms.arr(targets,np.ndarray.flatten(pc['T']),aterm,p0,sharedata['gasconst'])#,sigma)
            sensmat = ripe.kinforms.arrjac(targets,np.ndarray.flatten(pc['T']),aterm,p0,sharedata['gasconst'])        
        if 'Tref' in pc.keys():
            sr = ripe.kinforms.refarr(targets,np.ndarray.flatten(pc['T']),sharedata['Tref'],aterm,p0,sharedata['gasconst'])#,sigma)
            sensmat = ripe.kinforms.refarrjac(targets,np.ndarray.flatten(pc['T']),sharedata['Tref'],aterm,p0,sharedata['gasconst'])
    else:
    ...
    mmat = np.matmul(np.matmul(np.ndarray.transpose(sensmat),np.diag(flat_sigma)),sensmat)
