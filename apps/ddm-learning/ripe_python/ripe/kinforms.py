    ...
    for i in range(n):
        for j in range(s):
            for k in range(ln):
                ...
                out[q,k+ln] = p[k]*(-1/(gc*T[i]))*np.exp((-p[k+ln]/(gc*T[i])))*x[i,j,k] 
            q+=1
    return out
...
def refarr(y,T, Tref, x, p,gc):
    ...
    for i in range(n):
        for j in range(s):
            guess = 0.0
            for k in range(ln):
                guess += p[k]*np.exp(-1.0*p[k+ln]/(gc)*(1/(T[i])-1/(Tref)))*x[i,j,k]
            out[q] = ((y[i,j] - guess) **2) #/ sig[i,j]
            q+=1
    return out
...
def refarrjac(y,T, Tref, x, p,gc):
    ...
    for i in range(n):
        for j in range(s):
            for k in range(ln):
                out[q,k] = np.exp( ((-1.0*p[k+ln])/(gc)) * (1/(T[i])-1/(Tref)) )*x[i,j,k]
                out[q,k+ln] = p[k]*((-1.0/(gc))*(1/(T[i])-1/(Tref))) *np.exp(((-1*p[k+ln])/(gc))*((1/(T[i]))-(1/(Tref))))*x[i,j,k]
            q+=1
    return out
