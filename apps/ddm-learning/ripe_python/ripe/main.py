...
#    if np.linalg.matrix_rank(np.array(stoich)) < len(stoich):
    ...
    aterm , fdata , pc , data, scales= ripe.atermconstruct.makeaterm(data,stoich,mechanisms,kwargs,ncons,mechlist,fixarray,sharedata)
    ...
    targets,s_targets,savetargets,sharedata = ripe.targets.gentargets(data,kwargs,fdata,pc,sharedata)
    ...
    if 'sigma' in kwargs.keys():
        if type(kwargs['sigma']) == type(0.0) or type(kwargs['sigma']) == type(0):
        ...
        sharedata = ripe.bounds.get_bounds([[aterm,targets,sigma,sharedata['bounds']],ptype,fixarray,ncons,0,pc,sharedata])
        # Need to claculate refined variable bounds here
    else:
        sigma = [[1.0]*int(ns)]*int(n)
        ccon = np.min([ns,ncons,n,nm,nh])
        sys.stdout.write('Calcuating residual variance with data provided with cc = '+str(ccon)+'\n')
        sigresults = ripe.genpyomo.ripeomo([aterm,targets,sigma,sharedata['bounds']],ptype,fixarray,ccon,0,pc,sharedata)
        sigma = [sigresults['sigma']*int(ns)]*int(n)
        if sharedata['wls'] == True:
    ...
    if 'ccon' in kwargs.keys():
        ...
        sys.stdout.write('Solving RIPE model with cardinality constraint = '+str(ccon)+'\n')
        results = ripe.genpyomo.ripeomo([aterm,targets,sigma,sharedata['bounds']],ptype,fixarray,ccon,1,pc,sharedata)
    else:
        ...
        d_results = dict.fromkeys([0]+ccon_list)
        sys.stdout.write('   ---- Calculating null values for model selection ----    \n')
        # Getting sigma estimates from ~unbiased estimates
        d_results[0] = ripe.genpyomo.ripeomo([aterm,targets,sigma,sharedata['bounds']],ptype,fixarray,0,1,pc,sharedata)
        bic.append(d_results[0]['OBJ'])
        sys.stdout.write(' - Null model BIC = '+str(bic[-1])+'\n')
        for ccon in ccon_list:
            sys.stdout.write(' - Solving RIPE model with cardinality constraint = '+str(ccon)+' - \n')
            d_results[ccon] = ripe.genpyomo.ripeomo([aterm,targets,sigma,sharedata['bounds']],ptype,fixarray,ccon,1,pc,sharedata)
            bic.append(float(d_results[ccon]['OBJ'])+np.log(n*ns)*ccon)
            sys.stdout.write(' - '+str(ccon)+'-term model BIC = '+str(bic[-1])+'\n')
            if bic[-1] > bic[-2]:
                ccon = ccon - 1
                break
    ...
    if 'sigma' not in kwargs.keys():
        ...
        sigma = savesig
    kres, eres, stoichres, mechres,r2 = ripe.confinv(aterm,targets,results,ccon,ptype,pc,sharedata,sigma)#,scales,s_targets)
#    kres, eres, stoichres, mechres,r2 = ripe.confinv(scales['aterm_u'],savetargets,results,ccon,ptype,pc,sharedata,sigma,scales,s_targets)
    ...
    s_kres, s_eres , ci_k, ci_e = print_results(kres, eres, stoichres, mechres,r2,sharedata)
...
def ripewrite(sd,string):
    import sys
    with file('riperesults.txt', 'a') as a:
        a.write(string+'\n')
    if sd['hide_output'] == False:
...
def print_results(kres,eres,stoichres,mechres,r2,sd):
    ...
    ci_k = []
    ci_e = []
    tstring = '  ----  RIPE selected '+str(cc)+' models in the optimal reaction network  ----  \n'
    sys.stdout.write(tstring)
    with file('riperesults.txt', 'w') as a:
        a.write(tstring)
