...
##############################################################################
# This subroutine is no longer used in the ripe-pyomo implementaiton
def writeripe(aterm,ccon):
    ...
    for ccon in clist:
        ...
        if kils == 1:
            ...
            for i in range(len(lf2)):
                sigmaest[i] = lf2[i] / (ndata-1)
            bic[ccon] = sum(np.divide(lf2,sigmaest)) + np.log(data['otherdata']['npc']*len(data['otherdata']['slabels']))*ccon
            ripeci(aterm,ccon)                
        else:
            exit
...
def writemodel(aterm,ccon):
    ...
    if ('clc' in data['otherdata']):
        aterm=np.zeros([data['otherdata']['npc'],data['otherdata']['ntimedata'],data['rxnet']['mechanisms']])
            ...
            for j in range(data['otherdata']['ntimedata']):
                ...
                if ('clc' in data['otherdata']):
                    for m in range(len(data['otherdata']['clc'])):
                        ...
                        if(mech=='c1'): aterm[i,j,t]=1.0 - data['concdata']['X'][i][j]  #random nucleation
                        if(mech=='c2'): aterm[i,j,t]=2.0/3.0*  data['concdata']['X'][i][j]**(2.0/3.0-3.0/2.0) #Power law of oer 2/3
                        if(mech=='c3'): aterm[i,j,t]=2.0 * data['concdata']['X'][i][j]**(2.0 - 1.0/2.0) # Power law model of order 2 
                        if(mech=='c4'): aterm[i,j,t]=4.0 * data['concdata']['X'][i][j]**(4.0 - 1.0/4.0) # power law model of order 4
                        if(mech=='c5'): aterm[i,j,t]=3.0 * data['concdata']['X'][i][j]**(3.0 - 1.0/3.0) # power law model of order 4
                        if(mech=='c6'): aterm[i,j,t]=1.0 # power law model of order 1
                        if(mech=='c7'): aterm[i,j,t]= 2.0 * (1.0 - data['concdata']['X'][i][j]) * np.power(-np.log(1.0 - data['concdata']['X'][i][j]),1.5) #nucleation mode - Avrami-Erofeev eq of order 2
                        if(mech=='c8'): aterm[i,j,t]= 3.0 * (1.0 - data['concdata']['X'][i][j]) * np.power(-np.log(1.0 - data['concdata']['X'][i][j]),3.0 - (1.0/3.0)) # '' order 3
                        if(mech=='c9'): aterm[i,j,t]= 0.5 * (1.0 - data['concdata']['X'][i][j]) * (-np.log(1.0 - data['concdata']['X'][i][j]))**(0.5 - 1.0/0.5) # '' order 0.5
                        if(mech=='c10'): aterm[i,j,t]= 1.5 * (1.0 - data['concdata']['X'][i][j]) * np.power(-np.log(1.0 - data['concdata']['X'][i][j]),1.5 - (1.0/1.5)) # '' order 1.5
                        if(mech=='c11'): aterm[i,j,t]= 4.0 * (1.0 - data['concdata']['X'][i][j]) * (-np.log(1.0 - data['concdata']['X'][i][j]))**(4.0 - 1.0/4.0) # '' order 4
                        if(mech=='c12'): aterm[i,j,t]= data['concdata']['X'][i][j] * (1.0 - data['concdata']['X'][i][j])   #Prout tompkins
                        # diffusion models
                        if(mech=='c13'): aterm[i,j,t]= 3.0 * (1.0 - data['concdata']['X'][i][j])**(1.0/3.0)*(2.0*((1.0-data['concdata']['X'][i][j])**(-1.0/3.0)-1.0))**(-1) #Jander equation (3d difsusion spherical sym)
                        if(mech=='c14'): aterm[i,j,t]= 3.0/2.0 * (1.0 + data['concdata']['X'][i][j])**(2.0/3.0)*(((1.0+data['concdata']['X'][i][j])**(-1.0/3.0)-1.0))**(-1) #antijander
                        if(mech=='c15'): aterm[i,j,t]= 1.0 / (-np.log(1.0-data['concdata']['X'][i][j])) # valensi
                        if(mech=='c16'): aterm[i,j,t]= 1.0 / (2.0 * data['concdata']['X'][i][j]) # parabolic
                        if(mech=='c17'): aterm[i,j,t]= (3.0/2.0)*(((1.0-data['concdata']['X'][i][j])**(-1.0/3.0)-1.0))**(-1)  #Ginstling-Brounstein eq 3d diff cylindrical
                        if(mech=='c18'): aterm[i,j,t]= 3.0/2.0 * (1.0 - data['concdata']['X'][i][j])**(4.0/3.0)*(((1.0-data['concdata']['X'][i][j])**(-1.0/3.0)-1.0))**(-1) #zhuralev, lesohkhin, tempelman
                        if(mech=='c19'): aterm[i,j,t]=(1.0 - data['concdata']['X'][i][j])**(2.0/3.0) 
#                    if(mech=='c19'): aterm[i,j,t]=  (0.2)**(0.535) * 1.5 * (0.112)**(1.0/1.5)*((1.0)-data['concdata']['X'][i][j])*(-np.log(1.0-data['concdata']['X'][i][j]))**(1.0-1.0/1.5)                                                                                                                                                                      #                    if(mech=='c20'): aterm[i,j,t]=  (0.3)**(0.312) * 2.76 * (11.24)**(1.0/2.76)*((0.18 *1.479)-data['concdata']['X'][i][j])*(-np.log(1.0-data['concdata']['X'][i][j])/(0.18 *1.479))**(1.0-1.0/2.76)                                                                                                                                              #                    if(mech=='c21'): aterm[i,j,t]=  (0.3)**(2.13) * 1.63 * (28.4)**(1.0/1.63)*((0.135*1.479)-data['concdata']['X'][i][j])*(-np.log(1.0-data['concdata']['X'][i][j])/(0.135*1.479))**(1.0-1.0/1.63)               
        ...
        for m in range(data['rxnet']['mechanisms']):
            ...
            for s in data['otherdata']['slabels']:
                if data['rxnet'][i,s] < 0:
                    tempa = tempa * float(data['concdata'][s][i][j])**float(abs(data['rxnet'][i,s]))
                    aterm[i,j,m] = tempa
    ...
    if len(data['otherdata']['slabels']) == 1:
        if 'T' not in data['otherdata']['pclabels']:
            ...
            r = alamopy.doalamo(np.asarray(aterm[0,:,:]), np.asarray(data['ratedata'][str(data['otherdata']['slabels'][0])][0,:]),linfcns=1,constant=0,maxterms=1,expandoutput=1)
            ...
            slist = ['Selected model is : '+rspace['clcnames'][int(selected)-1],'Selected model form : '+rspace['clcforms'][int(selected)-1],'Estimated rate constant : '+cfi]
...
#            r.write('set pc /1*'+str(ndatadic['T'])+'/;\n')
                for i in range(ndatadic['T']):
                    r.write("temp('"+str(i+1)+"')="+str(data['procdata']['T'][i])+";\n")
            else:
        ...
        if ( 'time' in data['procdata'] ):
            ...
            for i in range(ndatadic['T']):
                for j in range(ndatadic['time']):
                    r.write("t('"+str(i+1)+"','"+str(j+1)+"')="+str(data['procdata']['time'][i][j])+";\n")
        ...
        if ( data['otherdata']['npc'] > 0):
            if 'time' in data['procdata']:
                r.write('parameters conc(pc,time,s),rate(pc,time,s),aterm(pc,time,m);\n')
#                r.write('parameters conc(pc,time,s),rate(pc,time,s),aterm(pc,time,rc);\n')
                for i in range(ndatadic['T']):
                    for j in range(ndatadic['time']):
                        for s in data['otherdata']['slabels']:
                            r.write("conc('"+str(i+1)+"','"+str(j+1)+"','"+s+"')="+str(data['concdata'][s][i][j])+";\n")
                            r.write("rate('"+str(i+1)+"','"+str(j+1)+"','"+s+"')="+str(data['ratedata'][s][i][j])+";\n")
                            for q in range (data['rxnet']['mechanisms']):
                                r.write("aterm('"+str(i+1)+"','"+str(j+1)+"','"+str(q+1)+"')="+str(aterm[i,j,q])+";\n")
                ...
                for i in range(data['otherdata']['npc']):
                    for s in data['otherdata']['slabels']:
                        r.write("conc('"+str(i+1)+"','"+s+"')="+str(data['concdata'][s][i])+";\n")
                        r.write("rate('"+str(i+1)+"','"+s+"')="+str(data['ratedata'][s][i])+";\n")
       
        elif ( 'time' in data['procdata'] ):
            r.write('parameters conc(time,s),rate(time,s),aterm(time,m);\n')
            for j in range(ndatadic['time']):
                for s in data['otherdata']['slabels']:
                    r.write("conc('"+str(j+1)+"','"+s+"')="+str(data['concdata'][s][0][j])+";\n")
                    r.write("rate('"+str(j+1)+"','"+s+"')="+str(data['ratedata'][s][0][j])+";\n")
                for m in range(len(data['otherdata']['clc'])):
                    r.write("aterm('"+str(j+1)+"','"+str(m+1)+"')="+str(aterm[0,j,m])+";\n")
        if ('ads' in data['otherdata']['mechanisms']):
            for i in data['otherdata']['adslabel']:
                ...
                for j in data['otherdata']['slabels']:
                    r.write(i+"(pc,'"+j+"')="+data['procdata']['adata'][i][j]+";\n")
        if(1==0):
#        if ( 'adata' in data['procdata'] ):
            ...
            r.write(' /;\n')
            r.write('set noads(s) / ')
            for s in [val for val in data['otherdata']['slabels'] if val not in data['otherdata']['adsspec']]:
                r.write(s)
                if (s != [val for val in data['otherdata']['slabels'] if val not in data['otherdata']['adsspec']][-1]):
                    r.write(',')
            r.write(' /;\n')
            ...
            if ( 'time' in data['procdata']):
                ...
                for n in range(data['otherdata']['npc']):
                    for x in range(data['rxnet']['nrx']):
                        for t in range(data['otherdata']['ntimedata']):
                            for s in data['otherdata']['slabels']:
                                if (data['rxnet'][x,s] < 0 and s in data['otherdata']['adsspec']):
                                    r.write("aax('"+str(n+1)+"','"+str(x+1)+"','"+str(t+1)+"')=kads('")
                                    r.write(str(n+1)+"','"+s+"')/(1+sum(s1,conc('"+str(n+1)+"','"+str(t+1)+"',s1)")
                                    r.write("*kads('"+str(n+1)+"',s1)));\n")
                                elif(data['rxnet'][x,s] < 0 and s not in data['otherdata']['adsspec']):
                              #  else:                                                                                                                                            
                                    r.write("aax('"+str(n+1)+"','"+str(x+1)+"','"+str(t+1)+"')=1.0;\n")
                ...
                for n in range(data['otherdata']['npc']):
                    for x in range(data['rxnet']['nrx']):
                        swtch=0
                        for s in data['otherdata']['slabels']:
                            if ((data['rxnet'][x,s] < 0) and s in data['otherdata']['adsspec'] and swtch == 0):
                                r.write("aax('"+str(n+1)+"','"+str(x+1)+"')=kads('")
                                r.write(str(n+1)+"','"+s+"')/(1+sum(s1,conc('"+str(n+1)+"',s1)")
                                r.write("*kads('"+str(n+1)+"',s1)));\n")
                                swtch=1
                            elif((data['rxnet'][x,s] < 0) and s not in data['otherdata']['adsspec'] and swtch == 0):
                                r.write("aax('"+str(n+1)+"','"+str(x+1)+"')=1.0;\n")
                                swtch=1
            ...
            for i in data['otherdata']['revlabel']:
                ...
                for rx in range(data['rxnet']['nrx']):
                    if rx in data['rxnet']['rev']:
                        ...
                        r.write(i+"(pc,'"+str(int(rx)+1)+"')="+data['procdata']['revdata'][i][j]+";\n")
                        jj=jj+1
                    else:
            ...
            for j in range(data['rxnet']['mechanisms']):
                ...
                for s in data['otherdata']['slabels']:
                    if (1 == 1):
                        ...
                        if (mech == 'rev'):
                            if(data['rxnet'][i,s]<0):
                                r.write("rmat('"+str(j+1)+"','"+str(i+1)+"','"+s+"')="+str(data['rxnet'][i,s])+";\n")
                                r.write("pmat('"+str(j+1)+"','"+str(i+1)+"','"+s+"')="+str(0)+";\n")
                            elif(data['rxnet'][i,s]>0):
                                r.write("rmat('"+str(j+1)+"','"+str(i+1)+"','"+s+"')="+str(data['rxnet'][i,s])+";\n")
                                r.write("pmat('"+str(j+1)+"','"+str(i+1)+"','"+s+"')="+str(data['rxnet'][i,s])+";\n")
                            else:
                                r.write("rmat('"+str(j+1)+"','"+str(i+1)+"','"+s+"')="+str(0)+";\n")
                                r.write("pmat('"+str(j+1)+"','"+str(i+1)+"','"+s+"')="+str(0)+";\n")
                        elif(mech == 'ma'):
                            if (data['rxnet'][i,s]<0):
                                r.write("rmat('"+str(j+1)+"','"+str(i+1)+"','"+s+"')="+str(data['rxnet'][i,s])+";\n")
                                r.write("pmat('"+str(j+1)+"','"+str(i+1)+"','"+s+"')="+str(abs(data['rxnet'][i,s]))+";\n")
                            elif (data['rxnet'][i,s]>0):
                                r.write("rmat('"+str(j+1)+"','"+str(i+1)+"','"+s+"')="+str(data['rxnet'][i,s])+";\n")
                                r.write("pmat('"+str(j+1)+"','"+str(i+1)+"','"+s+"')="+str(0)+";\n")
                                r.write("rmat('"+str(j+1)+"','"+str(i+1)+"','"+s+"')="+str(data['rxnet'][i,s])+";\n")
...
#                            for q in data['otherdata']['clc']:                                                                                                                           
                        ...
                        r.write("rmat('"+str(j+1)+"','"+str(i+1)+"','"+s+"')=0;\n")
                        r.write("pmat('"+str(j+1)+"','"+str(i+1)+"','"+s+"')=0;\n")
...
#        if ( 'adata' in data['procdata']):
        ...
        if ( 'time' in data['procdata']):
            if (1 == data['otherdata']['eobj']):
                r.write("obj.. modelcomp =e= sum(pc,sum(s,sum(time,err(pc,time,s))));\n")
            else:
                r.write("obj.. modelcomp =e= sum(pc,sum(s,sum(time,power(rate(pc,time,s)-estrate(pc,time,s),2))));\n")
            # first write irrev model
            if ('clc' in data['otherdata']):
                if (1 == data['otherdata']['eobj']):
                    r.write("  res1(pc,time,s).. rate(pc,time,s) - estrate(pc,time,s) =l= err(pc,time,s);\n")
                    r.write("  res2(pc,time,s).. estrate(pc,time,s) - rate(pc,time,s) =l= err(pc,time,s);\n")
                r.write("  er(pc,time,s).. estrate(pc,time,s) =e=\n")
                if (data['otherdata']['npc'] > 1):
                    r.write("  sum(mclc,(sum(rx,(rmat(mclc,rx,s)*k(rx,mclc)*exp((-1*E(rx,mclc))/(8.314*(temp(pc))))*\n")
                else:
                ...
                r.write("  er(pc,time,s).. estrate(pc,time,s) =e=\n")
                r.write("  (sum(rx,(rmat('1',rx,s)*k(rx,'1')*exp(-E(rx,'1')/(8.314*temp(pc)))*\n")
...
#            if ( 'adata' in data['procdata']):
            ...
            if ( 'rev' in data['otherdata']['mechanisms']):
                ...
                r.write("-(prod(s3,power(conc(pc,time,s3),revp('2',rx,s3)))/keq(pc))))))),2))));\n")
        else:
            mcount = 1                                
            # Input reactor is not dynamic 
            r.write("obj.. modelcomp =e= sum(pc,sum(s,(1/sigest(s))*power((rate(pc,s)-estrate(pc,s)),2)));\n")
            # Estiamted rate
            r.write("  er(pc,s).. estrate(pc,s) =e=\n")
            if 'iT' in data['otherdata']['mechanisms']:
                mcount = 1
                r.write("  sum(rx,(rmat('"+str(mcount)+"',rx,s)*k(rx,'"+str(mcount)+"')*\n")
                if len(data['otherdata']['mechanisms']) > 1:
                    r.write("  (prod(s1,power(conc(pc,s1),pmat('"+str(mcount)+"',rx,s1)))))+(\n")
                else:
                    r.write("  (prod(s1,power(conc(pc,s1),pmat('"+str(mcount)+"',rx,s1))))));\n")
...
#                if ( 'adata' in data['procdata']):
                    r.write("adsaux(pc,rx)*\n")
                r.write("  (prod(s1,power(conc(pc,s1),pmat('1',rx,s1)))\n")
                r.write("-(prod(s3,power(conc(pc,s3),pmat('2',rx,s3)))/keq(pc,rx))))+(\n")
            ...
            if('ads' in data['otherdata']['mechanisms']):
                ...
                r.write("sum(s1,kads(pc,s1)*conc(pc,s1))/power(1+sum(s1,kads(pc,s1)*conc(pc,s1)),2)*\n")
                r.write("  (prod(s1,power(conc(pc,s1),pmat('1',rx,s1))))));\n")
#            r.write("))),2)));\n")
        ...
        if ( 'rev' in data['otherdata']['mechanisms']):
            for i in range(data['rxnet']['nrx']):
                if ( i in data['rxnet']['rev']):
                    ...
                    r.write("req"+str(i+1)+".. y('"+str(i+1)+"','2')+y('"+str(i+2)+"','1')=l=1;\n")
...
#            r.write("E.l('1','19')="+str(55319)+";\n")               
            ...
            for i in range(1,len(debug['true']['k'])+1):
                ...
                r.write("k.fx('"+str(i)+"','1')="+str(debug['true']['k'][i-1])+";\n")
                if(data['otherdata']['npc'] > 1): r.write("E.fx('"+str(i)+"','1')="+str(debug['true']['e'][i-1])+";\n")
    #    r.write("solve ccmodel minimizing modelcomp using minlp;\n") 
        ...
        if ('clc' in data['otherdata']):
            ...
            r.write("resid(s) = sum(pc, power( rate(pc,s) - (sum(rx,(rmat('1',rx,s)*k.l(rx,'1')*(prod(s1,power(conc(pc,s1),pmat('1',rx,s1))))))),2));\n")
            r.write("file rates;\n")
            r.write('put rates;\n')
            r.write('  loop(s,\n')
