...
The control volumes are configured the same as the ``ControlVolume0DBlock`` in the Heater model 
(link broken). The ``HeatExchanger`` model contains additional constraints that calculate the 
amount of heat transferred from the hot side to the cold side.
...
The default constants can be overridden by providing :ref:`alternative rules <technical_specs/model_libraries/generic/unit/heat_exchanger:Callbacks>` for
...
.. Note::
  ...
  the hot and cold sides (``shell`` and ``tube`` by default) as in :ref:`the example <technical_specs/model_libraries/generic/unit/heat_exchanger:Example>`.
