...
Subcritical Power Plant (simplified description)
...
* Helmholtz Equation of State (IAPWS95): Water and steam
* IDEAL GAS: Air and Flue Gas

Introduction
++++++++++++

A 300 MW subcritical coal-fired power plant is modeled using the unit model library has been developed for demonstration purposes only. 
...
The platen superheater panels are parallel to the furnace side walls.  The boiler has one drum, eight downcomers, and a backpass with two reheaters, a primary superheater, an economizer, and the air preheater (this model is a simplified tri-sector Ljungström type)

The steam cycle equipment includes a typical multistage turbine with single reheat.  It has a throttle valve, multiple stages for HP, IP, and LP sections with steam extraction to 3 low-pressure feed water heaters and 2 high-pressure feed water heaters as well as a deaerator.  There is also a turbine for main boiler feed pump. The steam cycle also includes the main and auxiliary condensers, a hotwell tank, a condensate pump, a booster pump and a main pump.  Multiple control valves are used to control the water levels of hotwell tank, deaerator tank, and feed water heaters and the spray flow to main steam attemperator.
...
The dynamic model considers the mass and energy inventories in large vessels in the system including drum, deaerator, feed water heaters, and condenser hotwell.  Meanwhile, inventories in downcomers and multiple waterwall zones are also considered.  Due to the limitation of the solver, the inventory in the 2D boiler heat exchanger model for the reheaters, the primary superheater, and the economizer are not considered.  However, the internal energy held by the tube metal of those heat exchangers are considered.  Some unit models are treated as the steady-state models on the dynamic flowsheet.  For example, the boiler fire-side model is assumed as steady-state since the flue gas density is low and so is the residence time (2-3 seconds).  All turbine stages, condensers and pumps are modeled as steady state.  The entire process is totally pressure driven, indicating the flow rates of air, flue gas, water, and steam are related to the pressures in the system.
...
=============== ========================== ================================ ===========
aBoiler         Boiler fire-side surrogate BoilerFireside                   False
aDrum           1D boiler drum             Drum1D                           True
blowdown_split  Splitter for blowdown      HelmSplitter                     False
aDowncomer      Downcomer                  Downcomer                        True
Waterwalls      12 waterwall zones         WaterwallSection                 True
aRoof           Roof superheater           SteamHeater                      False
aPlaten         Platen superheater         SteamHeater                      False
aRH1            2D Cold reheater           HeatExchangerCrossFlow2D_Header  True *
aRH2            2D Hot reheater            HeatExchangerCrossFlow2D_Header  True *
aPSH            2D Primary superheater     HeatExchangerCrossFlow2D_Header  True *
aECON           2D Economizer              HeatExchangerCrossFlow2D_Header  True *
aPipe           Pipes from eco. to drum    WaterPipe                        False
Mixer_PA        Mixer of hot PA and TA     Mixer                            False
Attemp          Attemperator               HelmMixer                        False
aAPH            Air preheater              HeatExchangerWith3Streams        False
=============== ========================== ================================ ===========
...
Controllers are also included in the dynamic flowsheet.  There are several regulatory level controllers, all of which are either PI controllers, including those to control the levels of FWH2, FWH3, FWH5, FWH6, deaerator tank, and hotwell tank.  The corresponding control valves involved in those level controllers are drain valves of FWH2, FWH3, FWH5, FWH6, the condensate valve, and the makeup water valve, respectively.  The controller for the makeup water is configured to have the default valve opening bounds between 0.05 and 1.
