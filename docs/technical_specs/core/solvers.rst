...
:ref:`utility function <technical_specs/core/util/misc:get_solver>` (``idaes.core.util.misc.get_solver``)
...
(:ref:`documented here<user_guide/logging:idaes.solve Loggers>`).
IDAES also has features for redirecting solver output to a log (see
:ref:`Logging Solver Output<user_guide/logging:Logging Solver Output>`).
