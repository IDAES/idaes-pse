...
Property packages represent the core of any process model, and having a suitable property package is key to successfully modeling any process system. However, developing property packages is a significant challenge even for the most experienced modelers as they involve complex, non-linear equations. The goal of the IDAES Generic Property Package Framework is to provide a flexible platform on which users can build property packages for common types of systems by calling upon libraries of modular sub-models to build up complex property calculations with the least effort possible.
...
4. (Optional) Define any :ref:`phase equilibria<model_libraries/core_library/property_models/general/phase_equilibrium:Phase Equilibrium Formulations>` which occurs in the system and methods associated with calculating this.
