...
``Initializers`` intended for “add-on” type models (i.e., models attached to other models after the parent model has been constructed) and used as part of  hierarchical initialization routine should also implement the ``addon_prepare``, ``addon_initialize`` and ``addon_finalize`` methods as necessary.
