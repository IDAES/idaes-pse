...
The `state_bounds` configuration argument should be a `dict` where the keys are the names of the state variables (using the standard naming convention) and the values a tuple with the form `(lower, nominal, upper, units)`. The `lower` and `upper` values are used to set the lower and upper bounds respectively, whilst the `nominal` value is used to set the initial value for the state variable. The `units` value is option, and is used to specify the units of measurement for the values provided, which will be used to automatically convert these values to the base set of units defined for the property package if required. If the `units` value is omitted, it is assumed that the values provided are in the base unit set for the property package.
