...
def test_build():
    ...
    assert not hasattr(m.fs.tray, "liq_side_draw")
    assert m.fs.tray.config.has_vapor_side_draw
    assert hasattr(m.fs.tray, "vap_side_sf")
    assert hasattr(m.fs.tray, "vap_side_draw")


def test_set_inputs():

    # Check variables and constraints when using FTPz
    assert number_variables(m.fs.tray) == 72
...
def test_solution():
    ...
    # side vap_out port
    assert (pytest.approx(0.76831, abs=1e-3) ==
            value(m.fs.tray.vap_side_draw.flow_mol[0]))
    assert (pytest.approx(0.55031, abs=1e-3) ==
            value(m.fs.tray.vap_side_draw.mole_frac_comp[0, "benzene"]))
    assert (pytest.approx(0.44968, abs=1e-3) ==
            value(m.fs.tray.vap_side_draw.mole_frac_comp[0, "toluene"]))
    assert (pytest.approx(370.567, abs=1e-3) ==
            value(m.fs.tray.vap_side_draw.temperature[0]))
    assert (pytest.approx(101325, abs=1e-3) ==
            value(m.fs.tray.vap_side_draw.pressure[0]))
