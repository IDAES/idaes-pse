...
def test_build():
    ...
    assert m.fs.tray.config.has_liquid_side_draw
    assert hasattr(m.fs.tray, "liq_side_sf")
    assert hasattr(m.fs.tray, "liq_side_draw")
...
def test_solution():
    ...
    # side liq_out port
    assert (pytest.approx(0.23168, abs=1e-3) ==
            value(m.fs.tray.liq_side_draw.flow_mol[0]))
    assert (pytest.approx(0.33313, abs=1e-3) ==
            value(m.fs.tray.liq_side_draw.mole_frac_comp[0, "benzene"]))
    assert (pytest.approx(0.66686, abs=1e-3) ==
            value(m.fs.tray.liq_side_draw.mole_frac_comp[0, "toluene"]))
    assert (pytest.approx(370.567, abs=1e-3) ==
            value(m.fs.tray.liq_side_draw.temperature[0]))
    assert (pytest.approx(101325, abs=1e-3) ==
            value(m.fs.tray.liq_side_draw.pressure[0]))
