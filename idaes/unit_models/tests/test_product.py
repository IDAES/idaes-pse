...
def test_build():
    ...
    assert len(m.fs.prod.inlet[0].vars) == 4
    assert hasattr(m.fs.prod.inlet[0], "flow_vol")
    assert hasattr(m.fs.prod.inlet[0], "conc_mol_comp")
    assert hasattr(m.fs.prod.inlet[0], "temperature")
    assert hasattr(m.fs.prod.inlet[0], "pressure")
...
def test_initialize():
    ...
    assert m.fs.prod.inlet[0].flow_vol== 1.0e-03
    assert m.fs.prod.inlet[0].conc_mol_comp["H2O"] == 55388.0
    assert m.fs.prod.inlet[0].conc_mol_comp["NaOH"] == 100.0
    assert m.fs.prod.inlet[0].conc_mol_comp["EthylAcetate"] == 100.0
    assert m.fs.prod.inlet[0].conc_mol_comp["SodiumAcetate"] == 0.0
    assert m.fs.prod.inlet[0].conc_mol_comp["Ethanol"] == 0.0
