...
from pyomo.environ import ConcreteModel, ExternalFunction, value
...
def root_finder():
    ...
    m.proc_Z_liq_x = ExternalFunction(library=_so, function="ceos_z_liq_extend")
    m.proc_Z_vap_x = ExternalFunction(library=_so, function="ceos_z_vap_extend")
        ...
        for T in t_set:
            for P in p_set:
                ...
                try:
                    ...
                    # converge to root
                    assert pytest.approx(0, abs=TOL) == res or pytest.approx(
                        0, abs=TOL
                    ) == value(res / dz)
                    ...
                    if TEST_DERS:
                        ...
                        if not A_skip:
                            assert (
                                pytest.approx(dZdA_p, FD_TOL) == g[1]
                                or pytest.approx(dZdA_m, FD_TOL) == g[1]
                                or (
                                    between(g[1], dZdA_p, dZdA_m)
                                    and (
                                        pytest.approx(dZdA_p, 10 * FD_TOL) == g[1]
                                        or pytest.approx(dZdA_m, 10 * FD_TOL) == g[1]
                                    )
                                )
                            )
                        ...
                        if not B_skip:
                            assert (
                                pytest.approx(dZdB_p, FD_TOL) == g[2]
                                or pytest.approx(dZdB_m, FD_TOL) == g[2]
                                or (
                                    between(g[2], dZdB_p, dZdB_m)
                                    and (
                                        pytest.approx(dZdB_p, 10 * FD_TOL) == g[2]
                                        or pytest.approx(dZdB_m, 10 * FD_TOL) == g[2]
                                    )
                                )
                            )
                        ...
                        if not A_skip:
                            assert (
                                pytest.approx(d2ZdA2_p, FD_TOL) == h[2]
                                or pytest.approx(d2ZdA2_m, FD_TOL) == h[2]
                                or between(h[2], d2ZdA2_p, d2ZdA2_m)
                            )
                        if not A_skip and not B_skip:
                            assert (
                                pytest.approx(d2ZdAB_p, FD_TOL) == h[4]
                                or pytest.approx(d2ZdAB_m, FD_TOL) == h[4]
                                or between(h[4], d2ZdAB_p, d2ZdAB_m)
                            )
                        if not B_skip:
                            ...
                            # this point
                            assert (
                                pytest.approx(d2ZdB2_p, FD_TOL) == h[5]
                                or pytest.approx(d2ZdB2_m, FD_TOL) == h[5]
                                or between(h[5], d2ZdB2_p, d2ZdB2_m)
                            )
            ...
            for P in p_set:
                ...
                if TEST_DERS:
                    try:
                        ...
                        # In these cases, skip the derivative tests.
                        if abs(ZAp - Z) > 1e-3 or abs(ZAm - Z) > 1e-3 or abs(a) < 1e-1:
                            A_skip = True
                        else:
                            A_skip = False
                        if (
            ...
            for P in p_set:
                ...
                if TEST_DERS:
                    try:
                        ...
                        # the partial derivatvies will be very sensitive.
                        # In these cases, skip the derivative tests.
                        if abs(ZAp - Z) > 1e-3 or abs(ZAm - Z) > 1e-3 or abs(a) < 0.5:
                            A_skip = True
                        else:
                            A_skip = False
                        if (
