...
@pytest.mark.skipif(not swco2.swco2_available(), reason="Property lib not available")
class TestHelm(object):
    ...
    def tst_hs_state(self, model):
        ...
        model.te.add_funcs(names=["func_h", "func_s"])
    ...
    def test_external_memo(self, model):
        ...
        for i in [il, ig]:
            ...
            assert value(model.memo_test_tau(h / mw / 1000, p / 1000)) == pytest.approx(
                Tc / T, rel=0.10
            )
    ...
    def test_thermo_expression_writter(self, model):
        ...
        for i, T in enumerate(data["T"]):
            ...
            assert value(te.s(u=u, p=p)) == pytest.approx(value(s), rel=0.05)
