...
m.fs.properties_ideal_l = BTXParameterBlock(
    default={"valid_phase": "Liq", "activity_coeff_model": "Ideal"}
)
...
m.fs.properties_ideal_v = BTXParameterBlock(
    default={"valid_phase": "Vap", "activity_coeff_model": "Ideal"}
)
...
def test_build_inlet_state_block():
    ...
    # vapor-liquid (ideal)
    assert m.fs.properties_ideal_vl_FTPz.config.valid_phase == (
        "Vap",
        "Liq",
    ) or m.fs.properties_ideal_vl_FTPz.config.valid_phase == ("Liq", "Vap")
...
def test_solve():
    ...
    assert value(
        m.fs.state_block_ideal_vl_FTPz.mole_frac_phase_comp["Liq", "benzene"]
    ) == pytest.approx(0.4121, abs=1e-3)
    assert value(
        m.fs.state_block_ideal_vl_FTPz.mole_frac_phase_comp["Vap", "benzene"]
    ...
    # Check for results
    assert value(
        m.fs.state_block_ideal_l.mole_frac_phase_comp["Liq", "benzene"]
    ) == pytest.approx(0.5, abs=1e-3)
    assert value(
        m.fs.state_block_ideal_l.mole_frac_phase_comp["Liq", "toluene"]
    ) == pytest.approx(0.5, abs=1e-3)
    ...
    # Check for results
    assert value(
        m.fs.state_block_ideal_v.mole_frac_phase_comp["Vap", "benzene"]
    ) == pytest.approx(0.5, abs=1e-3)
    assert value(
        m.fs.state_block_ideal_v.mole_frac_phase_comp["Vap", "toluene"]
    ) == pytest.approx(0.5, abs=1e-3)
