...
class TestGenericReactionBlock(object):
    ...
    def test_basic_scaler_object(self, model):
        ...
        assert get_scaling_factor(model.rblock[1].equilibrium_constraint["e2"]) == 1 # log constraint
        ...
        # Scaling hints
        assert get_scaling_factor(model.rblock[1].dh_rxn["e1"]) == pytest.approx( 1 / (300 * 8.3144), rel = 1e-3)
        assert get_scaling_factor(model.rblock[1].dh_rxn["e2"]) == pytest.approx( 1 / (300 * 8.3144), rel = 1e-3)
        assert get_scaling_factor(model.rblock[1].dh_rxn["r1"]) == pytest.approx( 1 / (300 * 8.3144), rel = 1e-3)
