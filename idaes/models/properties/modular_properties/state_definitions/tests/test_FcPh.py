...
class Test1PhaseDefinedStateFalseNoBounds(object):
    ...
    def frame(self):
        ...
        m.props[1].enth_mol_phase = Var(
            m.params.phase_list, initialize=1, units=pyunits.J / pyunits.mol
        )
    ...
    def test_vars(self, frame):
        ...
        assert check_units_equivalent(frame.props[1].enth_mol, pyunits.J / pyunits.mols)
    ...
    def frame(self):
        ...
        m.params = DummyParameterBlock(
            ...
            state_bounds={
                ...
                "pressure": (100000.0, 100000.0, 500000.0),
                "enth_mol": (0, 500, 1000),
            },
            base_units={
                "time": pyunits.s,
...
class TestCommon(object):
    ...
    def test_calculate_scaling_factors(self, frame):
        ...
        assert len(frame.props[1].scaling_factor) == 26
    ...
    def test_cloning(self, frame):
        ...
        for p, j in blk.params._phase_component_set:
            assert blk.props[1].get_material_flow_terms(p, j).parent_block() is blk.props[1]

        # Test get_enthalpy_flow_terms method
        for p in blk.params.phase_list:
            assert blk.props[1].get_enthalpy_flow_terms(p).parent_block() is blk.props[1]

        # Test get_material_density_terms
        for p, j in blk.params._phase_component_set:
            assert blk.props[1].get_material_density_terms(p, j).parent_block() is blk.props[1]

        # Test get_energy_Density_terms
        for p in blk.params.phase_list:
            assert blk.props[1].get_energy_density_terms(p).parent_block() is blk.props[1]
