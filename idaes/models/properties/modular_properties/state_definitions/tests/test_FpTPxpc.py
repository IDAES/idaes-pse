...
from idaes.models.properties.modular_properties.state_definitions.FpTPxpc import (
    FpTPxpc,
    define_state,
    set_metadata,
)
...
class TestInvalidBounds(object):
    ...
    def test_mole_frac(self, caplog):
        ...
        m.params = DummyParameterBlock(
            ...
            state_bounds={"mole_frac_phase_comp": (None, None, None)},
        )
        ...
        assert (
            "props[1] - found state_bounds argument for mole_frac_phase_comp."
...
class Test1PhaseDefinedStateFalseNoBounds(object):
    ...
    def test_vars(self, frame):
        ...
        # phase_frac Expression line 168
    ...
    def test_expressions(self, frame):
        ...
        # flow_mol_phase_comp Expression line 133
        ...
        for p, i in frame.props[1].flow_mol_phase_comp:
            assert (p, i) in frame.props[1].params._phase_component_set
            assert str(frame.props[1].flow_mol_phase_comp[p, i].expr) == str(
                frame.props[1].flow_mol_phase[p] * frame.props[1].mole_frac_phase_comp[p, i]
            )
    ...
    def test_constraints(self, frame):
        ...
        # sum_mole_frac_out Constraint line 179
        ...
        for p in frame.props[1].phase_frac:
            assert p in frame.params.phase_list
            assert str(frame.props[1].sum_mole_frac_out[p].body) == str(
                sum(frame.props[1].mole_frac_phase_comp[p, i] for i in frame.props[1].params.component_list if (p, i) in frame.props[1].params._phase_component_set)
            )

        # mole_frac_comp_eq Constraint line 183
        ...
        for i in frame.props[1].mole_frac_comp_eq:
            assert str(frame.props[1].mole_frac_comp_eq[i].body) == str(
                frame.props[1].mole_frac_comp[i] * frame.props[1].flow_mol -
                sum(
                    frame.props[1].mole_frac_phase_comp[p, i] * frame.props[1].flow_mol_phase[p]
                    for p in frame.props[1].params.phase_list
                    if (p, i) in frame.props[1].params._phase_component_set
                )
            )
        ...
        for p in frame.props[1].phase_frac_eqn:
            assert p in frame.params.phase_list
            assert str(frame.props[1].phase_frac_eqn[p].body) == str(frame.props[1].phase_frac[p])

        assert_units_consistent(frame.props[1])


class Test1PhaseDefinedStateTrueWithBounds(object):
    # Test define_state method with bounds and defined_State = True
    @pytest.fixture(scope="class")
    def frame(self):
        m = ConcreteModel()
    ...
    def test_vars(self, frame):
        ...
        for p in frame.props[1].flow_mol_phase:
            assert p in frame.params.phase_list
            assert frame.props[1].flow_mol_phase[p].value == 100
            assert frame.props[1].flow_mol_phase[p].lb == 0
            assert frame.props[1].flow_mol_phase[p].ub == 200
        assert check_units_equivalent(
            frame.props[1].flow_mol_phase, pyunits.mol / pyunits.s
        )
...
class Test2PhaseDefinedStateFalseNoBounds(object):
    ...
    def test_constraints(self, frame):
        ...
        # sum_mole_frac_out Constraint line 179
        assert isinstance(frame.props[1].sum_mole_frac_out, Constraint)
        assert len(frame.props[1].sum_mole_frac_out) == 2
        for p in frame.props[1].phase_frac:
            assert p in frame.params.phase_list
            assert str(frame.props[1].sum_mole_frac_out[p].body) == str(
                sum(frame.props[1].mole_frac_phase_comp[p, i] for i in frame.props[1].params.component_list if (p, i) in frame.props[1].params._phase_component_set)
        ...
        for p in frame.props[1].phase_frac_eqn:
            assert p in frame.params.phase_list
            assert str(frame.props[1].phase_frac_eqn[p].body) == str(frame.props[1].phase_frac[p] * frame.props[1].flow_mol - frame.props[1].flow_mol_phase[p])
...
class Test3PhaseDefinedStateFalseNoBounds(object):
    ...
    def test_constraints(self, frame):
        ...
        # sum_mole_frac_out Constraint line 179
        assert isinstance(frame.props[1].sum_mole_frac_out, Constraint)
        assert len(frame.props[1].sum_mole_frac_out) == 3
        for p in frame.props[1].phase_frac:
            assert p in frame.params.phase_list
            assert str(frame.props[1].sum_mole_frac_out[p].body) == str(
                sum(frame.props[1].mole_frac_phase_comp[p, i] for i in frame.props[1].params.component_list if (p, i) in frame.props[1].params._phase_component_set)
...
class TestCommon(object):
    ...
    def frame(self):
        ...
        m.params = DummyParameterBlock(
            ...
            temperature_ref=300,
            state_bounds={
                "flow_mol_phase": (0, 0.1, 0.2, pyunits.kmol / pyunits.s),
    ...
    def test_define_state_vars(self, frame):
        assert frame.props[1].define_state_vars() == {
            "flow_mol_phase": frame.props[1].flow_mol_phase,
            "mole_frac_phase_comp": frame.props[1].mole_frac_phase_comp,
