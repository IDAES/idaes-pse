        ...
        for j in model.components_in_phase(p):
            ...
            if mw is None:
                if model.is_property_constructed(model.enth_mol_phase):
                    ...
                    sf_H = None
                if sf_H is None:
                    # User has already been warned about the lack of a
                    # scaling factor for enth_mol_phase in the parent method
                    continue
                sf_cp = sf_H/sf_T
            else:
            ...
            if model.is_property_constructed(model.enth_mol_phase_comp):
                self.set_component_scaling_factor(model.enth_mol_phase_comp[p, j], sf_H, overwrite=overwrite)
            if model.is_property_constructed(model.gibbs_mol_phase_comp):
                self.set_component_scaling_factor(model.gibbs_mol_phase_comp[p,j], sf_H, overwrite=overwrite)
            if model.is_property_constructed(model.entr_mol_phase_comp):
                self.set_component_scaling_factor(model.entr_mol_phase_comp[p, j], sf_cp, overwrite=overwrite)
            if model.is_property_constructed(model.energy_internal_mol_phase_comp):
                self.set_component_scaling_factor(model.energy_internal_mol_phase_comp[p, j], sf_H, overwrite=overwrite)
