...
class HelmholtzStateBlockData(StateBlockData):
    ...
    def build(self, *args):
        ...
        def rule_enth_mol_phase(b, p):
            if (
                params.config.phase_presentation == PhaseType.LG
                or params.config.phase_presentation == PhaseType.MIX
            ):
                return self.func_h(delta[p], tau) * params.uc_kJ_per_kg_to_J_per_mol
            elif (
                params.config.phase_presentation == PhaseType.L
                or params.config.phase_presentation == PhaseType.G
            ):
                return self.enth_mol
            else:
                # The only time this should be encountered is if somebody's enhancing this property framework.
                return BurntToast(
                    f"Unknown phase presentation {params.config.phase_presentation} encountered."
                )
