...
from idaes.models.properties.core.pure import NIST
...
def model():
    ...
    m.props = m.params.state_block_class(default={"defined_state": True,
                      "parameters": m.params,
                      "has_phase_equilibrium": False})
    configuration["phases"]["Vap"]["equation_of_state"] = Ideal
    m.params_IG = GenericParameterBlock(default=configuration)
    m.props_IG = m.params.state_block_class(default={"defined_state": True,
...
class CoolPropTester():
    ...
    def __init__(self,mole_frac_comp):
        ...
        for comp in self.component_list:
            self.Href[comp] = CP.PropsSI("HMOLAR", "T", standard_temp,
                                         "P", standard_pressure, "PR::"+comp)
            self.Sref[comp] = CP.PropsSI("SMOLAR", "T", standard_temp,
                                         "P", standard_pressure, "PR::"+comp)
    ...
    def enth_mol(self):
        return self.AS.hmolar()-sum(self.mole_frac_comp[comp]*self.Href[comp] 
                                    for comp in self.component_list)
    
    def entr_mol(self):
        return self.AS.smolar()-sum(self.mole_frac_comp[comp]*self.Sref[comp] 
                                    for comp in self.component_list)
    
    def gibbs_mol(self):
        return self.AS.gibbsmolar() -sum(self.mole_frac_comp[comp]
                                          *(self.Href[comp] 
                                            - self.AS.T()*self.Sref[comp])
                                    for comp in self.component_list)
    ...
    def gibbs_res_mol(self):
        return sum(self.mole_frac_comp[comp] * self.gibbs_res_mol_comp(comp)
                   for comp in self.component_list)
...
def test_thermo(model,dataframe):
    ...
    def rule_H_res(blk,j):
        return m.H_res[j] == (1E-3*(pyunits.kJ/pyunits.J)
                        * (m.props.enth_mol_phase_comp["Vap",j] 
                            -  m.props_IG.enth_mol_phase_comp["Vap",j]))
    def rule_G_res(blk,j):
        return m.G_res[j] == (1E-3*(pyunits.kJ/pyunits.J)
                        * (m.props.gibbs_mol_phase_comp["Vap",j] 
                             - m.props_IG.gibbs_mol_phase_comp["Vap",j]))
    def rule_V(blk,j):
        return m.V[j] == (1E3*(pyunits.L/pyunits.m**3)
                            * m.props.vol_mol_phase_comp["Vap",j])
    m.H_res_eq = Constraint(m.params.component_list,
                           rule=rule_H_res)
    m.G_res_eq = Constraint(m.params.component_list,
                           rule=rule_G_res)
    m.V_eq = Constraint(m.params.component_list,
                           rule=rule_V)
    ...
    for idx,row in df.iterrows():
        ...
        for comp in tester.component_list:
            assert(value(m.props.log_mole_frac_phase_comp["Vap",comp])
                   == approx(value(log(m.props.mole_frac_phase_comp["Vap",comp]))
                             , rel=1E-6)
                   )
        ...
        assert(approx(value(m.props.entr_mol_phase["Vap"]), rel=1E-6) ==
               value(sum(m.props.mole_frac_comp[comp] 
                         *m.props.entr_mol_phase_comp["Vap",comp]
                                           for comp in tester.component_list))
               )
        
        assert(approx(value(m.props.enth_mol_phase["Vap"]), rel=1E-6) ==
               value(sum(m.props.mole_frac_comp[comp] 
                         *m.props.enth_mol_phase_comp["Vap",comp]
                                           for comp in tester.component_list))
               )
        
        assert(approx(value(m.props.gibbs_mol_phase["Vap"]), rel=1E-6) ==
               value(sum(m.props.mole_frac_comp[comp] 
                         *m.props.gibbs_mol_phase_comp["Vap",comp]
                                           for comp in tester.component_list))
               )
        
        assert(approx(value(m.props.vol_mol_phase["Vap"]), rel=1E-6) ==
               value(sum(m.props.mole_frac_comp[comp] 
                         *m.props.vol_mol_phase_comp["Vap",comp]
                                           for comp in tester.component_list))
               )
        
        for comp in tester.component_list:
            assert(approx(value(m.props.gibbs_mol_phase_comp["Vap",comp])
                          , rel=1E-6)
                    == value(m.props.enth_mol_phase_comp["Vap",comp]
                        - m.props.temperature
                            *m.props.entr_mol_phase_comp["Vap",comp])
                )
        
        assert(approx(value(m.props.entr_mol_phase["Vap"]
                            -m.props_IG.entr_mol_phase["Vap"]), rel=1E-4) 
               == tester.entr_res_mol())
        ...
        for comp in tester.component_list:
            print('Mole fraction ' + comp + ': {0:.3f}'.format(
                value(m.props.mole_frac_comp[comp])))
        
        print('')
        print('Excess enthalpy (kJ/mol):')
        print('IDAES:    {0:.4f}'.format(1E-3*value(m.props.enth_mol_phase["Vap"]
                                                    -m.props_IG.enth_mol_phase["Vap"])))
        print('CoolProp: {0:.4f}'.format(1E-3*tester.enth_res_mol()))
        print('Paper:    {0:.4f}'.format(sum(mole_frac_comp[comp] * row["H_res_"+comp]
                                    for comp in tester.component_list)))
        
        assert(approx(value(m.props.enth_mol_phase["Vap"]
                            -m.props_IG.enth_mol_phase["Vap"]), rel=1E-4) 
               == tester.enth_res_mol())
        ...
        for comp in tester.component_list:            
            ...
            if comp == "NH3":
                print(comp+':  {0:.2f}  {1:.2f}'.format(value(m.H_res[comp]),row["H_res_"+comp]))
            else:
                print(comp+':    {0:.2f}   {1:.2f}'.format(value(m.H_res[comp]),row["H_res_"+comp]))
        print("")
        
        print('Excess Gibbs energy (kJ/mol):')
        print('IDAES:    {0:.4f}'.format(1E-3*value(m.props.gibbs_mol_phase["Vap"]-m.props_IG.gibbs_mol_phase["Vap"])))
        print('CoolProp: {0:.4f}'.format(1E-3*tester.gibbs_res_mol()))
        print('Paper:    {0:.4f}'.format(sum(mole_frac_comp[comp] * row["G_res_"+comp]
                                    for comp in tester.component_list)))
        assert(approx(value(m.props.gibbs_mol_phase["Vap"]
                            -m.props_IG.gibbs_mol_phase["Vap"]), rel=1E-4) 
        ...
        for comp in tester.component_list:
            assert (approx(value(m.G_res[comp]), rel=1E-4) 
                    == 1E-3*tester.gibbs_res_mol_comp(comp))
            if comp == "NH3":
                print(comp+':  {0:.4f}  {1:.4f}  {2:.4f}'.format(
                    value(m.G_res[comp]),
                    1E-3*tester.gibbs_res_mol_comp(comp),row["G_res_"+comp]))
            else:
                print(comp+':    {0:.4f}   {1:.4f}   {2:.4f}'.format(
                    value(m.G_res[comp]),
                    1E-3*tester.gibbs_res_mol_comp(comp),row["G_res_"+comp]))
        ...
        print('Paper:    {0:.4f}'.format(sum(mole_frac_comp[comp] * row["V_"+comp]
                                    for comp in tester.component_list)))
        ...
        for comp in tester.component_list:
            ...
            if comp == "NH3":
                print(comp+':   {0:.4f}  {1:.4f}'.format(value(m.V[comp]),row["V_"+comp]))
            else:
                print(comp+':    {0:.4f}  {1:.4f}'.format(value(m.V[comp]),row["V_"+comp]))
        print("")
        print("")