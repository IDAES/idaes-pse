...
from idaes.models.properties.core.phase_equil.bubble_dew import _valid_VL_component_list
...
def calculate_teq(b, phase_pair):
    ...
    if hasattr(b, "eq_temperature_bubble"):
        _t1 = getattr(b, "_t1" + suffix)
        _t1.value = max(value(b.temperature), b.temperature_bubble[phase_pair].value)
    else:
        _t1 = b.temperature

    if hasattr(b, "eq_temperature_dew"):
        b._teq[phase_pair].value = min(_t1.value, b.temperature_dew[phase_pair].value)
    else:
