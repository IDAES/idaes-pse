...
@pytest.mark.unit
def test_variable_scaling_no_input(test_model):
...
@pytest.mark.unit
def test_variable_scaling_no_heat_deltaP():
    m = ConcreteModel()
    m.fs = FlowsheetBlock(dynamic=False)
    ...
    for v in m.fs.unit.lagrange_mult.values():
        assert m.fs.unit.scaling_factor[v] == pytest.approx(1 / (8.314 * 500), rel=1e-4)


@pytest.mark.unit
def test_variable_scaling_inlet_state(test_model):
...
@pytest.mark.unit
def test_variable_scaling_submodel_scalers(test_model):
    scaler = GibbsReactorScaler()
    ...
    assert not test_model.fs.unit.control_volume.properties_out[0]._dummy_scaler_test
