...
##############################################################################
"""
Standard IDAES PBR model.
"""
# Import Pyomo libraries
from pyomo.environ import Constraint, Var, Reference, Block, Expression
...
__author__ = "Andrew Lee, John Eslick, Alejandro Garciadego, Kanishka Ghosh"


@declare_process_block_class("PBR")
class PBRData(UnitModelBlockData):
    """
...
domain,
**default** - 3."""))
    CONFIG.declare("mass_catalyst", ConfigValue(
        default=1.1,
        description="catalyst mass",
        doc="""catalyst mass,
**default** - 1."""))
    CONFIG.declare("catalyst_density", ConfigValue(
        default=1,
        description="true density of the catalyst",
        doc="""true density of the catalyst,
**default** - 1.1"""))
    CONFIG.declare("bulk_density", ConfigValue(
        default=1,
        description="bulk density of the catalyst",
        doc="""bulk density of the catalyst,
**default** - 1.1"""))
    def build(self):
        """
        ...
        self.area = Var(initialize=1,
                        doc="area",
                        units=units.get_derived_units("area"))

        self.geometry = Constraint(expr=self.volume == self.config.mass_catalyst/self.config.bulk_density)

        self.area_from_volume = Constraint(expr=self.area == self.volume/self.length)

        def calculate_void_fraction(self):
            return 1 - self.config.bulk_density/self.config.catalyst_density
        self.epsilon = Expression(rule=calculate_void_fraction)

        self.real_area = Constraint(expr=self.control_volume.area == self.area*
                                        self.epsilon)

        # Add PBR performance equation
        @self.Constraint(self.flowsheet().config.time,
                         self.control_volume.length_domain,
                         self.config.reaction_package.rate_reaction_idx,
                         doc="PBR performance equation")
    ...
    def get_costing(self, year=None, module=costing, **kwargs):
        ...
        self.diameter_eq = Constraint(expr=self.volume
                                      == (self.length*const.pi
                                          * self.diameter**2)/4)
        module.PBR_costing(self.costing, **kwargs)
