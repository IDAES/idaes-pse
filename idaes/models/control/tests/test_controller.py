...
from idaes.models_extra.power_generation.unit_models.soc_submodels.testing import _build_test_utility
...
def test_not_dynamic_exception():
    ...
    with pytest.raises(ConfigurationError, match="PIDControllers work only with dynamic flowsheets."):
        m.fs.unit = PIDController(process_var=m.fs.process_var, manipulated_var=m.fs.manipulated_var)
...
def test_pi_control():
    ...
    unit = m.fs.unit = PIDController(process_var=m.fs.process_var, manipulated_var=m.fs.manipulated_var)
...
def test_antiwindup_no_bounds():
    ...
    with pytest.raises(ConfigurationError,
            match="User specified antiwindup method for unbounded MV."):
        unit = m.fs.unit = PIDController(
            process_var=m.fs.process_var,
            manipulated_var=m.fs.manipulated_var,
            type=ControllerType.PID,
