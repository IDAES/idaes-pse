...
##############################################################################
"""
Tests for steady-state stripper  MEA Column

Data Reference No.:  K5
Data Source:
...
def test_build(run=False):
    ...
    if run:
      ...
      assert m.fs.reg.vapor_phase.properties[0, 1].temperature.value == pytest.approx(396.79057912844183,abs=1e-4)
      assert m.fs.reg.vapor_phase.properties[0, 1].pressure.value == pytest.approx(183430,abs=1e-4)
      assert m.fs.reg.vapor_phase.properties[0, 1].flow_mol_comp['CO2'].value == pytest.approx(1.365991774692739,abs=1e-4)
      assert m.fs.reg.vapor_phase.properties[0, 1].flow_mol_comp['H2O'].value == pytest.approx(8.528339472045591,abs=1e-4)

      assert m.fs.reg.liquid_phase.properties[0, 0].temperature.value == pytest.approx(393.9197958570096,abs=1e-4)
      assert m.fs.reg.liquid_phase.properties[0, 0].pressure.value == pytest.approx(183430,abs=1e-4)
      assert m.fs.reg.liquid_phase.properties[0, 0].flow_mol_comp['CO2'].value == pytest.approx(1.6839882253072609,abs=1e-4)
      assert m.fs.reg.liquid_phase.properties[0, 0].flow_mol_comp['H2O'].value == pytest.approx(80.91902452795442,abs=1e-4)
      assert m.fs.reg.liquid_phase.properties[0, 0].flow_mol_comp['MEA'].value == pytest.approx(9.478655999999999,abs=1e-4)
