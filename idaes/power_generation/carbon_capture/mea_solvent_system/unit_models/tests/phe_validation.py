...
for t in m.fs.time:
  for i in range(len(hot_molar_flowrate)):
    ...
    m.fs.hx.hot_inlet.mole_frac[t,"CO2"].fix(xh_CO2[i])
    m.fs.hx.hot_inlet.mole_frac[t,"H2O"].fix(xh_H2O[i])
    m.fs.hx.hot_inlet.mole_frac[t,"MEA"].fix(xh_MEA[i])
    ...
    m.fs.hx.cold_inlet.mole_frac[t,"CO2"].fix(xc_CO2[i])
    m.fs.hx.cold_inlet.mole_frac[t,"H2O"].fix(xc_H2O[i])
    m.fs.hx.cold_inlet.mole_frac[t,"MEA"].fix(xc_MEA[i])
