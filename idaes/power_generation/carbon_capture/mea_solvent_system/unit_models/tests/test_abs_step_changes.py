...
def test_build(run=False):
    ...
    if run:
        ...
        for t in m.fs.time:
            rich_loading.append(
                value(m.fs.abs.liquid_phase.properties[t, 0].mole_frac_comp['CO2'] /
                      m.fs.abs.liquid_phase.properties[t, 0].mole_frac_comp['MEA']))
            percentage_capture.append(
                value(100 - 100 *
                      m.fs.abs.vapor_phase.properties[t, 1].flow_mol_comp['CO2'] /
