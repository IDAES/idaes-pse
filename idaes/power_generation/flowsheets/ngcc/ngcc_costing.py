...
def get_ngcc_costing(m, evaluate_cost=False):
    ...
    m.fs.b1 = pyo.Block()
    feedwater_flowrate = (
        m.fs.HP_ECON1.side_1.properties_in[0].flow_mol * 18.01528 * 0.00220462 * 3600
    )  # mol/s to lb/hr conversion

    if evaluate_cost:
        m.fs.b1.feedwaterflowrate = pyo.Var(initialize=pyo.value(feedwater_flowrate))
        m.fs.b1.feedwaterflowrate.fix()
        get_PP_costing(m.fs.b1, FW_accounts, m.fs.b1.feedwaterflowrate, "lb/hr", 6)
    else:
        get_PP_costing(m.fs.b1, FW_accounts, feedwater_flowrate, "lb/hr", 6)
    ...
    if evaluate_cost:
        m.fs.b2.rawwaterwithdrawal = pyo.Var(initialize=pyo.value(raw_water_withdrawal))
        m.fs.b2.rawwaterwithdrawal.fix()
        get_PP_costing(
            m.fs.b2, RW_withdraw_accounts, m.fs.b2.rawwaterwithdrawal, "gpm", 6
        )
    else:
        get_PP_costing(m.fs.b2, RW_withdraw_accounts, raw_water_withdrawal, "gpm", 6)
    ...
    fuelgas_flowrate = (
        fuelgas_flowrate
        * pyo.value(m.fs.feed_fuel1.properties[0.0].mw)
        * 1000
        * 0.00220462
        * 3600
    )  # mol/s to lb/hr conversion
    ...
    if evaluate_cost:
        m.fs.b3.fuelgasflowrate = pyo.Var(initialize=pyo.value(fuelgas_flowrate))
        m.fs.b3.fuelgasflowrate.fix()
        get_PP_costing(m.fs.b3, FuelG_accounts, m.fs.b3.fuelgasflowrate, "lb/hr", 6)
    else:
        get_PP_costing(m.fs.b3, FuelG_accounts, fuelgas_flowrate, "lb/hr", 6)
    ...
    if evaluate_cost:
        m.fs.b4.processwaterdischarge = pyo.Var(
            initialize=pyo.value(process_water_discharge)
        )
        m.fs.b4.processwaterdischarge.fix()
        get_PP_costing(
            m.fs.b4, PW_discharge_accounts, m.fs.b4.processwaterdischarge, "gpm", 6
        )
    else:
        get_PP_costing(
            m.fs.b4, PW_discharge_accounts, process_water_discharge, "gpm", 6
        )
    ...
    if evaluate_cost:
        m.fs.b5.fluegasflowrate = pyo.Var(initialize=pyo.value(fluegas_flowrate))
        m.fs.b5.fluegasflowrate.fix()
        get_PP_costing(m.fs.b5, FG_accounts, m.fs.b5.fluegasflowrate, "acfm", 6)
    else:
        get_PP_costing(m.fs.b5, FG_accounts, fluegas_flowrate, "acfm", 6)
    ...
    if evaluate_cost:
        ...
        get_PP_costing(m.fs.b6, CT_grosspower_accounts, m.fs.b6.CTgrosspower, "kW", 6)
    else:
        get_PP_costing(m.fs.b6, CT_grosspower_accounts, CT_gross_power, "kW", 6)
    ...
    HRSG_duty = (
        sum(
            m.fs.HP_SH4.side_2.properties_in[0.0].flow_mol_comp[k]
            for k in m.fs.prop_gas.component_list
        )
        * m.fs.HP_SH4.side_2.properties_in[0.0].enth_mol
        - sum(
            m.fs.LP_ECON.side_2.properties_out[0.0].flow_mol_comp[k]
            for k in m.fs.prop_gas.component_list
        )
    ...
    if evaluate_cost:
        ...
        get_PP_costing(m.fs.b7, HRSG_duty_accounts, m.fs.b7.HRSGduty, "MMBtu/hr", 6)
    else:
        get_PP_costing(m.fs.b7, HRSG_duty_accounts, HRSG_duty, "MMBtu/hr", 6)
    ...
    if evaluate_cost:
        ...
        get_PP_costing(
            m.fs.b8, Stack_flow_gas_accounts, m.fs.b8.stackflowgas, "acfm", 6
        )
    else:
        get_PP_costing(m.fs.b8, Stack_flow_gas_accounts, stack_flow_gas, "acfm", 6)
    ...
    if evaluate_cost:
        ...
        get_PP_costing(
            m.fs.b9, Steam_turbine_gross_power_accounts, m.fs.b9.STgrosspower, "kW", 6
        )
    else:
        get_PP_costing(
            m.fs.b9, Steam_turbine_gross_power_accounts, ST_gross_power, "kW", 6
        )
    ...
    if evaluate_cost:
        ...
        get_PP_costing(
            m.fs.b10, Condenser_duty_accounts, m.fs.b10.condenserduty, "MMBtu/hr", 6
        )
    else:
        get_PP_costing(m.fs.b10, Condenser_duty_accounts, condenser_duty, "MMBtu/hr", 6)
    ...
    if evaluate_cost:
        m.fs.b11.coolingtowerduty = pyo.Var(initialize=pyo.value(cooling_tower_duty))
        m.fs.b11.coolingtowerduty.fix()
        get_PP_costing(
            m.fs.b11, Cooling_tower_accounts, m.fs.b11.coolingtowerduty, "MMBtu/hr", 6
        )
    else:
        get_PP_costing(
            m.fs.b11, Cooling_tower_accounts, cooling_tower_duty, "MMBtu/hr", 6
        )
    ...
    exhaust_steam_LP_turbine = (
        m.fs.steam_turbine.outlet_stage.control_volume.properties_out[0.0].flow_mol
    )  # mol/s
    cir_water_flowrate = (
        217555 * exhaust_steam_LP_turbine / ((821097 * 453.592 / 18.01528) / 3600)
    )  # gpm

    if evaluate_cost:
        m.fs.b12.cirwaterflowrate = pyo.Var(initialize=pyo.value(cir_water_flowrate))
        m.fs.b12.cirwaterflowrate.fix()
        get_PP_costing(
            m.fs.b12, Circ_water_accounts, m.fs.b12.cirwaterflowrate, "gpm", 6
        )
    else:
        get_PP_costing(m.fs.b12, Circ_water_accounts, cir_water_flowrate, "gpm", 6)
    ...
    if evaluate_cost:
        m.fs.b13.plantgrosspower = pyo.Var(initialize=pyo.value(plant_gross_power))
        m.fs.b13.plantgrosspower.fix()
        get_PP_costing(
            m.fs.b13, plant_gross_power_accounts, m.fs.b13.plantgrosspower, "kW", 6
        )
    else:
        get_PP_costing(m.fs.b13, plant_gross_power_accounts, plant_gross_power, "kW", 6)
    ...
    if evaluate_cost:
        ...
        get_PP_costing(m.fs.b14, auxilliary_load_accounts, m.fs.b14.auxload, "kW", 6)
    else:
        get_PP_costing(m.fs.b14, auxilliary_load_accounts, aux_load, "kW", 6)
    ...
    get_PP_costing(m.fs.b15, stg_ctg_accounts, m.fs.b15.stgctgop, "kW", 6)
    ...
    if evaluate_cost:
        ...
        get_PP_costing(m.fs.b16, gasturbine_accounts, m.fs.b16.gtpower, "kW", 6)
    else:
...
def ngcc_costing_validation(m):
    ...
    assert (
        pytest.approx(
            sum(
                pyo.value(m.fs.b2.costing.total_plant_cost[ac])
    ...
    assert (
        pytest.approx(
            sum(
                pyo.value(m.fs.b4.costing.total_plant_cost[ac])
    ...
    assert (
        pytest.approx(
            sum(
                pyo.value(m.fs.b7.costing.total_plant_cost[ac])
...
def build_ngcc_OM_costs(m):
    ...
    m.fs.net_power_cost = pyo.Var(m.fs.time, initialize=650, units=pyunits.MW)
    m.fs.net_power_cost.fix()
    m.fs.natural_gas = pyo.Var(
        m.fs.time, initialize={0: 7239}, units=pyunits.MBtu / pyunits.day
    )
    ...
    m.fs.water_use = pyo.Var(
        m.fs.time, initialize={0: 2090 * 1000}, units=pyunits.gallon / pyunits.day
    )
    m.fs.water_use.fix()
    #
    get_variable_OM_costs(
...
if __name__ == "__main__":
    ...
    m.fs.flow_mass = pyo.Var(m.fs.time, initialize=27.3, units=pyunits.kg / pyunits.s)
    m.fs.lhv = pyo.Var(m.fs.time, initialize=47.2e6, units=pyunits.J / pyunits.kg)
    ...
    def eq1(c, t):
        return m.fs.natural_gas[t] == pyunits.convert(
            m.fs.flow_mass[t] * m.fs.lhv[t], pyunits.MBtu / pyunits.day
        )
