...
def get_model(m=None, init=True):
    ...
    m.fs.steam_turbine = helm.HelmTurbineMultistage(default={
    ...
    m.fs.steam_turbine_lp_mix = helm.HelmMixer(default={
        "property_package": m.fs.prop_water,
    ...
    m.fs.steam_turbine_lp_split = helm.HelmSplitter(default={
        "property_package": m.fs.prop_water,
    ...
    m.fs.return_mix = helm.HelmMixer(default={
        "property_package": m.fs.prop_water,
        "momentum_mixing_type": helm.MomentumMixingType.none,
    ...
    m.fs.t08 = Arc(
        source=m.fs.main_condenser.shell_outlet,
    ...
    m.fs.t10 = Arc(
        ...
        destination=m.fs.return_mix.pump)
    ...
    m.fs.steam_turbine.outlet_stage.control_volume.\
        properties_out[0].pressure.fix(0.01e6)
    ...
    iscale.set_scaling_factor(
        m.fs.steam_turbine.outlet_stage.control_volume.work, 1e-6)
    ...
    m.fs.steam_turbine.outlet_stage.control_volume.\
        properties_out[0].pressure.unfix()

    if init:
        print(f"Condenser pressure {pyo.value(m.fs.main_condenser.shell.properties_out[0].pressure)}")
