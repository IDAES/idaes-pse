...
def ge_model():
    ...
    m.fs.fg_translate = gum.Translator(
        default={
            "inlet_property_package": m.fs.gas_prop_params,
            "outlet_property_package": m.fs.prop_gas,
        }
    )
    m.fs.g08t = Arc(source=m.fs.exhaust_1.inlet, destination=m.fs.fg_translate.inlet)
    m.fs.g08t_tofg = Arc(
    ...
    m.fs.t05 = Arc(
        source=m.fs.LP_SH.side_1_outlet, destination=m.fs.steam_turbine_lp_mix.hrsg
    )
    ...
    print("Connect t11")
    m.fs.t11 = Arc(source=m.fs.return_mix.outlet, destination=m.fs.LP_ECON.side_1_inlet)
    ...
    m.fs.ng_preheater = HeatExchanger(
        default={
    ...
    imisc.copy_port_values(
        source=m.fs.feed_fuel1.outlet, destination=m.fs.ng_preheater.tube_inlet
    )
    imisc.copy_port_values(
        source=m.fs.IP_Splitter1.toNGPH, destination=m.fs.ng_preheater.shell_inlet
    )

    m.fs.fuel01dupe = Arc(
        source=m.fs.ng_preheater.tube_outlet, destination=m.fs.inject1.gas
    )
    m.fs.st01 = Arc(
        source=m.fs.IP_Splitter1.toNGPH, destination=m.fs.ng_preheater.shell_inlet
    )
    m.fs.st02 = Arc(
        source=m.fs.ng_preheater.shell_outlet, destination=m.fs.Mixer1.Preheater
    )
    ...
    iscale.set_scaling_factor(m.fs.ng_preheater.overall_heat_transfer_coefficient, 1e-2)
    iscale.calculate_scaling_factors(m)
    ...
    for c in [
        m.fs.HP_SH1.fcorrection_htc,
        m.fs.HP_SH2.fcorrection_htc,
        m.fs.HP_SH3.fcorrection_htc,
        m.fs.HP_SH4.fcorrection_htc,
    ]:
        c.value = pyo.value(c * 0.75)
    for c in [
        m.fs.IP_SH1.fcorrection_htc,
        m.fs.IP_SH2.fcorrection_htc,
    ...
    imisc.copy_port_values(
        source=m.fs.return_mix.reboiler, destination=m.fs.reboiler.outlet
    )
    imisc.copy_port_values(
        source=m.fs.steam_turbine_lp_split.reboiler, destination=m.fs.reboiler.inlet
    )
    m.fs.t17 = Arc(source=m.fs.reboiler.outlet, destination=m.fs.return_mix.reboiler)
    m.fs.t13 = Arc(
        source=m.fs.steam_turbine_lp_split.reboiler, destination=m.fs.reboiler.inlet
    )
    expand_arcs.apply_to(m)
    ...
    def aux_capture(b, t):  # scale to flue gas flow
        return (
            1e3 * 10600 * (b.gts2.control_volume.properties_out[t].flow_mass / 1090.759)
        )
    ...
    m.fs.fuel_lhv = pyo.Var(initialize=47.2e6, units=pyunits.J / pyunits.kg)  # J/kg
    m.fs.fuel_lhv.fix(47.2e6)

    @m.fs.Expression(m.fs.config.time)
    def fuel_thermal_in_mbtu(b, t):
        return pyunits.convert(
            m.fs.fuel_lhv * m.fs.inject1.gas_state[t].flow_mass,
    ...
    if os.path.isfile("init_control_steam.json.gz"):
        ms.from_json(
            m, fname="init_control_steam.json.gz", wts=ms.StoreSpec(suffix=False)
        )
    else:
    ...
    if os.path.isfile("init_reboiler_heat.json.gz"):
        ms.from_json(
            m, fname="init_reboiler_heat.json.gz", wts=ms.StoreSpec(suffix=False)
        )
    else:
    ...
    if os.path.isfile("init_add_costing.json.gz"):
        ms.from_json(
            m, fname="init_add_costing.json.gz", wts=ms.StoreSpec(suffix=False)
        )
    else:
...
if __name__ == "__main__":
    ...
    print(
        f"LHV Fuel thermal in = {pyo.value(m.fs.fuel_thermal_in_mbtu[0]):.2f} MBtu/hr"
    )
    print(f"Reboiler Duty = {pyo.value(-m.fs.reboiler.control_volume.heat[0]/1e6)} MW")
    gas_turbine.write_pfd_results(
        "pfd_results_gt.svg", m.tags, m.tag_format, infilename="gas_turbine.svg"
    )
    ...
    tab_out = OrderedDict(
        [
            ...
            (
                "Fixed O&M Cost ($/hr)",
                m.fs.costing.total_fixed_OM_cost / 365 / 24 * 1e6,
            ),
            (
                "Variable O&M Cost ($/hr)",
                m.fs.costing.total_variable_OM_cost[0] * (-m.fs.net_power[0] / 1e6),
            ),
        ]
    )
    ...
    for p in netpow:
        ...
        gas_turbine.write_pfd_results(
            f"pfd_results_gt_{p}.svg",
            m.tags,
            m.tag_format,
            infilename="gas_turbine.svg",
        )
