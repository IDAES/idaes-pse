...
def set_scaling_factors(m):
    ...
    if m.dynamic:
        for t, c in fs.aDrum.control_volume\
    ...
    iscale.set_scaling_factor(
        fs.aDowncomer.control_volume.energy_holdup, 1e-10)
    iscale.set_scaling_factor(
        fs.aDowncomer.control_volume.material_holdup, 1e-5)
    ...
    iscale.set_scaling_factor(
        fs.hotwell_tank.control_volume.energy_holdup, 1e-10)
    iscale.set_scaling_factor(
        fs.hotwell_tank.control_volume.material_holdup, 1e-6)
    if m.dynamic:
        for t, c in fs.hotwell_tank.control_volume\
                .energy_accumulation_disc_eq.items():
    ...
    if m.dynamic:
        for t, c in fs.da_tank.control_volume\
                .energy_accumulation_disc_eq.items():
            iscale.constraint_scaling_transform(c, 1e-7)
...
def add_overall_performance_expressions(m):
    ...
    def boiler_heat(b, t):
        return ((b.fs_stc.turb.inlet_split.mixed_state[t].flow_mol
                ...
                * (b.fs_stc.turb.inlet_split.mixed_state[t].enth_mol
                ...
                + b.fs_stc.turb.ip_stages[1].control_volume
    ...
    @m.fs_main.Expression(m.fs_main.time)
    def aux_power(b, t):
        steam_f = (m.fs_main.fs_stc.turb.inlet_split.mixed_state[t].flow_mass
                   * 7.937)
        air_f = m.fs_main.fs_blr.aBoiler.primary_air[t].flow_mass*7.937
        cw_f = (m.fs_main.fs_stc.condenser.tube.properties_in[t].flow_mass
                / 997 * 15850/1000)
        steam_t = ((m.fs_main.fs_blr.aPlaten.control_volume.properties_out[t]
                   .temperature - 273.15)*9/5 + 32)
        steam_p = (m.fs_main.fs_blr.aPlaten.control_volume
                   .properties_out[t].pressure - 79410)/6895
        air_p = m.fs_main.fs_blr.aAPH.side_2_inlet.pressure[t]/1000
        return (-7.743210e-2 * cw_f
...
def main_dynamic():
    ...
    m_dyn0 = get_model(dynamic=True,
                       time_set=[0, num_step[0]*step_size[0]],
                       nstep=num_step[0])
    ...
    if m_dyn0:
        copy_non_time_indexed_values(m_dyn0.fs_main,
                                     m_ss.fs_main,
                                     copy_fixed=True,
                                     outlvl=idaeslog.ERROR)
    if m_dyn1:
        copy_non_time_indexed_values(m_dyn1.fs_main,
                                     m_ss.fs_main,
                                     copy_fixed=True,
                                     outlvl=idaeslog.ERROR)
    ...
    m_dyn.fs_main.fs_stc.spray_ctrl.integral_of_error[:].value = \
        pyo.value(m_dyn.fs_main.fs_stc.spray_ctrl.integral_of_error_ref[t0])
    ...
    _log.info('boiler_master_setpoint={}'.format(
        m_dyn.fs_main.boiler_master_ctrl.setpoint[0].value))
    _log.info('sliding pressure={}'.format(
    ...
    for i in range(1, nperiod):
        ...
        for t in m_dyn.fs_main.time:
            if itype_list[i] != itype_list[i-1] or t != tlast:
                ...
                copy_values_at_time(m_dyn.fs_main, m_prev.fs_main, t, tlast,
                                    copy_fixed=True, outlvl=idaeslog.ERROR)
            _log.info('Spray control windup={}'.format(
            ...
            if mv_unbounded < pyo.value(
                ...
                if pyo.value(m_dyn.fs_main.fs_stc.spray_ctrl.
                             integral_of_error[t].value) > 3000:
                    m_dyn.fs_main.fs_stc.spray_ctrl.\
                        integral_of_error[t].value = 3000
                    _log.info('Reached lower bound of manipulated variable and'
                              ' maximum windup of 3000. Reset windup to 3000.')
            if mv_unbounded > pyo.value(
                    m_dyn.fs_main.fs_stc.spray_ctrl.mv_ub):
                # Reset spray control windup
                if pyo.value(m_dyn.fs_main.fs_stc.spray_ctrl.
                             integral_of_error[t].value) < -10000:
                    m_dyn.fs_main.fs_stc.spray_ctrl.\
                        integral_of_error[t].value = -10000
                    _log.info('Reached upper bound of manipulated variable and'
                              ' minimum windup of -1e4. Reset windup to -1e4.')
        _log.info("Solving for period number {} from {} s".format(
            i, tstart[i]))
...
def get_model(dynamic=True, time_set=None, nstep=None, init=True):
    ...
    if nstep is None:
        nstep = 2
    if m.dynamic:
        m.fs_main = FlowsheetBlock(default={
            "dynamic": True,
            "time_set": time_set,
    ...
    m.fs_main.slope_pslide = pyo.Param(
        initialize=0.02,
        doc="slope of sliding pressure")
    ...
    def main_steam_pressure_eqn(b, t):
        return b.main_steam_pressure[t] == \
            1e-6*b.fs_blr.aPlaten.outlet.pressure[t]
    ...
    if m.dynamic:
        ...
        m.fs_main.turbine_master_ctrl = PIDController(
            default={"pv": m.fs_main.fs_stc.power_output,
                     "mv": m.fs_main.fs_stc.turb.throttle_valve[1]
                     .valve_opening,
                     "type": 'PI'})
        ...
        m.fs_main.boiler_master_ctrl = PIDController(
            default={"pv": m.fs_main.main_steam_pressure,
        ...
        def boiler_master_setpoint_eqn(b, t):
            return b.boiler_master_ctrl.setpoint[t] == (
                0.02*(b.turbine_master_ctrl.setpoint[t]
        ...
        def dry_o2_in_flue_gas_dyn_eqn(b, t):
            return b.fs_blr.aBoiler.fluegas_o2_pct_dry[t] == (
                0.05*(b.fs_stc.spray_ctrl.setpoint[t]
                      - b.fs_stc.temperature_main_steam[t])
                - 0.00075*b.fs_blr.aBoiler.flowrate_coal_raw[t]**3
                + 0.067*b.fs_blr.aBoiler.flowrate_coal_raw[t]**2
                - 2.0*b.fs_blr.aBoiler.flowrate_coal_raw[t] + 22.95)
        ...
        def sliding_pressure_eqn(b, t):
            return b.main_steam_pressure[t] == 14.4 \
                + b.slope_pslide*(b.fs_stc.power_output[t]-320)
    ...
    if m.dynamic is False:
        ...
        _log.info("Power output of bfp turbine={}".format(
            pyo.value(-m.fs_main.fs_stc.bfp_turb.control_volume.work[0]
                      - m.fs_main.fs_stc.bfp_turb_os.control_volume.work[0])
            * 1e-6))
        _log.info("coal flow rate={}".format(
            pyo.value(m.fs_main.fs_blr.aBoiler.flowrate_coal_raw[0])))
        _log.info("feed water flow_mol={}".format(
        ...
        _log.info("reheat steam flow_mol={}".format(
            pyo.value(m.fs_main.fs_stc.turb.ip_stages[1].inlet.flow_mol[0])))
        _log.info("spray flow={}".format(
            m.fs_main.fs_stc.spray_valve.outlet.flow_mol[0].value))
        _log.info("bfpt steam flow={}".format(
            pyo.value(m.fs_main.fs_stc.bfp_turb.inlet.flow_mol[0])))
        _log.info("condenser condensate flow_mol={}".format(
            pyo.value(m.fs_main.fs_stc.turb.outlet_stage.outlet.flow_mol[0])))
        _log.info("SA temperature={}".format(
            pyo.value(m.fs_main.fs_blr.aBoiler
                      .secondary_air_inlet.temperature[0])))
        _log.info("FEGT={}".format(
            pyo.value(m.fs_main.fs_blr.aBoiler
                      .flue_gas_outlet.temperature[0])))
        _log.info("main steam temp={}".format(
            pyo.value(m.fs_main.fs_blr.aPlaten.control_volume
                      .properties_out[0].temperature)))
        _log.info("reheat steam temp={}".format(
            pyo.value(m.fs_main.fs_blr.aRH2.tube
                      .properties[0, 0].temperature)))
        _log.info("ECON inlet temperature={}".format(
            pyo.value(m.fs_main.fs_blr.aECON.tube
                      .properties[0, 1].temperature)))
        _log.info("ECON inlet temperature sat={}".format(
            pyo.value(m.fs_main.fs_blr.aECON.tube
                      .properties[0, 1].temperature_sat)))
        _log.info("ECON outlet temperature={}".format(
            pyo.value(m.fs_main.fs_blr.aECON.tube
                      .properties[0, 0].temperature)))
        _log.info("ECON outlet temperature sat={}".format(
            pyo.value(m.fs_main.fs_blr.aECON.tube
                      .properties[0, 0].temperature_sat)))
        _log.info("condenser temp={}".format(
            pyo.value(m.fs_main.fs_stc.turb.outlet_stage.control_volume
                      .properties_out[0].temperature)))
        _log.info("FW pressure={}".format(
            pyo.value(m.fs_main.fs_stc.bfp.outlet.pressure[0])))
        _log.info("ECON inlet pressure={}".format(
            pyo.value(m.fs_main.fs_blr.aECON.tube.properties[0, 1].pressure)))
        _log.info("drum pressure={}".format(
        ...
        _log.info("Cv cond_valve={}".format(
            m.fs_main.fs_stc.cond_valve.Cv.value))
        _log.info("Cv makeup_valve={}".format(
            m.fs_main.fs_stc.makeup_valve.Cv.value))
        _log.info("Cv spray_valve={}".format(
            m.fs_main.fs_stc.spray_valve.Cv.value))
        _log.info("Cv bfp_turb_valve={}".format(
            m.fs_main.fs_stc.bfp_turb_valve.Cv.value))
        ...
        _log.info("valve opening cond_valve={}".format(
            m.fs_main.fs_stc.cond_valve.valve_opening[0].value))
        _log.info("valve opening makeup_valve={}".format(
            m.fs_main.fs_stc.makeup_valve.valve_opening[0].value))
        _log.info("valve opening spray_valve={}".format(
            m.fs_main.fs_stc.spray_valve.valve_opening[0].value))
        _log.info("valve opening bfp_turb_valve={}".format(
        ...
        _log.info("fwh2 level={}".format(
            m.fs_main.fs_stc.fwh2.condense.level[0].value))
        _log.info("fwh3 level={}".format(
            m.fs_main.fs_stc.fwh3.condense.level[0].value))
        _log.info("fwh5 level={}".format(
            m.fs_main.fs_stc.fwh5.condense.level[0].value))
        _log.info("fwh6 level={}".format(
            m.fs_main.fs_stc.fwh6.condense.level[0].value))
        _log.info("hotwell tank level={}".format(
            m.fs_main.fs_stc.hotwell_tank.tank_level[0].value))
        _log.info("da tank level={}".format(
            m.fs_main.fs_stc.da_tank.tank_level[0].value))
        _log.info("gross heat rate={}".format(
...
def run_dynamic(m, x0, t0, pd, solver):
    ...
    for t in m.fs_main.time:
        ...
        pd['time'].append(t+t0)
        pd['coal_flow'].append(
            m.fs_main.fs_blr.aBoiler.flowrate_coal_raw[t].value)
        ...
        pd['ww_heat'].append(
            pyo.value(m.fs_main.fs_blr.aBoiler.heat_total_ww[t])/1e6)
        pd['fegt'].append(
            m.fs_main.fs_blr.aBoiler.flue_gas_outlet.temperature[t].value)
        pd['drum_level'].append(m.fs_main.fs_blr.aDrum.level[t].value)
        pd['feed_water_flow_sp'].append(
            m.fs_main.drum_slave_ctrl.setpoint[t].value)
        pd['drum_master_ctrl_op'].append(
            m.fs_main.flow_level_ctrl_output[t].value)
        pd['feed_water_flow'].append(
            m.fs_main.fs_blr.aECON.tube_inlet.flow_mol[t].value)
        pd['main_steam_flow'].append(
            m.fs_main.fs_blr.aPlaten.outlet.flow_mol[t].value)
        pd['rh_steam_flow'].append(
            m.fs_main.fs_blr.aRH2.tube_outlet.flow_mol[t].value)
        pd['bfpt_flow'].append(
            m.fs_main.fs_stc.bfp_turb_valve.outlet.flow_mol[t].value)
        pd['spray_flow'].append(
            m.fs_main.fs_stc.spray_valve.outlet.flow_mol[t].value)
        pd['main_steam_temp'].append(
            m.fs_main.fs_stc.temperature_main_steam[t].value)
        pd['rh_steam_temp'].append(
            pyo.value(m.fs_main.fs_blr.aRH2.tube.properties[t, 0].temperature))
        pd['fw_pres'].append(
            m.fs_main.fs_stc.bfp.outlet.pressure[t].value/1e6)
        pd['drum_pres'].append(
        ...
        pd['hw_tank_level'].append(
            m.fs_main.fs_stc.hotwell_tank.tank_level[t].value)
        pd['da_tank_level'].append(
            m.fs_main.fs_stc.da_tank.tank_level[t].value)
        ...
        pd['load_demand'].append(
            m.fs_main.turbine_master_ctrl.setpoint[t].value)
        pd['sliding_pressure'].append(
            pyo.value(m.fs_main.sliding_pressure[t]))
        pd['steam_pressure_sp'].append(
            m.fs_main.boiler_master_ctrl.setpoint[t].value)
        ...
        pd['temp_econ_out_sat'].append(
            pyo.value(m.fs_main.fs_blr.aECON.tube
                      .properties[t, 0].temperature_sat))
        ...
        pd['steam_cycle_efficiency'].append(
            pyo.value(m.fs_main.steam_cycle_eff[t]))
...
def print_pfd_results(m):
    ...
    with open("plant_pfd.svg", "r") as f:
        svg_tag(svg=f, tags=tags, outfile=f"plant_pfd_result.svg",
                tag_format=tag_formats)
