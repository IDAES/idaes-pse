...
def build_power_island(m):
    ...
    # TODO - cathode_exhaust_split (remove) not needed as all cathode exhaust now goes directly to the HRSG
    ...
    # TODO - cathode_exhaust_translator not needed as cathode exhaust goes to HRSG and not combustor
    ...
    # TODO - replace "cathode_inlet" into combustor_mix to O2 from ASU
    m.fs.combustor_mix = Mixer(
        default={"inlet_list": ["anode_inlet", "O2_inlet"],
                 "property_package": m.fs.syn_props})
    ...
    # TODO - not needed as cathode_inlet is replaced with O2_inlet
    ...
    #     destination=m.fs.combustor_mix.O2_inlet)
...
def set_power_island_inputs(m):
    ...
    # Combustor inputs
    #####################
    # TODO - temperature, pressure and compostion values match NETL ASU_CPU 2015 white paper numbers
    m.fs.combustor_mix.O2_inlet.flow_mol[0].fix(700)  # mol/s
    m.fs.combustor_mix.O2_inlet.temperature[0].fix(288.15)  # K
    m.fs.combustor_mix.O2_inlet.pressure[0].fix(126863.5)  # Pa, equal to 18.4 psia
    m.fs.combustor_mix.O2_inlet.mole_frac_comp[0, 'CO2'].fix(0.0003)
    m.fs.combustor_mix.O2_inlet.mole_frac_comp[0, 'H2O'].fix(0.0017)
    m.fs.combustor_mix.O2_inlet.mole_frac_comp[0, 'N2'].fix(0.002)
    m.fs.combustor_mix.O2_inlet.mole_frac_comp[0, 'O2'].fix(0.995)
    m.fs.combustor_mix.O2_inlet.mole_frac_comp[0, 'Ar'].fix(0.001)
    m.fs.combustor_mix.O2_inlet.mole_frac_comp[0, 'H2'].fix(0)
    m.fs.combustor_mix.O2_inlet.mole_frac_comp[0, 'CO'].fix(0)
    m.fs.combustor_mix.O2_inlet.mole_frac_comp[0, 'CH4'].fix(0)

    #####################
    # HRSG section inputs
    #####################
    # TODO - set to negligible value for now. This splitter unit is not needed
    m.fs.cathode_exhaust_split.split_fraction[0, 'combustor_outlet'].fix(1e-6)
...
def build_reformer(m):
    ...
    # TODO - remove as compressor costs for O2 will be captured in ASU
    ...
    # TODO - Update this constraint to account for O2 instead of Air as feed.
    # Also, how is the value of 2.868 used for computing air flow to reformer obtained
    @m.fs.Constraint()
    def reformer_oxygen_rule(fs):
        ng_flow = m.fs.reformer_recuperator.tube_inlet.flow_mol[0]
        oxygen_flow = m.fs.reformer_mix.oxygen_inlet.flow_mol[0]
        IR = m.fs.reformer_bypass.split_fraction[0, 'bypass_outlet']
        return oxygen_flow == 2.868*0.2077*(1 - IR)*ng_flow # 0.2077 is the mole fraction of O2 in air
...
def set_reformer_inputs(m):
    ...
    # TODO - this stream is replaced with pure oxygen provided from an ASU
    # TODO - O2 purity is set at 99.5% and 236 kWh/ton O2 (Ref: Iyengar et al., 2021. Journal of Energy Technology, vol 143.)
    # Probably remove the air compressor as the ASU surrogate replaces this. Replace with separator block?
    # Also update the temperature to the ASU outlet temp (55 F) from the NETL ASU_CPU 2015 white paper
    ...
    m.fs.reformer_mix.oxygen_inlet.mole_frac_comp[0, 'H2O'].fix(0.0017)
    m.fs.reformer_mix.oxygen_inlet.mole_frac_comp[0, 'N2'].fix(0.002)
    m.fs.reformer_mix.oxygen_inlet.mole_frac_comp[0, 'O2'].fix(0.995)
    m.fs.reformer_mix.oxygen_inlet.mole_frac_comp[0, 'Ar'].fix(0.001)
    # These don't need to be set
    # m.fs.reformer_mix.oxygen_inlet.mole_frac_comp[0, 'CO'].fix(0)
    # m.fs.reformer_mix.oxygen_inlet.mole_frac_comp[0, 'H2'].fix(0)
    # m.fs.reformer_mix.oxygen_inlet.mole_frac_comp[0, 'CH4'].fix(0)
    # m.fs.reformer_mix.oxygen_inlet.mole_frac_comp[0, 'C2H6'].fix(0)
    # m.fs.reformer_mix.oxygen_inlet.mole_frac_comp[0, 'C3H8'].fix(0)
    # m.fs.reformer_mix.oxygen_inlet.mole_frac_comp[0, 'C4H10'].fix(0)
...
def build_ASU(m):
    # Not needed for first pass, use pure O2 streams as these will be easier to vary.
    # TODO - update to be 99.5% purity O2
    m.fs.ASU = Separator(
    ...
    m.fs.ASU.split_fraction[0, 'O2_outlet', 'N2'].fix(0)
    m.fs.ASU.split_fraction[0, 'O2_outlet', 'Ar'].fix(0)
    
    m.fs.ASU_splitter = Separator(
        default={"outlet_list": ["to_reformer", "to_combustor"],
                 "property_package": m.fs.syn_props})
    # anode recycle and blower
    m.fs.ASU_splitter.split_fraction[0, 'to_reformer'].fix(0.75)    
...
def pfd_result(outfile, m, df):
    ...
    original_svg_file = os.path.join(this_file_dir(),
                                     "rsofc_sofc_mode_results_template.svg")
    with open(original_svg_file, "r") as f:
        svg_tag(tags, f, outfile=outfile)

def active_unfixed_vars(m):
        sourceFile = open('active_unfixed_vars.txt', 'w')
        for v in m.component_data_objects(pyo.Var, active=True, descend_into=True):
            if v.unfix:
                print("FOUND VAR:" + v.name, file=sourceFile)
        sourceFile.close()
...
def main():
    ...
    # pfd_result("rsofc_sofc_mode_results.svg", m, df)
