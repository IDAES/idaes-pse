...
# TODO - split inputs into set performance and set inputs (highlight inputs that need to be unfixed at final solve)
...
def set_inputs(m):
    ...
    # m.fs.hxh2.overall_heat_transfer_coefficient.fix(100)
    # m.fs.hxh2.shell_outlet.temperature[0].fix(400)
    # h_hxh2 = pyo.value(iapws95.htpx(None,1.1e5*pyo.units.Pa, x=1)) # enthalpy outlet
    # h_hxh2 = pyo.value(iapws95.htpx(376*pyo.units.K, 1.1e5*pyo.units.Pa)) # enthalpy outlet
    # m.fs.hxh2.tube_outlet.enth_mol[0].fix(h_hxh2)
    ...
    _set_port( # TODO - relate NG feed to air feed requirement
        m.fs.air_compressor_s1.inlet, F=5000, T=330, P=1.04e5, comp=air_comp, fix=True
    )
    _set_port(
        m.fs.ng_preheater.tube_inlet, F=380, T=330, P=1.04e5, comp=ng_comp, fix=True
    )
    _set_port(  # TODO - used as a dof instead of splta1.split_fraction
        m.fs.mxa1.air, F=6000, T=1073.15, P=1.04e5,
    ...
    _set_port(  # TODO - remove alongside mxa1 as recycle is no longer spec'd
        m.fs.mxa1.recycle, F=1000, T=1073.15, P=1.04e5,
        comp={"O2": 0.2074, "H2O": 0.0099, "CO2": 0.0003,
              "N2": 0.7732, "Ar": 0.0092,}, fix=True
    )
    ...
    m.fs.bhx2.tube_inlet.enth_mol.fix(iapws95.htpx(T=380*pyo.units.K, P=1.1e5*pyo.units.Pa))
    ...
    # TODO - setting cmb upper bound can lead to infeasible solutions
    # m.fs.cmb.outlet.temperature.setub(4000)
...
def do_initialize(m, solver):
    ...
    copy_port_values(source=m.fs.air_preheater.tube_outlet,
                 destination=m.fs.pre_oxycombustor_translator.inlet)
    ...
    copy_port_values(source=m.fs.bhx2.shell_outlet,
                 destination=m.fs.bhx1.shell_inlet)
    copy_port_values(source=m.fs.bhx2.tube_outlet,
                 destination=m.fs.main_steam_split.inlet)
    ...
    # m.fs.bhx2.tube_inlet.display()
    # m.fs.bhx2.tube_outlet.display()
    # m.fs.bhx2.shell_inlet.display()
    # m.fs.bhx2.shell_outlet.display()
    # import sys
    # sys.exit('stop')
...
def check_heat_exchanger_DT(m):
    ...
    print("-----------------------------------------------------------")    
    print('bhx2 DT')
    print('DT_out = ',pyo.value(m.fs.bhx2.delta_temperature_out[0]))
    print('DT_in = ',pyo.value(m.fs.bhx2.delta_temperature_in[0]))
    print('')
    print('Tube side_in T = ', pyo.value(m.fs.bhx2.cold_side.properties_in[0].temperature))
    print('Tube side_out T = ', pyo.value(m.fs.bhx2.cold_side.properties_out[0].temperature))
    print('Shell side_in T = ', pyo.value(m.fs.bhx2.hot_side.properties_in[0].temperature))
    print('Shell side_out T = ', pyo.value(m.fs.bhx2.hot_side.properties_out[0].temperature))
    print('')
    print('Area = ', pyo.value(m.fs.bhx2.area))
    print('HTC = ', pyo.value(m.fs.bhx2.overall_heat_transfer_coefficient[0]))
    print('')
    print("-----------------------------------------------------------")    
    print('air_preheater DT')
    print('DT_out = ',pyo.value(m.fs.air_preheater.delta_temperature_out[0]))
    print('DT_in = ',pyo.value(m.fs.air_preheater.delta_temperature_in[0]))
    print('')
    print('Tube side_in T = ', pyo.value(m.fs.air_preheater.cold_side.properties_in[0].temperature))
    print('Tube side_out T = ', pyo.value(m.fs.air_preheater.cold_side.properties_out[0].temperature))
    print('Shell side_in T = ', pyo.value(m.fs.air_preheater.hot_side.properties_in[0].temperature))
    print('Shell side_out T = ', pyo.value(m.fs.air_preheater.hot_side.properties_out[0].temperature))
    print('')
    print('Area = ', pyo.value(m.fs.air_preheater.area))
    print('HTC = ', pyo.value(m.fs.air_preheater.overall_heat_transfer_coefficient[0]))
    print('')
    print("-----------------------------------------------------------")    
