...
discretizing length domain (default=3)"""))
    CONFIG.declare("drum_inner_diameter", ConfigValue(
        default=1.0,
        description='inside diameter of drum',
        doc='define inside diameter of drum'))

    CONFIG.declare("drum_thickness", ConfigValue(
        default=0.1,
    ...
    def _set_geometry(self):
        ...
        self.drum_thickness = Param(
                initialize=self.config.drum_thickness,
    ...
    def _make_performance(self):
        ...
        def pressure_change_contraction_eqn(b, t):
            return b.deltaP_contraction[t] == \
                   -0.75 * b.control_volume.properties_out[t].\
                   dens_mass_phase["Liq"] * \
        ...
        def pressure_change_gravity_eqn(b, t):
            return b.deltaP_gravity[t] == \
                   ...
                   dens_mass_phase["Liq"] * const.acceleration_gravity\
                   * b.level[t]
        ...
        def therm_sigma_r(b, t, r):
            if r == b.radial_domain.first() or r == b.radial_domain.last():
                ...
                return 0.5 * b.Young_modulus * b.coefficient_therm_expansion\
                    / (1 - b.Poisson_ratio) * ((1 - b.drum_ri**2 / r**2) * (
        ...
        k_t_B = 1000
        k_t = sqrt((2 - (k_t_B + 2700) / (k_t_B + 1700) * k_t_A
                    + k_t_B / (k_t_B + 1700) * (exp(-7 * k_t_A) - 1))**2
                   + 0.81 * k_t_A**2)
    ...
    def initialize(blk, state_args_feedwater=None, state_args_water_steam=None,
        ...
        blk.mixer.SaturatedWater.flow_mol[:].value \
            = blk.flash.liq_outlet.flow_mol[0].value
        blk.mixer.SaturatedWater.pressure[:].value \
            = blk.flash.liq_outlet.pressure[0].value
        blk.mixer.SaturatedWater.enth_mol[:].value \
        ...
        blk.control_volume.properties_in[:].flow_mol.value \
            = blk.mixer.outlet.flow_mol[0].value
        blk.control_volume.properties_in[:].enth_mol.value \
            = blk.mixer.outlet.enth_mol[0].value
        blk.control_volume.properties_in[:].pressure.value \
            = blk.mixer.outlet.pressure[0].value
        ...
        init_log.info_high(
                "Initialization Step 4 Complete {}.".format(
