...
##############################################################################
"""
Steam turbine stage model. This is a standard isentropic turbine. Under
off-design conditions the base efficiency and pressure ratio do not change much
for the stages between the inlet and outlet. This model is based on:
    ...
    def initialize(
        ...
        for t in self.flowsheet().config.time:
            ...
            if value(Pout / Pin) > 0.99 or value(Pout / Pin) < 0.1:
                if (value(self.ratioP[t]) < 0.99 and
                        value(self.ratioP[t]) > 0.1):
