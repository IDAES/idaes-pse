    ...
    def _make_performance(self):
        ...
        if self.config.has_radiation is True:
            ...
            self.hconv_shell_rad = Var(self.flowsheet().time,
                                   ...
                                   "coefficient due to radiation - W / (m2 K)")
        ...
        self.hconv_shell_conv = Var(self.flowsheet().time,
                               initialize=100.0,
                               doc="Shell side convective heat transfer "
                               "coefficient due to convection - W / (m2 K)")
...
#           Surrogate model fitted using rigorous calculations 500 samples
...
#                X6: 0.01-0.07     (mol frac O2)
                ...
                return b.gas_emissivity[t] == (- 0.116916606892E-003 * X1 
...
#                X5: 0.075-0.15    (mol frac H2O)
#                X6: 0.01-0.07     (mol frac O2)
                return b.gas_emissivity_div2[t] == (- 0.116916606892E-003 * X1 
                - 0.29111124038936179309056E-001 * X2 
                + 0.50509651230704191577346E-006 * X3 
                + 1.1844222822155641150488 * X4 
                - 0.64720757767102773949652E-001 * X5 
...
#                X5: 0.075-0.15    (mol frac H2O)
#                X6: 0.01-0.07     (mol frac O2)
                return b.gas_emissivity_mul2[t] == (- 0.116916606892E-003 * X1 
                - 0.29111124038936179309056E-001 * X2 
                + 0.50509651230704191577346E-006 * X3 
                + 1.1844222822155641150488 * X4 
                - 0.64720757767102773949652E-001 * X5 
            ...
            def hconv_shell_rad_eqn(b, t):
                return b.hconv_shell_rad[t] == b.k_boltzmann*b.frad_gas_shell[t]* \
        ...
        def heat_transfer_correlation(b, t):
            return b.heat_duty[t] / 1e6 == (b.overall_heat_transfer_coefficient[t] *
        ...
        if self.config.has_pressure_change is True:
            ...
            self.deltaP_tube_uturn = Var(self.flowsheet().time, initialize= -10.0, 
        ...
        if self.config.has_pressure_change is True:
            ...
            def deltaP_tube_friction_eqn(b, t):
                ...
                b.v_tube[t]**2 * b.friction_factor_tube[t] * b.tube_length * \
                b.tube_nrow
            ...
            b.accel_gravity*(b.side_1.properties_in[t].dens_mass_phase[self.side_1_fluid_phase] \
            ...
            if self.config.tube_arrangement == TubeArrangement.inLine:
                ...
                def friction_factor_shell_eqn(b, t):
                    return b.friction_factor_shell[t] * b.N_Re_shell[t]**0.15 == \
                        (0.044 + 0.08 * b.pitch_x_to_do /(b.pitch_y_to_do - 1.0)**\
                         (0.43 + 1.13 / b.pitch_x_to_do))*b.fcorrection_dp_shell
        ...
        def overall_heat_transfer_coefficient_eqn(b, t):
            return b.overall_heat_transfer_coefficient[t] * (b.rcond_wall + \
                b.tube_r_fouling + b.shell_r_fouling + 
                1.0 / b.hconv_shell_total[t] 
                + b.do_tube / b.hconv_tube[t] / b.tube_di) == b.fcorrection_htc
    ...
    def initialize(blk, state_args_1={}, state_args_2={},
                   outlvl=0, solver='ipopt', optarg={'tol': 1e-6, 'max_iter':100}):
        '''
        ...
        for t in blk.flowsheet().time:
            ...
            if not blk.side_2.properties_out[t].temperature.fixed:
                blk.side_2.properties_out[t].temperature.fix(
                        value(blk.side_2.properties_in[t].temperature)-5.0) # assuming Delta T min approach
