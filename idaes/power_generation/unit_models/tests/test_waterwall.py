...
def test_basic_build(model):
    ...
    assert model.fs.Waterwalls[1].config.property_package is \
        model.fs.prop_water
...
def test_waterwall(model):
    ...
    Fhin = model.fs.Waterwalls[1].control_volume.properties_in[0].\
        flow_mol*model.fs.Waterwalls[1].control_volume.properties_in[0].enth_mol
    Fhout = model.fs.Waterwalls[10].control_volume.properties_out[0].flow_mol \
