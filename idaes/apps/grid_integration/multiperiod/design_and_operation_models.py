...
class DesignModelData(SkeletonUnitModelData):
    ...
    CONFIG.declare("model_func", ConfigValue(
        doc="Function that builds the design model",
    ))
    CONFIG.declare("model_args", ConfigValue(
        default={},
        doc="Dictionary containing arguements needed for model_func",
    ))
    CONFIG.declare("declare_cost_expr", ConfigValue(
        default=True,
        ...
        if self.config.declare_cost_expr:
            for ce in cost_expr_list.keys():
                if  not hasattr(self, ce):
                    ...
                    print(f"declare_cost_expr is set to True, but {ce} is not defined in {self.config.model_func.__name__}")
...
class OperationModelData(SkeletonUnitModelData):
    ...
    CONFIG.declare("declare_op_vars", ConfigValue(
        default=True,
        domain=In([True, False]),
