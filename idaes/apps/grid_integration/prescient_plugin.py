...
def initialize_bidding_object(options, simulator):
    ...
    bidder = DAM_thermal_bidding(rts_gmlc_data_dir = options.rts_gmlc_data_dir, \
                                 price_forecast_file = options.price_forecast_file,\
...
def pass_DA_bid_to_prescient(options, ruc_instance, bids):
...
def bid_into_DAM(options, simulator, ruc_instance, ruc_date, ruc_hour):
    ...
    # pass to prescient
    pass_DA_bid_to_prescient(options, ruc_instance, bids)
