...
def create_mp_rankine_block(pmin, pmax):
    ...
    b1.previous_power_output = pyo.Var(
    ...
    b1.ramp1 = pyo.Constraint(
        expr=b1.power_output - b1.previous_power_output <= turbine_ramp_rate
    )
    b1.ramp2 = pyo.Constraint(
        expr=b1.previous_power_output - b1.power_output <= turbine_ramp_rate
    )
    ...
    b2.energy_change = pyo.Constraint(
        expr=b2.soc
        == b2.previous_soc
        - b2.discharge / b2.efficiency
        + b1.P_to_battery * b2.efficiency
    )
    b2.energy_down_ramp = pyo.Constraint(
        expr=b2.previous_soc - b2.soc <= battery_ramp_rate
    )
    b2.energy_up_ramp = pyo.Constraint(
        expr=b2.soc - b2.previous_soc <= battery_ramp_rate
    )
...
def get_rankine_link_variable_pairs(t1, t2):
    ...
    """
    return [
        (t1.rankine.power_output, t2.rankine.previous_power_output),
...
def create_multiperiod_rankine_model(n_time_points=4, pmin=20, pmax=100):
    ...
    mp_rankine = MultiPeriodModel(
        n_time_points,
        lambda: create_mp_rankine_block(pmin, pmax),
