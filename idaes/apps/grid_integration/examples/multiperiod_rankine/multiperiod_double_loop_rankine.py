...
class MultiPeriodRankine:
    def __init__(
        self, horizon=4, pmin=20, pmax=100, default_bid_curve=None, generator_name="gen"
    ):
        """
        Arguments:
    ...
    def populate_model(self, b):
        ...
        mp_rankine = create_multiperiod_rankine_model(
            n_time_points=self.horizon, pmin=self.p_lower, pmax=self.p_upper
        )
    ...
    def get_implemented_profile(b, last_implemented_time_step):
        ...
        implemented_power_output = deque(
            [
                pyo.value(active_blks[t].rankine.power_output)
                for t in range(last_implemented_time_step + 1)
            ]
        )
        realized_soc = deque(
            [
                pyo.value(active_blks[t].battery.soc)
                for t in range(last_implemented_time_step + 1)
            ]
        )
