...
def test_startup_shutdown_constraints_with_multiple_startup_types():
    ...
    def op_blk(b, _):
        ...
        b.startup_type_vars = pyo.Var(list(startup_transition_time.keys()), within=pyo.Binary)
    ...
    assert pyo.value(ss.startup_duration["hot"]) == 4  # max(minimum_down_time=4, original=2)
...
def test_startup_shutdown_constraints_single_startup_type():
    ...
    assert pyo.value(ss.startup_duration["hot"]) == 3  # max(minimum_down_time=3, original=2)
