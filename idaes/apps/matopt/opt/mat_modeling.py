...
class PiecewiseLinear(DescriptorRule):
    ...
    con_type (string): indicates the bound type of the piecewise function. 
        Options:
            ...
            “EQ” - relevant descriptor is equal to the piecewise function. 
                   (Default)
    ...
    def _pyomo_cons(self, var):
        ...
        """
        Comb = IndexedElem.fromComb(var, self)
        return [Piecewise(*Comb.index_sets,
                          var._pyomo_var,
...
class Implies(DescriptorRule):
    ...
    def __init__(self, concs, **kwargs):
        ...
        **kwargs: Optional, index information passed to IndexedElem if 
        ...
        Comb = IndexedElem.fromComb(*(desc for desc, conc in self.concs),
                                    *(conc for desc, conc in self.concs))
        kwargs = {**Comb.index_dict, **kwargs}
        DescriptorRule.__init__(self, **kwargs)
    ...
    def _pyomo_cons(self, var):
        ...
        for (desc, conc) in self.concs:
            ...
            def rule_lb(m, *args):
                ...
                c = conc.expr._pyomo_expr(index=conc.expr.mask(args,
                                                               ConIndexes))
                body_lb = getLB(d - c)
                MLB = (body_lb if body_lb is not None
...
class ImpliesSiteCombination(DescriptorRule):
    ...
    def __init__(self, canv, concis, concjs, symmetric_bonds=False, **kwargs):
        ...
        **kwargs: Optional, index information passed to IndexedElem if 
        ...
        self.symmetric_bonds = symmetric_bonds
        Combi = IndexedElem.fromComb(*(desc for desc, conc in self.concis),
                                     *(conc for desc, conc in self.concis))
    ...
    def _pyomo_cons(self, var):
        ...
        for expri, conci in self.concis:
            def rule_i_lb(m, i, j):
                ...
                body = e - c
                body_LB = getLB(body)
                MLBi = (body_LB if body_LB is not None
            ...
            def rule_i_ub(m, i, j):
                ...
                body = e - c
                body_UB = getUB(body)
                MUBi = (body_UB if body_UB is not None
                        else ImpliesSiteCombination.DEFAULT_BIG_M)
        ...
        for exprj, concj in self.concjs:
            def rule_j_lb(m, i, j):
                ...
                body = e - c
                body_LB = getLB(body)
                MLBj = (body_LB if body_LB is not None
                        else -ImpliesSiteCombination.DEFAULT_BIG_M)
            ...
            def rule_j_ub(m, i, j):
                ...
                body = e - c
                body_UB = getUB(body)
                MUBj = (body_UB if body_UB is not None
                        else ImpliesSiteCombination.DEFAULT_BIG_M)
...
class ImpliesNeighbors(DescriptorRule):
    ...
    def _pyomo_cons(self, var):
        ...
        neighborhoods = (self.neighborhoods if self.neighborhoods is not None
                         else var.canv.NeighborhoodIndexes)
        bonds = [(i, j) for i in var.sites for j in neighborhoods[i]
                 if j is not None]
        result = []
        # NOTE: After much confusion, I found a bug in the line of code
        #       below. Be careful not to use variable names "expr"
        ...
        for expr_, conc in self.concs:
            ...
            ConIndexes = IndexedElem.fromComb(IndexedElem(bonds=bonds),
            ...
            def rule_lb(m, *args):
                ...
                e = expr_._pyomo_expr(index=expr_.mask((j, None, *args),
                                                       ConIndexes))
                c = conc.expr._pyomo_expr(index=conc.expr.mask((j, None, *args),
                                                               ConIndexes))
                body = e - c
                body_LB = getLB(body)
                MLB = (body_LB if body_LB is not None
...
class MaterialDescriptor(IndexedElem):
    ...
    # === STANDARD CONSTRUCTOR
    def __init__(self, name, canv=None, atoms=None, confDs=None,
                 bounds=(None, None), integer=False, binary=False,
                 rules=[], **kwargs):
...
class MatOptModel(object):
    ...
    def addGlobalDescriptor(self, name,
                            bounds=(None, None), integer=False, binary=False,
                            rules=None):
        ...
        Desc = MaterialDescriptor(name=name, bounds=bounds,
                                  integer=integer, binary=binary, rules=rules)
        setattr(self, name, Desc)
        self._descriptors.append(Desc)

    def addSitesDescriptor(self, name,
                           sites=None,
                           bounds=(None, None), integer=False, binary=False,
                           rules=None):
        ...
        Desc = MaterialDescriptor(name=name, canv=self.canv,
                                  sites=sites,
                                  bounds=bounds, integer=integer, binary=binary,
                                  rules=rules)
        setattr(self, name, Desc)
        self._descriptors.append(Desc)

    def addBondsDescriptor(self, name,
                           bonds=None,
                           bounds=(None, None), integer=False, binary=False,
                           rules=None,
    ...
    def addNeighborsDescriptor(self, name,
                               sites=None,
                               bounds=(None, None), integer=False, binary=False,
                               rules=None):
    ...
    def addGlobalTypesDescriptor(self, name,
                                 site_types=None, bond_types=None,
                                 bounds=(None, None),
        ...
        """
        assert (not hasattr(self, name))
        site_types = (site_types if site_types is not None else
                      (self.atoms if bond_types is None else None))
        bond_types = bond_types
        Desc = MaterialDescriptor(name=name, atoms=self.atoms,
                                  site_types=site_types, bond_types=bond_types,
                                  bounds=bounds, integer=integer, binary=binary,
                                  rules=rules)
        setattr(self, name, Desc)
    ...
    def addSitesTypesDescriptor(self, name,
                                sites=None, site_types=None,
                                bounds=(None, None), integer=False, binary=False,
                                rules=None):
        ...
        Desc = MaterialDescriptor(name=name, atoms=self.atoms, canv=self.canv,
                                  sites=sites, site_types=site_types,
                                  bounds=bounds, integer=integer, binary=binary,
                                  rules=rules)
        setattr(self, name, Desc)
        self._descriptors.append(Desc)

    def addBondsTypesDescriptor(self, name,
                                bonds=None, bond_types=None,
                                bounds=(None, None), integer=False, binary=False,
                                rules=None,
                                symmetric_bonds=False):
        ...
        bonds = (bonds if bonds is not None else
                 [(i, j)
                  ...
                  for j in self.canv.NeighborhoodIndexes[i]
                  if (j is not None and (not symmetric_bonds or j > i))])
        bond_types = (bond_types if bond_types is not None else
                      [(k, l) for k in self.atoms for l in self.atoms])
        Desc = MaterialDescriptor(name=name, atoms=self.atoms, canv=self.canv,
    ...
    def addNeighborsTypesDescriptor(self, name,
                                    sites=None, bond_types=None,
                                    bounds=(None, None),
                                    integer=False, binary=False,
                                    rules=None):
        ...
        bond_types = (bond_types if bond_types is not None else
                      [(k, l) for k in self.atoms for l in self.atoms])
        Desc = MaterialDescriptor(name=name, atoms=self.atoms, canv=self.canv,
                                  sites=sites, bond_types=bond_types,
                                  bounds=bounds, integer=integer, binary=binary,
                                  rules=rules)
        setattr(self, name, Desc)
        self._descriptors.append(Desc)

    def addSitesConfsDescriptor(self, name,
                                sites=None, confs=None,
                                bounds=(0, 1), integer=True, binary=True,
                                rules=None):
        ...
        confs = (confs if confs is not None else
                 (list(range(len(self.confDs))) if self.confDs is not None
                  else None))
        Desc = MaterialDescriptor(name=name, canv=self.canv,
                                  confDs=self.confDs,
                                  sites=sites, confs=confs,
                                  bounds=bounds, integer=integer, binary=binary,
                                  rules=rules)
        setattr(self, name, Desc)
        self._descriptors.append(Desc)
    ...
    def optimize(self, func, sense, nSolns=1,
                 tee=True, disp=1, keepfiles=False,
                 tilim=3600, trelim=None,
                 solver='cplex'):
    ...
    def populate(self, func, sense, nSolns,
                 tee=True, disp=1, keepfiles=False,
                 tilim=3600, trelim=None,
                 solver='cplex'):
        ...
        for iSoln in range(nSolns):
            ...
            D = self.__solve_pyomo_model(tee, disp - 1, keepfiles, tilim, trelim, solver)
            if D is not None:
                ...
                if len(self._pyomo_m.Yik) > 0:
                    ...
                    raise NotImplementedError('Decide what to do '
                                              'in this case...')
            else:
    ...
    def __solve_pyomo_model(self, tee, disp, keepfiles, tilim, trelim, solver):
        ...
        has_solution = (soln_status == SolutionStatus.optimal or
                        soln_status == SolutionStatus.feasible or
                        soln_status == SolutionStatus.bestSoFar or
                        soln_status == SolutionStatus.globallyOptimal or
                        soln_status == SolutionStatus.locallyOptimal)
