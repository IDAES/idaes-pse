...
from .flextest import FlexTestConfig, solve_flextest, FlexTestMethod, FlexTestTermination, FlexTest
import math
import logging
...
def _get_param_bounds(orig_param_bounds, nominal_values, flex_index):
    ...
    for p, (p_lb, p_ub) in orig_param_bounds.items():
        ...
        tmp_param_bounds[p] = (p_nom - (p_nom - p_lb) * flex_index, p_nom + (p_ub - p_nom) * flex_index)
    return tmp_param_bounds


def _add_table_row(log_level, outer_iter, flex_index_lower, flex_index_upper, fi_lb_max_viol, fi_ub_max_viol):
    ...
    logger.log(log_level, f"{outer_iter:<12}{lb_str:<12}{ub_str:<12}{lb_mv_str:<15}{ub_mv_str:<15}")


def solve_flex_index(m: _BlockData,
                     ...
                     log_level: int = logging.INFO) -> float:
    ...
    if not in_place:
        ...
        param_nominal_values = pe.ComponentMap((p, original_param_nominal_values[orig_p]) for orig_p, p in zip(original_uncertain_params, uncertain_params))
        param_bounds = pe.ComponentMap((p, original_param_bounds[orig_p]) for orig_p, p in zip(original_uncertain_params, uncertain_params))
    ...
    if config.method == FlexTestMethod.linear_decision_rule and reconstruct_decision_rule is None:
        reconstruct_decision_rule = True
    elif config.method == FlexTestMethod.relu_decision_rule and reconstruct_decision_rule is None:
        reconstruct_decision_rule = False
    elif reconstruct_decision_rule is None:
        reconstruct_decision_rule = False

    logger.log(log_level, f"{'Iter':<12}{'FI LB':<12}{'FI UB':<12}{'FI LB Max Viol':<15}{'FI UB Max Viol':<15}")
    ...
    nominal_res = solve_flextest(m=m, uncertain_params=uncertain_params, param_nominal_values=param_nominal_values,
                                 param_bounds=nominal_bounds, controls=controls, valid_var_bounds=valid_var_bounds,
                                 in_place=False, config=nominal_config)
    ...
    _add_table_row(log_level, outer_iter, flex_index_lower, None, fi_lb_max_viol, fi_ub_max_viol)
    ...
    if not cap_index_at_1:
        ...
        for _iter in range(10):
            tmp_param_bounds = _get_param_bounds(param_bounds, param_nominal_values, flex_index_upper)
            upper_res = solve_flextest(m=m, uncertain_params=uncertain_params,
                                       param_nominal_values=param_nominal_values,
                                       param_bounds=tmp_param_bounds, controls=controls,
                                       valid_var_bounds=valid_var_bounds,
                                       in_place=False, config=flextest_config)
            outer_iter += 1
            if upper_res.termination == FlexTestTermination.found_infeasible_point:
                fi_ub_max_viol = upper_res.max_constraint_violation
                _add_table_row(log_level, outer_iter,
                               flex_index_lower, flex_index_upper,
                               fi_lb_max_viol, fi_ub_max_viol)
    ...
    max_param_bounds = _get_param_bounds(param_bounds, param_nominal_values, flex_index_upper)
    if cap_index_at_1:
        if reconstruct_decision_rule:
            res = solve_flextest(m=m, uncertain_params=uncertain_params, param_nominal_values=param_nominal_values,
                                 param_bounds=max_param_bounds, controls=controls,
                                 valid_var_bounds=valid_var_bounds, in_place=False, config=flextest_config)
        else:
            ft = FlexTest(m=m, uncertain_params=uncertain_params, param_nominal_values=param_nominal_values,
                          max_param_bounds=max_param_bounds, controls=controls, valid_var_bounds=valid_var_bounds,
        ...
        if res.termination == FlexTestTermination.proven_feasible:
            ...
            raise RuntimeError('Unexpected termination from flexibility test')
        _add_table_row(log_level, outer_iter, flex_index_lower, flex_index_upper, fi_lb_max_viol, fi_ub_max_viol)
    ...
    while (flex_index_upper - flex_index_lower) > 1e-3:
        ...
        tmp_param_bounds = _get_param_bounds(param_bounds, param_nominal_values, midpoint)
        if reconstruct_decision_rule:
            res = solve_flextest(m=m, uncertain_params=uncertain_params, param_nominal_values=param_nominal_values,
                                 param_bounds=tmp_param_bounds, controls=controls,
                                 valid_var_bounds=valid_var_bounds, in_place=False, config=flextest_config)
