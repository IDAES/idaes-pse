...
def create_poly_model():
    ...
    m.c1 = pe.Constraint(expr=0.01*(m.z-offset)**4 - 0.05*(m.z-offset)**3 - (m.z-offset)**2 - (m.z-offset) - 10 + m.theta <= 0)
    m.c2 = pe.Constraint(expr=(-0.02*m.theta - 14)*m.z + (1.66*m.theta - 100) <= 0)
    m.c3 = pe.Constraint(expr=30*m.z - 50 - 4*m.theta + pe.exp(-0.2*m.theta + 1) <= 0)
...
class TestFlexTest(unittest.TestCase):
    def test_poly(self):
        ...
        build_active_constraint_flextest(m,
