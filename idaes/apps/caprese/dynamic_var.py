...
class DynamicVar(IndexedVar):
    ...
    Consistent with the NMPC/MHE/DAE literature, a "variable" here is
    ...
    >>> for var in model.component_objects(DynamicVar):
    >>>     var[:].set_value(var[0])
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(DynamicVar, self).__init__(*args, **kwargs)
...
class DiffVar(DynamicVar):
    _attr = 'differential'

class DerivVar(DynamicVar):
    _attr = 'derivative'

class AlgVar(DynamicVar):
    _attr = 'algebraic'

class InputVar(DynamicVar):
    _attr = 'input'

class FixedVar(DynamicVar):
    _attr = 'fixed'

class MeasuredVar(DynamicVar):
    _attr = 'measurement'
    
class ActualMeasurementVar(DynamicVar):
    _attr = 'actualmeasurement'
    
class MeasurementErrorVar(DynamicVar):
    _attr = 'measurementerror'

class ModelDisturbanceVar(DynamicVar):
    _attr = 'modeldisturbance'


class _DynamicVector(IndexedVar):
    """
    This is a class used as a ctype for references to `DynamicVar` objects.
    ...
    index, this class adds methods to iterate over the underlying `DynamicVar`
    ...
    >>> input_vars = Reference(INPUT_BLOCK[:].var[:], ctype=_DynamicVector)
    >>> input_vars[:, 0].fix()
    ...
    def _generate_referenced_vars(self):
        ...
        _slice._len -= 1
        for var in _slice:
            assert isinstance(var, DynamicVar)
            yield var
