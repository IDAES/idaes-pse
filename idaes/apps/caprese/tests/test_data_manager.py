...
from idaes.apps.caprese.data_manager import (
                    ...
                    add_setpoint_column_into_dataframe,
...
class TestDataManager(object):
    ...
    def test_add_variable_values_to_dataframe(self):
        ...
        #Case4: initial_time is given
        df4 = empty_dataframe_from_variables(variables)
        df4 = add_variable_values_to_dataframe(df4,
                                               variables,
                                               iteration = 400,
                                               initial_time = 30)
        assert len(df1.index) == 5
        assert all(i1 == i2 for i1, i2 in zip(df4.index, [30.0, 31.0, 32.0, 33.0, 34.0]))
        assert len(df1.columns) == 3
        assert all(ele == 400 for ele in df4["iteration"])
        assert all(ele == 0.1+ind*1. for ind, ele in enumerate(df4[str_cuids[0]]))
        assert all(ele == 0.2+ind*1. for ind, ele in enumerate(df4[str_cuids[1]]))
                
        #Case5: time_map is given
    ...
    @pytest.mark.unit
    def test_add_setpoint_column_into_dataframe(self):
        m = self.make_small_ContreteModel()
        variables = [pyo.Reference(m.var1[:, "A"]), pyo.Reference(m.var1[:, "B"])]
        str_cuids = [str(ComponentUID(var.referent)) for var in variables]
        
        df1 = empty_dataframe_from_variables(variables)
        df1 = add_setpoint_column_into_dataframe(df1, variables)
        assert df1.empty
        assert len(df1.index) == 0
        pred_column_strings1 = ["iteration"]+ \
                                [str(ComponentUID(var.referent)) for var in variables] + \
                                [str(ComponentUID(var.referent))+"_setpoint" for var in variables]
        assert all(i1 == i2 for i1, i2 in zip(pred_column_strings1, df1.columns))
    ...
    def test_add_variable_setpoints_to_dataframe(self):
        ...
        df1 = add_variable_setpoints_to_dataframe(df1,
                                                  variables,
                                                  time_subset = [0.0, 1.0, 3.0],
                                                  map_for_user_given_vars = m.var_mapping,
                                                  )
        
        assert all(val == 10.0 for val in df1["var1[*,A]_setpoint"])
        assert all(val == 20.0 for val in df1["var1[*,B]_setpoint"])
        ...
        assert all(val == 100.0 for val in df1["var1[*,A]_setpoint"][3:])
        assert all(val == 200.0 for val in df1["var1[*,B]_setpoint"][3:])
    ...
    def test_ControllerDataManager(self):
        ...
        nmpc_data = ControllerDataManager(plant,
                                          controller,
                                          states_of_interest,
                                          inputs_of_interest,)  
        
        assert isinstance(nmpc_data, NMPC_PlotLibrary)
        assert isinstance(nmpc_data, PlantDataManager)

        # Make sure all methods are there.
        assert hasattr(nmpc_data, "get_plant_dataframe")
        assert hasattr(nmpc_data, "save_initial_plant_data")
        assert hasattr(nmpc_data, "save_plant_data")
    ...
    def test_ControllerDataManager__init__(self):
        ...
        assert hasattr(nmpc_data, "plant_user_interested_states")
        assert hasattr(nmpc_data, "plant_user_interested_inputs")
        assert hasattr(nmpc_data, "plantblock")
        assert hasattr(nmpc_data, "plant_vars_of_interest")
        assert hasattr(nmpc_data, "plant_df")
    ...
    def test_save_controller_data(self):
        ...
        pred_columns_result = OrderedDict([("iteration", 10),
                                           ("mod.flow_in[*]", 5), 
                                           ("mod.flow_in[**]", 5),
                                           ("mod.conc[*,A]_setpoint", 0.2), 
                                           ("mod.conc[*,B]_setpoint", 0.3),])
        ...
        pred_columns_result = OrderedDict([("iteration", 20),
                                           ("mod.flow_in[*]", 15), 
                                           ("mod.flow_in[**]", 15),
                                           ("mod.conc[*,A]_setpoint", 5.2), 
                                           ("mod.conc[*,B]_setpoint", 5.3),])
    ...
    def test_EstimatorDataManager(self):
        ...
        states_of_interest = [pyo.Reference(p_model.conc[:, "A"])]
        mhe_data = EstimatorDataManager(plant,
                                        estimator,
                                        states_of_interest,)
        
        assert isinstance(mhe_data, MHE_PlotLibrary)
        assert isinstance(mhe_data, PlantDataManager)

        # Make sure all methods are there.
        assert hasattr(mhe_data, "get_plant_dataframe")
        assert hasattr(mhe_data, "save_initial_plant_data")
        assert hasattr(mhe_data, "save_plant_data")
