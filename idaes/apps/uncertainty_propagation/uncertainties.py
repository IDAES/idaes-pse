...
from idaes.apps.uncertainty_propagation.sens import (
    SensitivityInterface, get_dsdp, get_dfds_dcds) 
...
def quantify_propagate_uncertainty(model_function, model_uncertain, data, 
    ...
    if variable_clean:
        ...
        theta_out = theta
    propagate_results  =  propagate_uncertainty(model_uncertain, theta, 
                                                cov, theta_names, tee)
   
    Output = namedtuple('Output',['obj', 'theta', 'theta_names', 'cov',
                                  'gradient_f', 'gradient_c', 'dsdp', 
                                  'propagation_c', 'propagation_f','col','row'])
...
def propagate_uncertainty(model_uncertain, theta, cov, theta_names,
    ...
    Args:
        model_uncertain(function or Pyomo ConcreteModel) 
            : Function is a python Function that generates an instance of 
              the Pyomo model
        theta(dict)                    
    ...
    gradient_f, gradient_c, col,row, line_dic = get_dfds_dcds(model, 
                                                              theta_names, tee)
    ...
    #
    assert propagation_f.shape == (1,1), """propagation_f should be 
    ...
    Output = namedtuple('Output',['gradient_f', 'gradient_c', 'dsdp', 
                                  'propagation_c','propagation_f','col','row'])
    results= Output(gradient_f, gradient_c, sparse.csr_matrix(dsdp), 
                    propagation_c, propagation_f,col,row)
    return results
...
def clean_variable_name(theta_names):
    ...
    Note: The  current theta_est(calc_cov=True) of parmest in 
          Pyomo doesn't allow ' and spaces in the variable names.
    ...
    Returns:
            ...
            - var_dic(dict)                    
                : Dictionary with keys converted theta_names 
                  and values origianl theta_names 
    """
