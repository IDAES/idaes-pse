...
class TestIronOC(object):
    ...
    def iron_oc(self):
        ...
        for t in m.fs.time:
            ...
            m.fs.unit.gas_inlet.temperature[t].fix(298.15)  # K
        ...
        for x in m.fs.unit.length_domain:
            if x != m.fs.unit.length_domain.first():  # skip inlet as it's already fixed
                m.fs.unit.gas_phase.properties[t0, x].mole_frac_comp["CO2"].fix(0.005)
                m.fs.unit.gas_phase.properties[t0, x].mole_frac_comp["H2O"].fix(0.02)
    ...
    def test_scaling(self, iron_oc):
        ...
        for c in FB1D.volume_bed_constraint.values():
            assert pytest.approx(
                0.127324, rel=1e-5
            ) == iscale.get_constraint_transform_applied_scaling_factor(c)
        for t, c in FB1D.volume_solid_constraint.items():
            assert pytest.approx(
                0.127324, rel=1e-5
            ) == iscale.get_constraint_transform_applied_scaling_factor(c)
        for (t, j), c in FB1D.solids_material_holdup_constraints.items():
            assert pytest.approx(
                0.00127324, rel=1e-5
            ) == iscale.get_constraint_transform_applied_scaling_factor(c)
        for (t, j), c in FB1D.solids_material_accumulation_constraints.items():
            assert pytest.approx(
                0.127324, rel=1e-5
            ) == iscale.get_constraint_transform_applied_scaling_factor(c)
        for t, c in FB1D.mass_solids_constraint.items():
            assert pytest.approx(
                0.00127324, rel=1e-5
            ) == iscale.get_constraint_transform_applied_scaling_factor(c)
        for t, c in FB1D.sum_component_constraint.items():
            assert pytest.approx(
                10.00000, rel=1e-5
            ) == iscale.get_constraint_transform_applied_scaling_factor(c)
        for t, c in FB1D.solids_energy_holdup_constraints.items():
            assert pytest.approx(
                1.27324e-7, rel=1e-5
            ) == iscale.get_constraint_transform_applied_scaling_factor(c)
        for t, c in FB1D.solids_energy_accumulation_constraints.items():
            assert pytest.approx(
                1.27324e-7, rel=1e-5
            ) == iscale.get_constraint_transform_applied_scaling_factor(c)
...
class TestIronOC_EnergyBalanceType(object):
    ...
    #             10.00000, rel=1e-5
    #         ) == iscale.get_constraint_transform_applied_scaling_factor(c)
    #     for t, c in FB1D.isothermal_solid_phase.items():
    #         assert pytest.approx(
    #             0.01000, rel=1e-5
    #         ) == iscale.get_constraint_transform_applied_scaling_factor(c)
