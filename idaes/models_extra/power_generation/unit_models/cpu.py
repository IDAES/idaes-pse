...
class CarbonProcessingUnitData(UnitModelBlockData):
    ...
    def build(self):
        ...
        self.inlet = Port(noruleinit=True,
                          doc="A port for the co2 rich inlet stream")
        self.pureco2 = Port(noruleinit=True,
                            doc="A port for the pure CO2 outlet stream")
        self.water = Port(noruleinit=True,
                          doc="A port for the water outlet stream")
        self.vent = Port(noruleinit=True,
                         doc="A port for thevent gas outlet stream")
    ...
    def calculate_scaling_factors(self):
        ...
        for t in self.flowsheet().time:
            ...
            iscale.constraint_scaling_transform(
                self.mole_frac_comp_pureco2_eqn[t], 1)

            iscale.constraint_scaling_transform(
                self.mole_frac_comp_water_eqn[t], 1)

            iscale.constraint_scaling_transform(
                self.mole_frac_comp_vent_eqn[t], 1)

            iscale.constraint_scaling_transform(
                self.heat_duty_eq[t], 1e-6)

            iscale.constraint_scaling_transform(
                self.work_eq[t], 1e-6)

            iscale.constraint_scaling_transform(
                self.pureco2_temperature_eq[t], 1e-2)

            iscale.constraint_scaling_transform(
                self.pureco2_pressure_eq[t], 1e-5)

            iscale.constraint_scaling_transform(
                self.pureco2_total_flow_eq[t], 1e-2)

            iscale.constraint_scaling_transform(
                self.pureco2_co2_flow_mol_comp_eq[t], 1e-2)

            iscale.constraint_scaling_transform(
                self.pureco2_o2_flow_mol_comp_eq[t], 1)

            iscale.constraint_scaling_transform(
                self.pureco2_ar_flow_mol_comp_eq[t], 1)

            iscale.constraint_scaling_transform(
                self.pureco2_h2o_flow_mol_comp_eq[t], 1e-2)

            iscale.constraint_scaling_transform(
                self.water_total_flow_eq[t], 1e-2)

            iscale.constraint_scaling_transform(
                self.water_temperature_eq[t], 1e-2)

            iscale.constraint_scaling_transform(
                self.water_pressure_eq[t], 1e-5)

            iscale.constraint_scaling_transform(
                self.water_co2_flow_mol_comp_eq[t], 1e-2)

            iscale.constraint_scaling_transform(
                self.water_o2_flow_mol_comp_eq[t], 1)

            iscale.constraint_scaling_transform(
                self.water_ar_flow_mol_comp_eq[t], 1)

            iscale.constraint_scaling_transform(
                self.water_h2o_flow_mol_comp_eq[t], 1e-2)

            iscale.constraint_scaling_transform(
                self.vent_temperature_eq[t], 1e-2)

            iscale.constraint_scaling_transform(
                self.vent_pressure_eq[t], 1e-5)
