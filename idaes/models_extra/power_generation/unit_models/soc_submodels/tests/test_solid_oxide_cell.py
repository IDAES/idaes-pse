...
def _get_comp_dict_interconnect(nt, nz):
    return {
        pyo.Var: {
            "current_density": nz * nt,
            "potential": nt,
...
def build_tester_interconnect(cell, nt, nz):
    soc_testing._build_test_utility(
        cell,
        comp_dict=_get_comp_dict_interconnect(nt, nz),
    )
...
@pytest.fixture
def model_interconnect_heat_loss():
    time_set = [0]
    zfaces = np.linspace(0, 1, 6).tolist()
    xfaces_electrode = np.linspace(0, 1, 4).tolist()
    xfaces_electrolyte = np.linspace(0, 1, 12).tolist()
...
@pytest.mark.build
@pytest.mark.unit
def test_build_interconnect_heat_loss(
    model_interconnect_heat_loss,
):
    cell = model_interconnect_heat_loss.fs.cell
    nt = len(model_interconnect_heat_loss.fs.time)
    nz = len(cell.zfaces) - 1

    comp_dict = _get_comp_dict_interconnect(nt, nz)
    comp_dict[pyo.Var]["heat_loss_flux"] = nt * nz
    comp_dict[pyo.Var]["total_heat_loss"] = nt
    comp_dict[pyo.Constraint]["heat_loss_eqn"] = nt * nz
    comp_dict[pyo.Constraint]["total_heat_loss_eqn"] = nt * nz
...
@pytest.mark.unit
def test_no_interconnect_heat_loss():
    time_set = [0]
    zfaces = np.linspace(0, 1, 6).tolist()
    xfaces_electrode = np.linspace(0, 1, 4).tolist()
    xfaces_electrolyte = np.linspace(0, 1, 12).tolist()
    ...
    with pytest.raises(
        ...
        m.fs.cell = soc.SolidOxideCell(
            ...
            inert_oxygen_species_triple_phase_boundary=["N2", "H2O"],
            flow_pattern=HeatExchangerFlowPattern.cocurrent,
            flux_through_interconnect=False,
            control_volume_xfaces_interconnect=xfaces_interconnect,
            has_heat_loss_term=True,
        )
...
@pytest.fixture
def model_interconnect_log_exchange_current_modifier():
    time_set = [0]
    zfaces = np.linspace(0, 1, 6).tolist()
    xfaces_electrode = np.linspace(0, 1, 4).tolist()
    xfaces_electrolyte = np.linspace(0, 1, 12).tolist()
...
@pytest.mark.build
@pytest.mark.unit
def test_build_interconnect_log_exchange_current_modifier(
    model_interconnect_log_exchange_current_modifier,
):
    cell = model_interconnect_log_exchange_current_modifier.fs.cell
    nt = len(model_interconnect_log_exchange_current_modifier.fs.time)
    nz = len(cell.zfaces) - 1
    ...
    for tpb in tpb_list:
        ...
        assert cell.length_y is tpb.length_y.referent
        assert cell.length_z is tpb.length_z.referent
        assert hasattr(tpb, "log_exchange_current_modifier")
