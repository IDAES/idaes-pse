        ...
        for ctype, sub_dict in comp_dict.items():
            for attr, length in sub_dict.items():
                try:
                    ...
                    raise AttributeError(
                        f"{ctype} {attr} missing from block {block.name}."
                        )
...
class TestChannelYesHoldupNoDynamics(object):
    ...
    def test_build_modelNoHoldup(self, modelNoHoldup):
        ...
        build_test_utility(block=channel,
           comp_dict=comp_dict,
        )
...
class TestElectrode(object):
    ...
    def modelHoldupNotDynamic(self):
        ...
        m.fs.Dconc_x0 = pyo.Var(tset, iznodes, comps, initialize=0,
                                units=pyo.units.mol/pyo.units.m**3)
        m.fs.dconc_refdt = pyo.Param(tset, iznodes, comps, initialize=0,
                            units=pyo.units.mol/(pyo.units.m**3*pyo.units.s))
        m.fs.xflux_x0 = pyo.Var(tset, iznodes, comps, initialize=0,
...
class TestSolidOxideCell(object):
    ...
    def test_build(self, model):
        ...
        assert (cell.oxygen_electrode.Dtemp_x0 
