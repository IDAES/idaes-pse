...
from idaes.models_extra.power_generation.unit_models.soc_submodels import (
    comp_enthalpy_expr, comp_entropy_expr, SolidOxideCell)
from idaes.models.unit_models.heat_exchanger import HeatExchangerFlowPattern

data_cache = os.sep.join(
    [this_file_dir(), 'data_cache'])
...
def model_func():
    ...
    m.fs.prop_Iapws95 = iapws95.Iapws95StateBlock(default={"parameters": m.fs.propertiesIapws95})
    ...
    m.fs.cell.contact_flow_mesh_fuel_electrode.log_preexponential_factor\
        .fix(pyo.log(0.55e-4))
    m.fs.cell.contact_flow_mesh_oxygen_electrode.log_preexponential_factor\
        .fix(pyo.log(0.55e-4))
    m.fs.cell.contact_interconnect_fuel_flow_mesh.log_preexponential_factor\
        .fix(pyo.log(0.18e-4))
    m.fs.cell.contact_interconnect_oxygen_flow_mesh.log_preexponential_factor\
        .fix(pyo.log(0.18e-4))
    ...
    m.fs.cell.fuel_electrode.resistivity_log_preexponential_factor\
        .fix(pyo.log(8.856e-6))
    ...
    m.fs.cell.oxygen_electrode.resistivity_log_preexponential_factor\
        .fix(pyo.log(1.425e-4))
    ...
    m.fs.cell.electrolyte.resistivity_log_preexponential_factor.fix(
        pyo.log(1.07e-4))
    ...
    m.fs.cell.fuel_tpb.exchange_current_log_preexponential_factor.fix(
        pyo.log(5.5e8))
    ...
    m.fs.cell.oxygen_tpb.exchange_current_log_preexponential_factor.fix(
        pyo.log(7e8))
...
def kazempoor_braun_replication(model):
    ...
    tracked_vars = {
        ...
        "average_current_density":
            sum(cell.current_density[0,j] for j in cell.iznodes)
            / len(cell.iznodes),
        "average_temperature_z":
            sum(cell.temperature_z[0,j] for j in cell.iznodes)
            / len(cell.iznodes),
    ...
    for case in range(1,N_case+1):
        ...
        dHrxn = (comp_enthalpy_expr(T_in,"H2") + 0.5*comp_enthalpy_expr(T_in,"O2")
                  -comp_enthalpy_expr(T_in,"H2O"))
        dSrxn = (comp_entropy_expr(T_in,"H2") + 0.5*comp_entropy_expr(T_in,"O2")
                  -comp_entropy_expr(T_in,"H2O"))
        dGrxn = dHrxn-T_in*dSrxn
        
        V_nernst = pyo.value(1/(2*_constF)*(dGrxn - 8.31*T_in*pyo.log(
                                        y_H2O/(y_H2*0.21**0.5)*(P/101300)**-0.5)))
...
def test_model_replication(model):
    ...
    for i in range(len(out)):
        cached_results.append(pd.read_csv(
            os.sep.join([data_cache, f"case_{i+1}.csv"]), index_col=0))
    ...
    df = out[4]
    data = pd.read_csv(os.sep.join([data_cache,
                       "sweep_5_kazempoor_replication.csv"]),
                        names=["average_current_density","voltage"])
    intr = interp1d(x=data["voltage"].to_numpy(),
                    y=data["average_current_density"].to_numpy())
    for row in df.iterrows():
