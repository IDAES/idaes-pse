...
def test_PP_costing():
    ...
    assert pytest.approx(pyo.value(m.fs.costing.total_TPC), abs=1e-1) == 993753 / 1e3
...
def test_power_plant_costing():
    ...
    get_PP_costing(m.fs.NGCC, steam_turbine_accounts, m.fs.NGCC.turbine_power, "kW", 6)
    ...
    get_PP_costing(m.fs.AUSC, AUSC_accounts, m.fs.AUSC.feedwater_flow, "lb/hr", 7)
    ...
    get_PP_costing(m.fs.CCS, CCS_accounts, m.fs.CCS.CO2_flow, "lb/hr", 6)
...
def test_sCO2_costing():
    ...
    get_sCO2_unit_cost(m.fs.HTR, "Recuperator", m.fs.HTR.UA, temp_C=m.fs.HTR.temp)
    ...
    get_sCO2_unit_cost(m.fs.LTR, "Recuperator", m.fs.LTR.UA, temp_C=m.fs.LTR.temp)
