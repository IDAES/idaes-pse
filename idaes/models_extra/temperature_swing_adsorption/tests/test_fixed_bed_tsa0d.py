...
# To cover all methods well have 3 tests
# 1. Zeolite, calculate_beds=False, compressor=False, steam_calculations=None
# 2. Mg-MOF, calculate_beds=True, compressor=False, steam_calculations="simplified"
# 3. polystyrene-amine, calculate_beds=False, compressor=True, steam_calculations="rigorous"
...
assert not hasattr(m.fs.unit, "nL_inf")
assert hasattr(m.fs.unit, "qm0")
# make sure calculate beds is right
assert isinstance(m.fs.unit.velocity_in, Expression)
# make sure there is no compressor or heater
assert hasattr(m.fs.unit, "compressor")
assert not hasattr(m.fs.unit, "flow_mass_steam")
...
# )
# # air velocity
# assert pytest.approx(0.41747, abs=1e-3) == value(
