...
def _get_costing_electric_boiler(tsa):
    ...
    sorbent_makeup_rate = (
        units.convert(
            tsa.bed_volume, to_units=units.ft**3) *
        (1 - tsa.bed_voidage) *
        tsa.number_beds / 0.5 / 365 / units.d)  # [ft^3/d]
    # CO2 product mass flow rate - from model
    CO2_product_mass_flow = units.convert(
        tsa.mw["CO2"] *
        tsa.flow_mol_co2_rich_stream[0, "CO2"],
    ...
    total_auxiliary_load = (
        ...
        boiler_auxiliary_load +
        units.convert(
            tsa.compressor.unit.work_mechanical[0],
            to_units=units.kW))
    ...
    auxiliary_load_2_beds = 2 * units.convert(
        tsa.compressor.unit.work_mechanical[0],
        to_units=units.kW) / tsa.number_beds  # [kW]
    ...
    water_rate = (
        units.convert(tsa.flow_mol_in_total,
    ...
    sorbent_rate = (
        units.convert(
            tsa.bed_volume, to_units=units.ft**3) *
        (1 - tsa.bed_voidage) *
        tsa.number_beds / 0.5 / 365 / units.day
    )
    ...
    steam_rate = units.convert(
        tsa.flow_mass_steam, to_units=units.kg/units.day
    )
