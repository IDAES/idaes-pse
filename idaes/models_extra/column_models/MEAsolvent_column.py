...
class MEAColumnData(PackedColumnData):
    ...
    def build(self):
        ...
        self.velocity_vap_eqn = Constraint(self.flowsheet().time,
                         self.vapor_phase.length_domain,
        ...
        self.velocity_liq_eqn = Constraint(self.flowsheet().time,
                         self.liquid_phase.length_domain,
        ...
        self.area_interfacial_constraint = Constraint(self.flowsheet().time,
                                            self.vapor_phase.length_domain,
        ...
        self.holdup_parA = Var(initialize=24.23,
                            units=None,
        ...
        def rule_mass_transfer_coeff_liq(blk, t, x, j):
            if x == self.liquid_phase.length_domain.last():
                ...
                return blk.mass_transfer_coeff_liq[t, x, j] == blk.Cl_ref*(12**(1/6))*(blk.velocity_liq[t, x]*\
        ...
        self.co2_conc_interface = Expression(self.flowsheet().time,
                              self.liquid_phase.length_domain,
        ...
        def rule_instantaneous_E(blk, t, x):
            if x == blk.liquid_phase.length_domain.last():
                ...
                return 1 + (blk.liquid_phase.properties[t, x].diffus_phase_comp_true['Liq','MEA'] *
                            blk.liquid_phase.properties[t, x].conc_mol_phase_comp_true['Liq','MEA']) /\
                    (2 * blk.liquid_phase.properties[t, x].diffus_phase_comp_true['Liq','CO2'] *
                        blk.co2_conc_interface[t, x])
        ...
        def rule_heat_transfer_coeff(blk, t, x):
            if x == blk.vapor_phase.length_domain.first():
                ...
                return blk.heat_transfer_coeff_base[t, x] == blk.mass_transfer_coeff_vap[t, x, 'CO2'] *\
                    blk.vapor_phase.properties[t, x].pressure *\
        ...
        def rule_flood_velocity(blk, t, x):
            ...
            if x == blk.vapor_phase.length_domain.first():
                ...
                return (((CONST.acceleration_gravity*(blk.eps_ref)**3/blk.packing_specific_area)*(dens_liq/dens_vap)*
                        (blk.liquid_phase.properties[t, x_liq].visc_d_phase['Liq']/mu_water)**(-0.2)*
                            exp(-4*(H)**0.25))**0.5)
    
        self.gas_velocity_fld = Expression(self.flowsheet().time,
                                           self.vapor_phase.length_domain,
    ...
    def calculate_scaling_factors_props(blk):
        ...
        for x in blk.liquid_phase.length_domain:
            ...
            iscale.set_scaling_factor(blk.liquid_phase.properties[0, x].mole_frac_phase_comp_true['Liq','CO2'], 1e4)
            
            iscale.set_scaling_factor(blk.liquid_phase.properties[0, x].mole_frac_phase_comp_true['Liq','HCO3_-'], 100)
            
            iscale.set_scaling_factor(blk.liquid_phase.properties[0, x].mole_frac_phase_comp_true['Liq','MEACOO_-'], 10)
            
            iscale.set_scaling_factor(blk.liquid_phase.properties[0, x].mole_frac_phase_comp_true['Liq','MEA_+'], 10)
            
            iscale.set_scaling_factor(blk.liquid_phase.properties[0, x].mole_frac_phase_comp_true['Liq','MEA'], 1)
            
            iscale.set_scaling_factor(blk.liquid_phase.properties[0, x].mole_frac_phase_comp_true['Liq','H2O'], 1)
            ...
            iscale.set_scaling_factor(blk.liquid_phase.properties[0, x].log_k_eq['carbamate'], 1)
            
            iscale.set_scaling_factor(blk.liquid_phase.properties[0, x].log_k_eq['bicarbonate'], 1)
        ...
        for (t, x), v in blk.liquid_phase.properties.items():
            for (p, j), c in v.appr_to_true_species.items():
                iscale.constraint_scaling_transform(
                    ...
                    iscale.get_scaling_factor(
                        v.flow_mol_phase_comp_true[p, j],
                        default=1,
                        warning=True))
            ...
            for j, c in v.component_flow_balances.items():
                ...
                iscale.constraint_scaling_transform(
                    v.component_flow_balances['H2O'],1)
                iscale.constraint_scaling_transform(
                    v.component_flow_balances['MEA'],1)
    ...
    def calculate_scaling_factors_control_vol(blk):
        ...
        for x in blk.vapor_phase.length_domain:
            ...
            iscale.set_scaling_factor(blk.vapor_phase.enthalpy_flow_dx[0, x, 'Vap'], 1e-6)
                
        for x in blk.liquid_phase.length_domain:
            iscale.set_scaling_factor(blk.liquid_phase._enthalpy_flow[0, x, 'Liq'], 1e-8)
            
            iscale.set_scaling_factor(blk.liquid_phase.enthalpy_flow_dx[0, x, 'Liq'], 1e-6)
        ...
        for (t, x, p, j), v in blk.vapor_phase._flow_terms.items():
            iscale.set_scaling_factor(
                blk.vapor_phase._flow_terms[t, x, p, j], 0.01)
        ...
        for (t, x, p, j), v in blk.liquid_phase._flow_terms.items():
            iscale.set_scaling_factor(
                blk.liquid_phase._flow_terms[t, x, p, j], 0.01)
        ...
        for (t, x, p, j), v in blk.vapor_phase.material_flow_linking_constraints.items():
            iscale.constraint_scaling_transform(
                v,
                iscale.get_scaling_factor(
                    blk.vapor_phase._flow_terms[t, x, p, j],
                    default=1,
                    warning=True))
        ...
        for (t, x, p), v in blk.vapor_phase.enthalpy_flow_linking_constraint.items():
            iscale.constraint_scaling_transform(
                v,
                iscale.get_scaling_factor(
                    blk.vapor_phase._enthalpy_flow[t, x, p],
                    default=1,
                    warning=True))
        ...
        for (t, x, p), v in blk.vapor_phase.enthalpy_flow_dx_disc_eq.items():
            iscale.constraint_scaling_transform(
                v,
                iscale.get_scaling_factor(
                    blk.vapor_phase.enthalpy_flow_dx[t, x, p],
                    default=1,
                    warning=True))
        ...
        for (t, x, p, j), v in blk.liquid_phase.material_flow_linking_constraints.items():
            iscale.constraint_scaling_transform(
                v,
                iscale.get_scaling_factor(
                    blk.liquid_phase._flow_terms[t, x, p, j],
                    default=1,
                    warning=True))
        ...
        for (t, x, p), v in blk.liquid_phase.enthalpy_flow_linking_constraint.items():
            iscale.constraint_scaling_transform(
                v,
                iscale.get_scaling_factor(
                    blk.liquid_phase._enthalpy_flow[t, x, p],
                    default=1,
                    warning=True))
        ...
        for (t, x, p), v in blk.liquid_phase.enthalpy_flow_dx_disc_eq.items():
            iscale.constraint_scaling_transform(
                v,
                iscale.get_scaling_factor(
                    blk.liquid_phase.enthalpy_flow_dx[t, x, p],
                    default=1,
                    warning=True))
    ...
    def calculate_scaling_factors_unit_model(blk):
        ...
        for (t, x, j), v in blk.pressure_equil.items():
            if x!=0:
                iscale.set_scaling_factor(v, 1/value(blk.liquid_phase.properties[t, x].fug_phase_comp["Liq", j]))
            else:
                iscale.set_scaling_factor(v, 1)
                
        for (t, x, j), v in blk.interphase_mass_transfer.items():
            if x!=0:
                iscale.set_scaling_factor(v, 1/value(blk.interphase_mass_transfer[t, x, j]))
            else:
                iscale.set_scaling_factor(v, 1)
        ...
        for (t, x), v in blk.mechanical_equilibrium.items():
            iscale.constraint_scaling_transform(
                v, iscale.get_scaling_factor(
                    blk.liquid_phase.properties[t, x].pressure,
                    default=1,
                    warning=False))
        ...
        for (t, x), v in blk.enthalpy_transfer_eqn1.items():
            iscale.constraint_scaling_transform(
                v,
                iscale.get_scaling_factor(
                    blk.vapor_phase.enthalpy_transfer[t, x],
                    default=1,
                    warning=True))
        ...
        for (t, x), v in blk.enthalpy_transfer_eqn2.items():
            iscale.constraint_scaling_transform(
                v,
                iscale.get_scaling_factor(
                    blk.liquid_phase.enthalpy_transfer[t, x],
                    default=1,
                    warning=True))
    ...
    def set_init_values_correlation_vars(blk, nfe):
        ...
        k_v_co2_values = [0, 2.81735E-05, 2.82192E-05, 2.82675E-05, 2.83159E-05, 
                      2.83646E-05, 2.84139E-05, 2.84643E-05, 2.85162E-05, 
                      2.85701E-05, 2.86267E-05, 2.86865E-05, 2.87505E-05,
                      2.88196E-05, 2.88949E-05, 2.89779E-05, 2.90706E-05,
                      2.91755E-05, 2.92957E-05, 2.94356E-05, 2.96012E-05,
                      2.98008E-05, 3.00461E-05, 3.03537E-05, 3.07478E-05,
                      3.12618E-05, 3.19377E-05, 3.28062E-05, 3.37808E-05,
                      3.42191E-05, 3.1977E-05]
        
        k_v_h2o_values = [0, 3.70671E-05, 3.7119E-05, 3.71728E-05, 3.72266E-05,
                          3.72807E-05, 3.73355E-05, 3.73914E-05, 3.7449E-05,
                          3.75089E-05, 3.75716E-05, 3.76381E-05, 3.7709E-05,
                          3.77855E-05, 3.7869E-05, 3.7961E-05, 3.80637E-05, 
                          3.81798E-05, 3.83129E-05, 3.84679E-05, 3.86512E-05,
                          3.88721E-05, 3.91436E-05, 3.94843E-05, 3.99212E-05,
                          4.0492E-05, 4.12453E-05, 4.22199E-05, 4.33301E-05, 
                          4.38732E-05, 4.14247E-05]
        
        # Define liquid phase mass transfer coefficient known values
        k_l_co2_values = [9.40406E-05, 9.45346E-05, 9.50364E-05, 9.55322E-05,
                          9.60246E-05, 9.65175E-05, 9.70151E-05, 9.75218E-05,
                          9.80421E-05, 9.85809E-05, 9.91439E-05, 9.97374E-05,
                          0.000100369, 0.000101047, 0.000101783, 0.000102591,
                          0.000103486, 0.000104491, 0.000105632, 0.000106947,
                          0.000108484, 0.000110305, 0.000112497, 0.000115169,
                          0.000118462, 0.000122521, 0.000127414, 0.000132803,
                          0.000136779, 0.000131874, 0.001]
        
        # Define heat transfer coefficient values
        h_v_values = [100, 101.6962933, 101.8538124, 102.001834, 102.148048, 102.2944317,
                      102.4423311, 102.5930583, 102.7479984, 102.9086651, 103.0767604,
                      103.2542451, 103.4434301, 103.6470939, 103.8686406, 104.1123158,
                      104.3835061, 104.6891628, 105.0384114, 105.4434399, 105.9208118,
                      106.4934206, 107.1933964, 108.066322, 109.176855, 110.614142,
                      112.487677, 114.87132, 117.5153935, 118.6672687, 112.4694233]
        
        # Define interfacial area known values
        interfacial_area_values = [0, 197.8185136, 197.8671837, 197.9188717, 197.9708021, 
                                    198.0230645, 198.076072, 198.130292, 198.1862321, 
                                    198.2444542, 198.3055974, 198.3704047, 198.4397597, 
                                    198.5147328, 198.5966454, 198.6871572, 198.7883889,
                                    198.9030972, 199.0349284, 199.1887917, 199.3714147,
                                    199.5921766, 199.8643474, 200.2068651, 200.6465777,
                                    201.219827, 201.9679499, 202.9052757, 203.8836586,
                                    204.1277654, 201.2884216]
        
        # Define enhancement factor known values
        enhancement_factor_values = [10.59208019, 10.83945557, 11.08225811, 11.3273241,
                                      11.5773751, 11.83479677, 12.10215397, 12.38234194,
                                      12.6787182, 12.99526603, 13.33681033, 13.70931137,
                                      14.12027367, 14.57932831, 15.09908116, 15.69637908,
                                      16.39424972, 17.2249598, 18.23499171,19.49343044,
                                      21.10665852, 23.24523241, 26.19540321, 30.46301154,
    ...
    def initialize(blk,
        ...
        velocity_constraints = ["velocity_vap_eqn",
                                "velocity_liq_eqn"]
        ...
        init_log.info('Initializing interfacial area - degrees_of_freedom = {}'.format(degrees_of_freedom(blk)))
        ...
        init_log.info('Initializing liquid holdup- degrees_of_freedom = {}'.format(degrees_of_freedom(blk)))
