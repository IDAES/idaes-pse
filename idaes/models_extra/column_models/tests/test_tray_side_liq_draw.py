...
class TestBTXIdeal:
    ...
    def test_solution(self, btx_ftpz, btx_fctp):
        ...
        # liq_out port
        assert pytest.approx(0.23168, abs=1e-3) == value(
        ...
        # side liq_out port
        assert pytest.approx(0.23168, abs=1e-3) == value(
            btx_ftpz.fs.unit.liq_side_draw.flow_mol[0]
        )
        assert pytest.approx(0.33313, abs=1e-3) == value(
            btx_ftpz.fs.unit.liq_side_draw.mole_frac_comp[0, "benzene"]
        )
        assert pytest.approx(0.66686, abs=1e-3) == value(
            btx_ftpz.fs.unit.liq_side_draw.mole_frac_comp[0, "toluene"]
        )
        assert pytest.approx(370.567, abs=1e-3) == value(
            btx_ftpz.fs.unit.liq_side_draw.temperature[0]
        )
        assert pytest.approx(101325, abs=1e-3) == value(
            btx_ftpz.fs.unit.liq_side_draw.pressure[0]
        )
        ...
        # liq_out port
        assert pytest.approx(0.07717, abs=1e-3) == value(
            btx_fctp.fs.unit.liq_out.flow_mol_comp[0, "benzene"]
        )
        assert pytest.approx(0.15449, abs=1e-3) == value(
        ...
        # side liq_out port
        assert pytest.approx(0.07717, abs=1e-3) == value(
            btx_fctp.fs.unit.liq_side_draw.flow_mol_comp[0, "benzene"]
        )
        assert pytest.approx(0.15449, abs=1e-3) == value(
            btx_fctp.fs.unit.liq_side_draw.flow_mol_comp[0, "toluene"]
        )
        assert pytest.approx(370.567, abs=1e-3) == value(
            btx_fctp.fs.unit.liq_side_draw.temperature[0]
        )
        assert pytest.approx(101325, abs=1e-3) == value(
            btx_fctp.fs.unit.liq_side_draw.pressure[0]
        )

        # vap_out port
        assert pytest.approx(0.8456, abs=1e-3) == value(
