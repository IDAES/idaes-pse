...
from pyomo.environ import ConcreteModel, SolverFactory, value, \
    TransformationFactory, Constraint, Var, Param, Block
...
from idaes.unit_models.power_generation.IdealProp_FlueGas import FlueGasParameterBlock

# Import Unit Model Modules
from idaes.unit_models import Heater, Mixer
from idaes.unit_models.power_generation.boiler_heat_exchanger import (
...
#    print(h)
    ...
    m.fs.PrSH.side_2_inlet.flow_component[0,"N2"].fix(FGrate*(8.69
                                                     +14.49+2.47+0.06+0.2)/100)
...
#  ----- Finishing Superheater ----------------------
    ...
    m.fs.FSH.side_2_inlet.flow_component[0,"N2"].fix(FGrate*(8.69
                                                     +14.49+2.47+0.06+0.2)/100)
...
#  ----- Reheater Superheater ----------------------
    ...
    m.fs.RH.side_2_inlet.flow_component[0,"N2"].fix(FGrate*(8.69
                                                     +14.49+2.47+0.06+0.2)/100)
...
#   splitter flue gas from Finishing SH to Reheater and Primary SH
    ...
    m.fs.Spl1.inlet.flow_component[0,"N2"].fix(FGrate*(8.69
                                                     +14.49+2.47+0.06+0.2)/100)
...
#     ['Reheat_out', 'PrSH_out']
    ...
    m.fs.mix1.Reheat_out.flow_component[0,"N2"].fix(FGrate*(8.69
                                                    +14.49+2.47+0.06+0.2)/100)
    ...
    m.fs.mix1.PrSH_out.flow_component[0,"N2"].fix(FGrate*(8.69
                                                     +14.49+2.47+0.06+0.2)/100)
...
def unfix_inlets(m):
    ...
    # Use FG molar composition to set component flow rates (baseline report)
    m.fs.ECON.side_2_inlet.flow_component[0,"H2O"].unfix()
    m.fs.ECON.side_2_inlet.flow_component[0,"CO2"].unfix()
    m.fs.ECON.side_2_inlet.flow_component[0,"N2"].unfix()
    m.fs.ECON.side_2_inlet.flow_component[0,"O2"].unfix()
    m.fs.ECON.side_2_inlet.flow_component[0,"NO"].unfix()
    m.fs.ECON.side_2_inlet.flow_component[0,"SO2"].unfix()
    m.fs.ECON.side_2_inlet.temperature[0].unfix()
    m.fs.ECON.side_2_inlet.pressure[0].unfix()
    
    # PrSH Primary superheater inlets (steam and flue gas) --------------------
    m.fs.PrSH.side_1_inlet.flow_mol.unfix()
    m.fs.PrSH.side_1_inlet.enth_mol[0].unfix()      
    m.fs.PrSH.side_1_inlet.pressure[0].unfix()
    
    # Use FG molar composition to set component flow rates (baseline report)
    m.fs.PrSH.side_2_inlet.flow_component[0,"H2O"].unfix()
    m.fs.PrSH.side_2_inlet.flow_component[0,"CO2"].unfix()
    m.fs.PrSH.side_2_inlet.flow_component[0,"N2"].unfix()
    m.fs.PrSH.side_2_inlet.flow_component[0,"O2"].unfix()
    m.fs.PrSH.side_2_inlet.flow_component[0,"NO"].unfix()
    m.fs.PrSH.side_2_inlet.flow_component[0,"SO2"].unfix()
    m.fs.PrSH.side_2_inlet.temperature[0].unfix()
    m.fs.PrSH.side_2_inlet.pressure[0].unfix()
    ...
    # Use FG molar composition to set component flow rates (baseline report)
    m.fs.RH.side_2_inlet.flow_component[0,"H2O"].unfix()
    m.fs.RH.side_2_inlet.flow_component[0,"CO2"].unfix()
    m.fs.RH.side_2_inlet.flow_component[0,"N2"].unfix()
    m.fs.RH.side_2_inlet.flow_component[0,"O2"].unfix()
    m.fs.RH.side_2_inlet.flow_component[0,"NO"].unfix()
    m.fs.RH.side_2_inlet.flow_component[0,"SO2"].unfix()
    m.fs.RH.side_2_inlet.temperature[0].unfix()
    m.fs.RH.side_2_inlet.pressure[0].unfix()
    
    # Finishing Superheater (steam from Platen SH)-----------------------------
    m.fs.FSH.side_1_inlet.flow_mol.unfix()
    m.fs.FSH.side_1_inlet.enth_mol[0].unfix()      
    m.fs.FSH.side_1_inlet.pressure[0].unfix()
    ...
    m.fs.Spl1.inlet.flow_component[0,"H2O"].unfix()
    m.fs.Spl1.inlet.flow_component[0,"CO2"].unfix()
    m.fs.Spl1.inlet.flow_component[0,"N2"].unfix()
    m.fs.Spl1.inlet.flow_component[0,"O2"].unfix()
    m.fs.Spl1.inlet.flow_component[0,"NO"].unfix()
    m.fs.Spl1.inlet.flow_component[0,"SO2"].unfix()
    m.fs.Spl1.inlet.temperature[0].unfix()
    m.fs.Spl1.inlet.pressure[0].unfix()
    
    # unfix mixer inlets (now connected)
    m.fs.mix1.Reheat_out.flow_component[0,"H2O"].unfix()
    m.fs.mix1.Reheat_out.flow_component[0,"CO2"].unfix()
    m.fs.mix1.Reheat_out.flow_component[0,"N2"].unfix()
    m.fs.mix1.Reheat_out.flow_component[0,"O2"].unfix()
    m.fs.mix1.Reheat_out.flow_component[0,"NO"].unfix()
    m.fs.mix1.Reheat_out.flow_component[0,"SO2"].unfix()
    m.fs.mix1.Reheat_out.pressure.unfix()
    m.fs.mix1.Reheat_out.temperature.unfix()
    # PrSH output
    m.fs.mix1.PrSH_out.flow_component[0,"H2O"].unfix()
    m.fs.mix1.PrSH_out.flow_component[0,"CO2"].unfix()
    m.fs.mix1.PrSH_out.flow_component[0,"N2"].unfix()
    m.fs.mix1.PrSH_out.flow_component[0,"O2"].unfix()
    m.fs.mix1.PrSH_out.flow_component[0,"NO"].unfix()
    m.fs.mix1.PrSH_out.flow_component[0,"SO2"].unfix()
...
def print_results(m):
    ...
    if m.fs.PrSH.config.has_radiation == True:
...
if __name__ == "__main__":
   ...
   # unfix inlets to build arcs at the flowsheet level
    ...
    print('flowsheet degrees of freedom = ')
    print(degrees_of_freedom(m))
