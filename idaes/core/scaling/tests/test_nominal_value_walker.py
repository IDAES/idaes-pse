...
class TestNominalValueExtractionVisitor:
    ...
    def test_Expression_hint(self, m):
        ...
        assert NominalValueExtractionVisitor().walk_expression(expr=(1+m.expression)) == [
            1, 17
        ]
    @pytest.mark.unit
    def test_Expression_constant(self, m):
        m.expression2 = pyo.Expression(expr=2)
        assert NominalValueExtractionVisitor().walk_expression(expr=(1+m.expression2)) == [
            1, 2
        ]
    @pytest.mark.unit
    ...
    def test_Expression_evaluation_error(self, m):
        ...
        assert NominalValueExtractionVisitor().walk_expression(expr=(1+m.expression3)) == [
            1, 37
        ]
