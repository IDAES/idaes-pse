...
argument)."""))
    ...
    def add_geometry(self,
        ...
        if self.config.area_definition == DistributedVars.variant:
            self.area = Var(self.flowsheet().time,
                            self.length_domain,
                            initialize=1.0,
    ...
    def _add_material_balance_common(self,
        ...
        if hasattr(self.config, "reaction_package"):
            if self.config.reaction_package is not None:
                rxn_units = self.config.reaction_package.get_metadata().get_derived_units
                if rxn_units('length') is not None:
                ...
                else:  # reaction package has no unit container objects
                    rxn_flow_l_units = None
        ...
        self._flow_terms = Var(self.flowsheet().time,
                               self.length_domain,
                               pc_set,
                               initialize=1.0,
    ...
    def add_total_element_balances(self,
        ...
        self.elemental_flow_term = Var(self.flowsheet().time,
                                       self.length_domain,
    ...
    def add_total_enthalpy_balances(self,
        ...
        # Create variables
        self._enthalpy_flow = Var(self.flowsheet().time,
                                  self.length_domain,
                                  phase_list,
                                  initialize=1.0,
    ...
    def initialize(blk, state_args=None, outlvl=idaeslog.NOTSET, optarg=None,
        ...
        source = blk.properties[blk.flowsheet().time.first(),
                                source_idx]
