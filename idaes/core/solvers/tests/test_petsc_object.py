...
def test_refresh_true_execution_order():
    ...
    with patch.object(petsc_obj, "refresh_model", wraps=petsc_obj.refresh_model) as mock_refresh, \
         patch.object(petsc_obj, "get_initial_condition_problem", wraps=petsc_obj.get_initial_condition_problem) as mock_ic, \
         patch.object(petsc_obj, "get_timestep_problem", wraps=petsc_obj.get_timestep_problem) as mock_ts:
...
def test_refresh_model_false():
    ...
    with patch.object(petsc_obj, "refresh_model", wraps=petsc_obj.refresh_model) as mock_refresh:
        petsc_obj.dae_by_time_element(between=[m.t.first(), m.t.at(2)], refresh_model=False)
...
def test_get_initial_condition_problem_time_point_default():
    ...
    with patch.object(petsc_obj, "get_initial_condition_problem", wraps=petsc_obj.get_initial_condition_problem) as mock_ic:
