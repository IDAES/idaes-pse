...
@pytest.fixture
def pysmo_krig_surrogate():
    # import surrogates from external JSON

    pysmo_krig_surrogate = PysmoSurrogate.load_from_file(
            os.path.join(this_file_dir(), 'pysmo_krig_surrogate.json'))

    return pysmo_krig_surrogate
...
def test_scatter2D_pysmo_krig(pysmo_krig_surrogate, data_validation):
    ...
    with TempfileManager.new_context() as tf:
        ...
        surrogate_scatter2D(pysmo_krig_surrogate, data_validation,
...
def test_scatter3D_pysmo_krig(pysmo_krig_surrogate, data_validation):
    ...
    with TempfileManager.new_context() as tf:
        ...
        dname = tf.mkdtemp()
        filename = os.path.join(dname, 'results.pdf')
        surrogate_scatter3D(pysmo_krig_surrogate, data_validation,
                            filename=filename, show=False)
...
def test_parity_pysmo_krig(pysmo_krig_surrogate, data_validation):
    ...
    with TempfileManager.new_context() as tf:
        ...
        dname = tf.mkdtemp()
        filename = os.path.join(dname, 'results.pdf')
        surrogate_parity(pysmo_krig_surrogate, data_validation,
                         filename=filename, show=False)
...
def test_residual_pysmo_krig(pysmo_krig_surrogate, data_validation):
    ...
    with TempfileManager.new_context() as tf:
        ...
        dname = tf.mkdtemp()
        filename = os.path.join(dname, 'results.pdf')
        surrogate_residual(pysmo_krig_surrogate, data_validation,
                           filename=filename, show=False)
...
def test_scatter2D_noPDF_pysmo_krig(pysmo_krig_surrogate, data_validation):
    ...
    with TempfileManager.new_context() as tf:
        ...
        # create and step into new temporary directory
        dname = tf.mkdtemp()
        surrogate_scatter2D(pysmo_krig_surrogate, data_validation, show=False)
...
def test_scatter3D_noPDF_pysmo_krig(pysmo_krig_surrogate, data_validation):
    ...
    with TempfileManager.new_context() as tf:
        ...
        # create and step into new temporary directory
        dname = tf.mkdtemp()
        surrogate_scatter3D(pysmo_krig_surrogate, data_validation, show=False)
...
def test_parity_noPDF_pysmo_krig(pysmo_krig_surrogate, data_validation):
    ...
    with TempfileManager.new_context() as tf:
        ...
        # create and step into new temporary directory
        dname = tf.mkdtemp()
        surrogate_parity(pysmo_krig_surrogate, data_validation, show=False)
...
def test_residual_noPDF_pysmo_krig(pysmo_krig_surrogate, data_validation):
    ...
    with TempfileManager.new_context() as tf:
        ...
        # create and step into new temporary directory
        dname = tf.mkdtemp()
        surrogate_residual(pysmo_krig_surrogate, data_validation, show=False)
