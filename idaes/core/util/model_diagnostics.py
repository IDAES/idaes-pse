...
CONFIG.declare(
    ...
    ConfigValue(
        default=True,
        domain=Bool,
        description="Use the scaled Jacobian for numerical diagnostics.",
    ),
)
CONFIG.declare(
...
class DiagnosticsToolbox:
    ...
    @property
    def jacobian(self) -> tuple:
        """
        Tuple containing (Jacobian matrix in SciPy CSR format, PyNumero nlp)
        as generated by idaes.core.util.scaling.get_jacobian
        """
        if self.config.use_scaled_jacobian:
    ...
    def display_variables_with_extreme_jacobians(
        self, stream=None, use_scaled_jacobian: bool = None, force_jacobian_recalc: bool = False
    ):
        """
        ...
        Args:
            ...
            use_scaled_jacobian: True if scaled Jacobian should be used for diagnostic reporting (default = None)
                If used, overrides, self.config.use_scaled_jacobian (also defaults to True)
            force_jacobian_recalc: True if Jacobian should be re-calculated instead of using self._jacobian
                (default = False because this is usually run after report_numerical_issues())
                This is helpful if the model has changed or been re-solved since the last time diagnostics were run,
                so Jacobian would need to be recalculated to reflect changes.
        ...
        use_scaled_jacobian = (
            self.config.use_scaled_jacobian if use_scaled_jacobian is None else use_scaled_jacobian
        )
        if use_scaled_jacobian:
            if self._scaled_jacobian is None or force_jacobian_recalc:
                self._scaled_jacobian = get_jacobian(self._model, scaled=use_scaled_jacobian)
            jac, nlp = self._scaled_jacobian
        else:
            if self._jacobian is None or force_jacobian_recalc:
                self._jacobian = get_jacobian(self._model, scaled=use_scaled_jacobian)
            jac, nlp = self._jacobian

        xjc = extreme_jacobian_columns(
            m=self._model,
            scaled=use_scaled_jacobian, # ignored if jac and nlp are provided
            jac=jac,
            nlp=nlp,
    ...
    def display_constraints_with_extreme_jacobians(
        self, stream=None, use_scaled_jacobian: bool = None, force_jacobian_recalc: bool = False
    ):
        """
    ...
    def display_extreme_jacobian_entries(
        self, stream=None, use_scaled_jacobian: bool = None, force_jacobian_recalc: bool = False
    ):
        """
    ...
    def _collect_numerical_warnings(
        self, jac=None, nlp=None, ignore_parallel_components=False, use_scaled_jacobian: bool=None
    ):
        """
        ...
        Args:
            ...
            use_scaled_jacobian: True if scaled Jacobian should be used for diagnostic reporting
                (default = self.config.user_scaled_jacobian). If used, overrides
                self.config.use_scaled_jacobian (defaults to True).
    ...
    def _collect_numerical_cautions(self, jac=None, nlp=None, use_scaled_jacobian: bool = None):
        """
        ...
        if len(extreme_jac) > 0:
            cstring = "Entry" if len(extreme_jac) == 1 else "Entries"
            cautions.append(
    ...
    def report_numerical_issues(self, stream=None, use_scaled_jacobian: bool=None, force_jacobian_recalc: bool=True):
        """
        ...
        Args:
            ...
            use_scaled_jacobian: True if scaled Jacobian should be used for diagnostic reporting
                ...
                self.config.use_scaled_jacobian (defaults to True).
            force_jacobian_recalc: True if Jacobian should be re-calculated instead of using self._jacobian
                (default = True because this is usually the first step in evaluating a newly solved model)
                This is helpful if the model has changed or been re-solved since the last time diagnostics were run,
                so Jacobian would need to be recalculated to reflect changes.
        ...
        try:
            stats.append(
                f"Jacobian Condition Number: {jacobian_cond(jac=jac):.3E}"
            )
...
class DegeneracyHunter2:
    ...
    def find_irreducible_degenerate_sets(self, tee=False):
        ...
        if self.degenerate_set:
            ...
            for count, k in enumerate(self.degenerate_set, start=1):
...
def check_parallel_jacobian(
    ...
    Args:
        ...
        scaled: True if scaled Jacobian should be used for diagnostic reporting (default = True),
            only used if jac or nlp are omitted.
