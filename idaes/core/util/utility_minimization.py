...
def min_utility(blk, heating, cooling, DTmin, eps=1e-6,
    ...
    def Theta_(blk, i):
        if i in heatingdict.keys():
            return (Q[i] / (0.5*((blk.Tout[i] - blk.Tin[i] + (EpsT))
    ...
    def rule_heat_above_pinch(blk, p):
        return (blk.QAh[p] == sum(blk.Theta[i]*(
                0.5*((blk.Tin[i] - blk.T_[p]) +
                     sqrt((blk.Tin[i] - blk.T_[p])**2 + eps)) -
                0.5*((blk.Tout[i] - blk.T_[p]) +
    ...
    def rule_heat_below_pinch(blk, p):
        return (blk.QAc[p] == sum(blk.Theta[i]*(
                0.5*((blk.Tout[i] - blk.T_[p] + DTmin) +
                     sqrt((blk.Tout[i] - blk.T_[p] + DTmin)**2 + eps)) -
                0.5*((blk.Tin[i] - blk.T_[p] + DTmin) +
...
def heat_data(blk, heating, cooling, DG_units=pyunits.Mwatt):
    ...
    for i in pinch_streamsdict.keys():
        Q[i] = value(pyunits.convert(pinch_streamsdict[i].heat_duty[0],
                     to_units=DG_units))
    ...
    exchangeData = {}
    for i in pinch_streamsdict.keys():
        exchangeData[i] = {'T_in': T_in[i], 'T_out': T_out[i],
                           'FCp_': FCp_[i], 'Q': Q[i]}
...
def pinch_calc(heating, cooling, exchangeData, DTmin, eps):
    ...
    initQw = (-sum(value(exchangeData[i]['Q']) for i in exchangerdict.keys())
              + initQs)
...
def generate_curves(CD):
    ...
    plt.plot(-Qhot, Thot, color="r",
             label="Hot Streams / Streams that are Cooled")
...
def heat_ex_data(blk, heating, cooling):
    ...
    T_units = pyunits.get_units(pinch_streams[0].control_volume.properties_in[0].temperature)
    ...
    for i in coolingdict.keys():
        ...
        CQ[j] = value(pyunits.convert(coolingdict[i].control_volume.heat[0],
                                      to_units=DG_units))
        j += 1
    ...
    for i in heatingdict.keys():
        ...
        HQ[j] = value(pyunits.convert(heatingdict[i].control_volume.heat[0],
                      to_units=DG_units))
        j += 1
...
def gen_curves(Tin, Tout, Q):
    ...
    for i in range(nTin):
        for j in range(nTstar):
            Qmat[i, j] = linear_interpolation([Tin[i], Tout[i]],
                                              [0, Q[i]], Tstar[j])
    Qstar = sum(Qmat, 0)
