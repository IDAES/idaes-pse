...
def hx_costing(self, hx_type='U-tube', FM='stain_steel', 
    ...
    # ------------------------ Heat Exchanger cost ------------------------
    # heat exchanger cost
    #build generic costing variables (base cost, purchase cost, material factor)
    _make_vars(self)
...
def pressure_changer_costing(self, FM_mat = "stain_steel", # applies for all types (pump, compressor, fan, blower)
                             mover_type = "compressor", #fan, blower, compressor
                             compressor_type="centrifugal", #only for compressor
                             driver_mover_type= "electrical_motor", #only for compressors
                             pump_type = "centrifugal", #centrifugal, external_gear, reciprocating
                             pump_type_factor = '1.4', #1.1 to 1.4, 2.1 and 2.2 (needs to be wise-selected by user see table)
    ...
    *default option
    ...
    if self.parent_block().config.compressor == False:
        ...
        def CP_rule(self):
                return self.purchase_cost == 530*(-1*work_hp)**0.81 ## NETL Baseline report?
        self.cp_cost_eq = Constraint(rule=CP_rule)
    
    #if compressor is True (the pressure changer could be a Pump, Fan, Blower, or Compressor)
    else:
        ...
        if self.parent_block().config.thermodynamic_assumption.name == 'pump':
            ...
            if(self.parent_block().config.property_package.get_metadata().
                ...
                Q_gpm = self.parent_block().control_volume.properties_in[0].flow_vol * 15850.32314
