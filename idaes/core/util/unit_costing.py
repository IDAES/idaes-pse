...
def vessel_costing(
    ...
    if ref_parameter_diameter is None and ref_parameter_length is None:
        try:
            ...
            raise ConfigurationError(
                "{} Component diameter does not exist in unit model".format(self.name)
            )
        try:
            L = self.parent_block().length
        except AttributeError:
            raise ConfigurationError(
                "{} Component length does not exist in unit model".format(self.name)
            )
    elif ref_parameter_diameter is None or ref_parameter_length is None:
        raise ConfigurationError(
            "provide both ref_parameter_diameter " "and ref_parameter_length"
        )
    else:
    ...
    self.vessel_purchase_cost = Var(
        initialize=1e5, domain=NonNegativeReals, doc="Vessel cost in $"
    )
    ...
    self.shell_thickness = Param(
        mutable=True, initialize=1.25, doc="Shell thickness in in", units=pyunits.inch
    )
    self.material_factor = Param(
        mutable=True, initialize=3, doc="Construction material correction factor"
    )
    self.material_density = Param(
        mutable=True,
        initialize=0.284,
    ...
    if weight_limit == "option1":
        # Blower Cost Correlation
        c_alf1 = {"horizontal": 8.9552, "vertical": 7.0132}

        c_alf2 = {"horizontal": -0.2330, "vertical": 0.18255}

        c_alf3 = {"horizontal": 0.04333, "vertical": 0.02297}
    ...
    if PL is True:
        self.base_cost_platf_ladders = Var(
            initialize=1000,
            domain=NonNegativeReals,
            doc="Base cost of" " platforms and ladders in $",
        )
        platforms_ladders(self, alignment=alignment, L_D_range=L_D_range, D=D, L=L)
    ...
    def CP_vessel_rule(self):
        return self.vessel_purchase_cost == (
            ...
            + (self.base_cost_platf_ladders * self.number_of_units if PL else 0)
        )
    ...
    # True if platform and ladder costs are incuded
    if plates is True:
        self.purchase_cost_trays = Var(
            initialize=1e6, domain=NonNegativeReals, doc="Purchase cost of trays in $"
        )
        plates_cost(
            self,
            ...
            tray_type=tray_type,
            D=D,
            number_tray=number_tray,
        )

    def CP_cost_rule(self):
        return self.purchase_cost == self.vessel_purchase_cost + (
            self.purchase_cost_trays * self.number_of_units if plates else 0
        )

    self.cp_cost_eq = Constraint(rule=CP_cost_rule)


def platforms_ladders(self, alignment="horizontal", L_D_range="option1", D=10, L=20):
    if alignment == "horizontal":
        ...
        def CPL_rule(self):
            if L_D_range == "option1":
                return self.base_cost_platf_ladders == (
                    361.8
                    * (D / pyunits.foot) ** 0.73960
                    * (L / pyunits.foot) ** 0.70684
                )
            elif L_D_range == "option2":
                return self.base_cost_platf_ladders == (
                    309.9
                    * (D / pyunits.foot) ** 0.63316
                    * (L / pyunits.foot) ** 0.80161
                )
            else:
                # PYLINT-TODO-FIX fix exception message with correct number of arguments
                raise ValueError(
                    "{} - L_D_range option not supported. "  # pylint: disable=E1305
                    "Please see documentation for list of "
                    "supported options.".format(self.name, L_D_range)
                )
        ...
        # PYLINT-TODO-FIX fix exception message with correct number of arguments
        raise ValueError(
            "{} - vessel alignment type not supported. "  # pylint: disable=E1305
            "Please see documentation for list of "
            "supported options.".format(self.name, alignment)
        )


def plates_cost(
    self, tray_mat_factor="carbon_steel", tray_type="sieve", D=10, number_tray=10
):

    self.base_cost_trays = Var(
        initialize=1e4, domain=NonNegativeReals, doc="Base cost of trays in $"
    )
    self.type_tray_factor = Param(mutable=True, initialize=1.0, doc="Tray type factor")
    self.number_tray_factor = Param(
        mutable=True, initialize=1.0, doc="Number of trays factor"
    )
    self.number_trays = Param(mutable=True, initialize=15.0, doc="Tray type factor")
    self.tray_material_factor = Var(
        initialize=1.0, doc="FTM material of construction" " factor"
    )
    ...
    type_tray_dic = {"sieve": 1, "valve": 1.18, "bubble_cap": 1.87}
    ...
    def mt_factor_rule(self):
        return self.tray_material_factor == (
            t_alf1[tray_mat_factor] + t_alf2[tray_mat_factor] * D / pyunits.foot
        )
...
def fired_heater_costing(
    ...
    self.pressure_factor = Var(
        initialize=1.1, domain=NonNegativeReals, doc="Pressure design factor"
    )
    ...
    Q = pyunits.convert(Q, to_units=pyunits.BTU / pyunits.hr) / self.number_of_units

    # material factor -------------------
    material_factor_dic = {"carbon_steel": 1.0, "Cr-Mo_alloy": 1.4, "stain_steel": 1.7}
    ...
    def CB_rule(self):
        if fired_type == "fuel":
            ...
            return self.base_cost_per_unit == exp(
                9.593
                - 0.3769 * log((Q / pyunits.BTU * pyunits.hr))
            ...
            # PYLINT-TODO-FIX fix exception message with correct number of arguments
            raise ValueError(
                "{} - fired heater type not supported. "  # pylint: disable=E1305
                "Please see documentation for list of "
                "supported FH types.".format(self.name, fired_type)
            )
