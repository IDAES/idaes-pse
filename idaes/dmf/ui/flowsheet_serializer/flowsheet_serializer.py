...
class FlowsheetSerializer:
    ...
    def _construct_output_json(self):
        ...
        for orphan_port in self.orphaned_ports:
            #  unit_model_name = "" ?? never used ??
            try:
                if num_open_inlets <= 0:
                    num_open_inlets = len(
                        self.ports[orphan_port].create_inlet_list()
            ...
            dest_anchor = link_position_mapping[
                self.unit_models[self.ports[orphan_port]]["type"]
            ]["inlet_anchors"]
            ...
            if icon_type == "feed":
                ...
                source_anchor = link_position_mapping[
                    self.unit_models[self.ports[orphan_port]]["type"]
                ]["inlet_anchors"]
            ...
            self._create_link_json(
                out_json, source_anchor, dest_anchor, source_id, dest_id, id_counter
            )
