...
@pytest.mark.unit
def test_dens_mol_liq_comp_deprecated(frame_deprecated):
    dens_mol_liq_comp.build_parameters(frame_deprecated.params)

    assert isinstance(frame_deprecated.params.dens_mol_liq_comp_coeff_eqn_type,
                      Param)
    assert value(frame_deprecated.params.dens_mol_liq_comp_coeff_eqn_type) == 1

    expr = dens_mol_liq_comp.return_expression(
        frame_deprecated.props[1], frame_deprecated.params,
        frame_deprecated.props[1].temperature)
    assert value(expr) == pytest.approx(55.583e3, rel=1e-4)

    frame_deprecated.props[1].temperature.value = 333.15
    assert value(expr) == pytest.approx(54.703e3, rel=1e-4)
