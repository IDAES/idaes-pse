...
class Cubic(EoSBase):
    ...
    def common(b, pobj):
        ...
        def rule_delta(m, p, i):
            ...
            a = getattr(m, cname+"_a")
            am = getattr(m, cname+"_am")
            kappa_A = getattr(m.params, cname+"_kappa_A")
            kappa_B = getattr(m.params, cname+"_kappa_B")
            kappa_C = getattr(m.params, cname+"_kappa_C")
            return (2*sqrt(a[i])/am[p] *
                    sum(m.mole_frac_phase_comp[p, j]*sqrt(a[j]) *
                        (1-(kappa_A[i, j] +
                            kappa_B[i, j] * m.temperature/m.params.temperature_ref +
                            kappa_C[i, j] * (m.temperature/m.params.temperature_ref)**2))
            ...
            def rule_delta_eq(m, p1, p2, p3, i):
                ...
                return (2*sqrt(a[p1, p2, i])/am[p1, p2, p3] *
                        sum(m.mole_frac_phase_comp[p3, j]*sqrt(a[p1, p2, j]) *
                            (1-(kappa_A[i, j] +
                                kappa_B[i, j] * m._teq[p1, p2]/m.params.temperature_ref +
                                kappa_C[i, j] * (m._teq[p1, p2]/m.params.temperature_ref)**2))
    ...
    def build_parameters(b):
        ...
        if hasattr(param_block, cname+"_kappa_A"):
            # Common components already constructed by previous phase
            return

        kappa_A_data = param_block.config.parameter_data[cname+"_kappa_A"]
        param_block.add_component(
            cname+'_kappa_A',
            Var(param_block.component_list,
                param_block.component_list,
                within=Reals,
                initialize=kappa_A_data,
                doc=cname+'A binary interaction parameters',
                units=None))

        if hasattr(param_block, cname+"_kappa_B"):
            # Common components already constructed by previous phase
            return

        kappa_B_data = param_block.config.parameter_data[cname+"_kappa_B"]
        param_block.add_component(
            cname+'_kappa_B',
            Var(param_block.component_list,
                param_block.component_list,
                within=Reals,
                initialize=kappa_B_data,
                doc=cname+'B binary interaction parameters',
                units=None))

        if hasattr(param_block, cname+"_kappa_C"):
            # Common components already constructed by previous phase
            return

        kappa_C_data = param_block.config.parameter_data[cname+"_kappa_C"]
        param_block.add_component(
            cname+'_kappa_C',
            Var(param_block.component_list,
                param_block.component_list,
                within=Reals,
                initialize=kappa_C_data,
                doc=cname+'C binary interaction parameters',
                units=None))
    ...
    def log_fug_phase_comp_Tbub(blk, p, j, pp):
        ...
        kappa_A = getattr(blk.params, cname+"_kappa_A")
        kappa_B = getattr(blk.params, cname+"_kappa_B")
        kappa_C = getattr(blk.params, cname+"_kappa_C")
        am = sum(sum(x[xidx, i]*x[xidx, j]*sqrt(a(i)*a(j))*
                    (1-(kappa_A[i, j] +
                        kappa_B[i, j] * blk.temperature_bubble[pp]/blk.params.temperature_ref +
                        kappa_C[i, j] * (blk.temperature_bubble[pp]/blk.params.temperature_ref)**2))
        ...
        delta = (2*sqrt(a(j))/am * sum(x[xidx, i]*sqrt(a(i))*
                (1-(kappa_A[j, i] +
                    kappa_B[j, i] * blk.temperature_bubble[pp]/blk.params.temperature_ref +
                    kappa_C[j, i] * (blk.temperature_bubble[pp]/blk.params.temperature_ref)**2))
                    for i in blk.component_list))
    ...
    def log_fug_phase_comp_Tdew(blk, p, j, pp):
        ...
        am = sum(sum(x[xidx, i]*x[xidx, j]*sqrt(a(i)*a(j))*
                (1-(kappa_A[i, j] +
                kappa_B[i, j] * blk.temperature_dew[pp]/blk.params.temperature_ref +
                kappa_C[i, j] * (blk.temperature_dew[pp]/blk.params.temperature_ref)**2))
                for j in blk.component_list)
                for i in blk.component_list)
        ...
        delta = (2*sqrt(a(j))/am * sum(x[xidx, i]*sqrt(a(i))*
                (1-(kappa_A[j, i] +
                    kappa_B[j, i] * blk.temperature_dew[pp]/blk.params.temperature_ref +
                    kappa_C[j, i] * (blk.temperature_dew[pp]/blk.params.temperature_ref)**2))
                    for i in blk.component_list))
    ...
    def log_fug_phase_comp_Pbub(blk, p, j, pp):
        ...
        am = sum(sum(x[xidx, i]*x[xidx, j] *
                     sqrt(a[i]*a[j])*
                     (1-(kappa_A[i, j] +
                     kappa_B[i, j] * blk.temperature/blk.params.temperature_ref +
                     kappa_C[i, j] * (blk.temperature/blk.params.temperature_ref)**2))
                     for j in blk.component_list)
                 for i in blk.component_list)
        ...
        delta = (2*sqrt(a[j])/am * sum(x[xidx, i]*sqrt(a[i])*
                (1-(kappa_A[j, i] +
                    kappa_B[j, i] * blk.temperature/blk.params.temperature_ref +
                    kappa_C[j, i] * (blk.temperature/blk.params.temperature_ref)**2))
                    for i in blk.component_list))
...
def rule_am_default(m, cname, a, p, pp=()):
    k_A = getattr(m.params, cname+"_kappa_A")
    k_B = getattr(m.params, cname+"_kappa_B")
    k_C = getattr(m.params, cname+"_kappa_C")
    return sum(sum(
        m.mole_frac_phase_comp[p, i]*m.mole_frac_phase_comp[p, j] *
        sqrt(a[pp, i]*a[pp, j])*
            (1-(k_A[i, j] +
                k_B[i, j] * m.temperature/m.params.temperature_ref +
                k_C[i, j] * (m.temperature/m.params.temperature_ref)**2))
        for j in m.components_in_phase(p))
        for i in m.components_in_phase(p))
