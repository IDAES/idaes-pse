...
class TestParamBlock(object):
    ...
    def test_build(self):
        ...
        assert model.params.bmimPF6.mw.value == 284.18E-3
        assert model.params.bmimPF6.pressure_crit.value == 24e5
        assert model.params.bmimPF6.temperature_crit.value == 860

        assert model.params.carbon_dioxide.mw.value == 44.010E-3
        assert model.params.carbon_dioxide.pressure_crit.value == 71.8e5
        assert model.params.carbon_dioxide.temperature_crit.value == 304.1
...
class TestStateBlock(object):
    ...
    def test_build(self, model):
        ...
        assert model.props[1].pressure.lb == 5e-4
        ...
        for i in model.props[1].mole_frac_comp:
            assert value(model.props[1].mole_frac_comp[i]) == 1/2
    ...
    def test_get_material_flow_terms(self, model):
        for p in model.params.phase_list:
            for j in model.params.component_list:
                if j == "bmimPF6":
                    assert model.props[1].get_material_flow_terms("Liq", j) == (
                        model.props[1].flow_mol_phase["Liq"] *
                        model.props[1].mole_frac_phase_comp["Liq", j])
                else:
    ...
    def test_get_material_density_terms(self, model):
        for p in model.params.phase_list:
            for j in model.params.component_list:
                if j == "bmimPF6":
                    assert model.props[1].get_material_density_terms("Liq", j) == (
    ...
    def test_initialize(self, model):
        ...
        model.props[1].temperature.fix(200.00)
        model.props[1].pressure.fix(101325)
        model.props[1].mole_frac_comp["carbon_dioxide"].fix(1/2)
        model.props[1].mole_frac_comp["bmimPF6"].fix(1/2)
    ...
    def test_solution(self, model):
        ...
        assert model.props[1].mole_frac_phase_comp["Liq", "carbon_dioxide"].value == \
            pytest.approx(0.3188, abs=1e-4)
        assert model.props[1].mole_frac_phase_comp["Vap", "carbon_dioxide"].value == \
            pytest.approx(0.9999, abs=1e-4)
        assert model.props[1].phase_frac["Vap"].value == \
            pytest.approx(0.2659, abs=1e-4)
