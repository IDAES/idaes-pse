...
from idaes.generic_models.properties import swco2, iapws95
from idaes.power_generation.properties import FlueGasParameterBlock
from idaes.generic_models.unit_models import (HeatExchangerLumpedCapacitance,
...
class TestHXLCGeneric(object):
    ...
    def test_units_unconstrained(self, unconstrained_model):
        ...
        assert_units_equivalent(m.fs.unit.ua_hot_side_to_wall,
                                pyunits.W / pyunits.K)
        assert_units_equivalent(m.fs.unit.temperature_wall, pyunits.K)
        assert_units_equivalent(m.fs.unit.heat_capacity_wall,
                                pyunits.J / pyunits.K)
        assert_units_equivalent(m.fs.unit.thermal_resistance_wall,
                                pyunits.K / pyunits.W)
        assert_units_equivalent(m.fs.unit.thermal_fouling_hot_side,
                                pyunits.K / pyunits.W)
        assert_units_equivalent(m.fs.unit.thermal_fouling_cold_side,
                                pyunits.K / pyunits.W)
...
class TestHXLCTransientSCO2(object):
    ...
    def model(self):
        ...
        m.fs.unit = HeatExchangerLumpedCapacitance(default={
            ...
            "tube": {"property_package": m.fs.sco2,
                     "has_pressure_change": True},
            "shell": {"property_package": m.fs.fluegas,
        ...
        tube_inlet_enthalpy = swco2.htpx(T=tube_inlet_temperature * pyunits.K,
                                         P=tube_inlet_pressure * pyunits.Pa)
    ...
    def test_steady_state(self, model):
        ...
        self.check_temperatures(model, np.array(model.fs.time), exp_sco2,
                                exp_air, exp_wall)
    ...
    @pytest.mark.static
    def check_temperatures(self, model, times, expected_sco2_temps,
                           expected_air_temps, expected_wall_temps,
                           tol=1e-3):

        for i, t in enumerate(times):
            assert value(model.fs.unit.tube.properties_out[t].temperature) \
                == pytest.approx(expected_sco2_temps[i], rel=tol)
            assert value(model.fs.unit.shell.properties_out[t].temperature) \
                == pytest.approx(expected_air_temps[i], rel=tol)
            assert value(model.fs.unit.temperature_wall[t]) \
                == pytest.approx(expected_wall_temps[i], rel=tol)
