            ...
            for P in p_set:
                ...
                try:
                    ...
                    if TEST_DERS:
                        ...
                        # 2*TOL of one of the numerical values
                        assert (pytest.approx(dZdA_p, FD_TOL) == g[1] or
                                pytest.approx(dZdA_m, FD_TOL) == g[1] or
                                (between(g[1], dZdA_p, dZdA_m) and (
                                 pytest.approx(dZdA_p, 2*FD_TOL) == g[1] or
                                 pytest.approx(dZdA_m, 2*FD_TOL) == g[1])))

                        assert (pytest.approx(dZdB_p, FD_TOL) == g[2] or
                                pytest.approx(dZdB_m, FD_TOL) == g[2] or
                                (between(g[2], dZdB_p, dZdB_m) and (
                                 pytest.approx(dZdB_p, 2*FD_TOL) == g[2] or
                                 pytest.approx(dZdB_m, 2*FD_TOL) == g[2])))
...
#                            assert (d2ZdB2_p == pytest.approx(h[5], FD_TOL) or
#                                    d2ZdB2_m == pytest.approx(h[5], FD_TOL) or
#                                    between(h[5], d2ZdB2_p, d2ZdB2_m))

                except AssertionError:
                    # Print values at failure and raise exception
                    print(eos_type, T, P, A, B, Z, h[2], gAp[1], g[1], gAm[1], ZAp, ZAm)
                    raise
