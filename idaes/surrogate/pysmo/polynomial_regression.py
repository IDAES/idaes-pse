...
class PolynomialRegression:
    ...
    **Example:**
    ...
    def pyomo_optimization(x, y):
        ...
        model.y_predictions = Expression(model.M, model.P, rule=xy_product, doc='Predicted value calc: y = hx')
    ...
    def error_plotting(vector_of_results):
        ...
        ax1.plot(vector_of_results[:, 0], vector_of_results[:, 2], 'green', vector_of_results[:, 0],
                 vector_of_results[:, 3], 'red')
        ...
        ax4.plot(vector_of_results[:, 0], vector_of_results[:, 6], 'blue', vector_of_results[:, 0],
                 vector_of_results[:, 7], 'red')
    ...
    def user_defined_terms(self, additional_regression_features):
        ...
        for i in range(0, number_additional_features):
            ...
            if isinstance(additional_regression_features[i], np.ndarray) and (
                    len(additional_regression_features[i]) == self.regression_data.shape[0]) and (
                    additional_regression_features[i].ndim == 1):
                additional_features_array[:, i] = additional_regression_features[i]
            elif isinstance(additional_regression_features[i], pd.DataFrame) and (
                    len(additional_regression_features[i]) == self.regression_data.shape[0]) and (
                    additional_regression_features[i].ndim == 1):
                additional_features_array[:, i] = additional_regression_features[i].values
            elif isinstance(additional_regression_features[i], pd.Series) and (
                    len(additional_regression_features[i]) == self.regression_data.shape[0]) and (
                    additional_regression_features[i].ndim == 1):
                additional_features_array[:, i] = additional_regression_features[i].values
            else:
        ...
        if (additional_regression_features is None) or (len(additional_regression_features) == 0):
            ...
            for poly_order in range(1, self.max_polynomial_order + 1):
                for cv_number in range(1, self.number_of_crossvalidations + 1):
                    phi, train_error, cv_error = self.polyregression(poly_order,
            ...
            print('Initial Regression Model Performance:\nOrder: ', order_best, ' / MAE: %4f' % mae_error,
                  ' / MSE: %4f' % mse_error, ' / R^2: %4f' % r_square, ' / Adjusted R^2: %4f' % r_square_adj)
            ...
            while (self.regression_data.shape[0] < stopping_criterion) and (mae_error > eps_neg) and (
                    mse_error > eps_neg) and (
                    r_square < eps_pos) and (iteration_number < self.max_iter) and (self.regression_data.shape[0] + self.no_adaptive_samples < self.original_data.shape[0]):
                print('\n-------------------------------------------------')
                ...
                print('Regression performance on full data in iteration', iteration_number, '\nOrder: ', order_best,
                      ' / MAE: %4f' % mae_error,
                      ' / MSE: %4f' % mse_error, ' / R_sq: %4f' % r_square, ' / Adjusted R^2: %4f' % r_square_adj)
                ...
                vector_of_results[iteration_number, :] = [iteration_number, order_opt, train_error_opt, best_error_opt,
                                                          mae_error_opt, mse_error_opt, r_square_opt, r_square_adj_opt, self.regression_data.shape[0]]
                iteration_number += 1
            ...
            print('\n-------------------------------------------------\n-------------------------------------------------')
            print('Best solution found: ', '\nOrder: ', order_opt, ' / MAE: %4f' % mae_error_opt,
                  ' / MSE: %4f' % mse_error_opt, ' / R_sq: %4f' % r_square_opt, ' / Adjusted R^2: %4f' % r_square_adj_opt)
