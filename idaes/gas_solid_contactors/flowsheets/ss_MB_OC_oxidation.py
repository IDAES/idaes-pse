...
##############################################################################
"""
Flowsheet example of the MB model for an iron-oxide oxidation
case study
...
#            fe_set.append(fe_b + (i-nfe*fe_a)*(1-fe_b)/(nfe*(1-fe_a)))
    ...
    m.fs.MB.bed_height.fix(3)  # m

    # Fix inlet port variables for gas and solid
    m.fs.MB.gas_inlet.flow_mol[0].fix(1067.79)  # mol/s
    m.fs.MB.gas_inlet.temperature[0].fix(373)  # K
    m.fs.MB.gas_inlet.pressure[0].fix(3)  # bar
    m.fs.MB.gas_inlet.mole_frac[0, "O2"].fix(0.2095)
    m.fs.MB.gas_inlet.mole_frac[0, "N2"].fix(0.7808)
    m.fs.MB.gas_inlet.mole_frac[0, "CO2"].fix(0.0004)
    m.fs.MB.gas_inlet.mole_frac[0, "H2O"].fix(0.0093)

    m.fs.MB.solid_inlet.flow_mass[0].fix(1030.865)  # kg/s
    m.fs.MB.solid_inlet.temperature[0].fix(1173.9)  # K
    m.fs.MB.solid_inlet.mass_frac[0, "Fe2O3"].fix(0.244162011502)
    m.fs.MB.solid_inlet.mass_frac[0, "Fe3O4"].fix(0.201998299487)
    m.fs.MB.solid_inlet.mass_frac[0, "Al2O3"].fix(0.553839689011)
...
def print_summary(self):
    ...
    Conv_gas = 1 - (
            (m.fs.MB.gas_phase.properties[0, 1].flow_mol.value *
             m.fs.MB.gas_phase.properties[0, 1].mole_frac['O2'].value) /
            (m.fs.MB.gas_phase.properties[0, 0].flow_mol.value *
             m.fs.MB.gas_phase.properties[0, 0].mole_frac['O2'].value))
    Conv_OC = 1 - (
            (m.fs.MB.solid_phase.properties[0, 0].flow_mass.value *
             m.fs.MB.solid_phase.properties[0, 0].mass_frac['Fe3O4'].value) /
            (m.fs.MB.solid_phase.properties[0, 1].flow_mass.value *
             m.fs.MB.solid_phase.properties[0, 1].mass_frac['Fe3O4'].value))
    print("\nO2 conversion:", value(Conv_gas)*100, " %")
    print("Fe3O4 conversion:", value(Conv_OC)*100, " %")
