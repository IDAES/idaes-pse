...
##############################################################################
"""
Flowsheet example of the 2-region BFB model for an iron-oxide oxidation
case study
...
#            fe_set.append(fe_b + (i-nfe*fe_a)*(1-fe_b)/(nfe*(1-fe_a)))
    ...
    m.fs.BFB.gas_inlet.mole_frac[0, "O2"].fix(0.2095)
    m.fs.BFB.gas_inlet.mole_frac[0, "N2"].fix(0.7808)
    m.fs.BFB.gas_inlet.mole_frac[0, "CO2"].fix(0.0004)
    m.fs.BFB.gas_inlet.mole_frac[0, "H2O"].fix(0.0093)

    m.fs.BFB.solid_inlet.flow_mass[0].fix(1230.865)  # kg/s
    m.fs.BFB.solid_inlet.temperature[0].fix(1173.9)  # K
    m.fs.BFB.solid_inlet.mass_frac[0, "Fe2O3"].fix(0.244162011502)
    m.fs.BFB.solid_inlet.mass_frac[0, "Fe3O4"].fix(0.201998299487)
    m.fs.BFB.solid_inlet.mass_frac[0, "Al2O3"].fix(0.553839689011)
...
def print_summary(self):
    if m.fs.BFB.config.solid_emulsion_config.reaction_package is not None:
        print()
        print("Reaction stoichiometry ratio check (expected value is 4) =>")

        mole_gas_reacted = (
                m.fs.BFB.gas_inlet.flow_mol[0].value *
                m.fs.BFB.gas_inlet.mole_frac[0, 'O2'].value -
                m.fs.BFB.gas_outlet.flow_mol[0].value *
                m.fs.BFB.gas_outlet.mole_frac[0, 'O2'].value)

        mole_solid_reacted = (
                (m.fs.BFB.solid_inlet.flow_mass[0].value *
                 m.fs.BFB.solid_inlet.mass_frac[0, 'Fe3O4'].value /
                 m.fs.BFB.solid_inlet_block[0]._params.mw['Fe3O4']) -
                (m.fs.BFB.solid_outlet.flow_mass[0].value *
                 m.fs.BFB.solid_outlet.mass_frac[0, 'Fe3O4'].value /
                 m.fs.BFB.solid_outlet_block[0]._params.mw['Fe3O4']))
    ...
    if m.fs.BFB.config.solid_emulsion_config.reaction_package is not None:
        ...
        Conv_gas = 1 - (
            ((m.fs.BFB.gas_outlet.flow_mol[0].value *
              m.fs.BFB.gas_outlet_block[0].mole_frac['O2'].value)) /
            (m.fs.BFB.gas_inlet.flow_mol[0].value *
             m.fs.BFB.gas_inlet_block[0].mole_frac['O2'].value))
        ...
        print("O2 conversion:", value(Conv_gas)*100, "%")
