...
def test_setInputs_state_block():
    ...
    m.state_block.mole_frac["CO2"].fix(0.1622)
    m.state_block.mole_frac["H2O"].fix(0.0186)
    m.state_block.mole_frac["CH4"].fix(0.0217)
    m.state_block.mole_frac["CO"].fix(0.4631)
    m.state_block.mole_frac["H2"].fix(0.3344)
...
def test_solve():
    ...
    assert value(m.state_block.mole_frac['CO2']) == \
        pytest.approx(0.1622, abs=1e-3)
    assert value(m.state_block.mole_frac['CH4']) == \
        pytest.approx(0.0217, abs=1e-3)
    assert value(m.state_block.mole_frac['CO']) == \
        pytest.approx(0.4631, abs=1e-3)
    assert value(m.state_block.mole_frac['H2']) == \
        pytest.approx(0.3344, abs=1e-3)
