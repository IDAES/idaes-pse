...
def test_setInputs_state_block():
    ...
    m.state_block.mole_frac["CO2"].fix(0.0004)
    m.state_block.mole_frac["H2O"].fix(0.0093)
    m.state_block.mole_frac["O2"].fix(0.2095)
    m.state_block.mole_frac["N2"].fix(0.7808)
...
def test_solve():
    ...
    assert value(m.state_block.mole_frac['CO2']) == \
        pytest.approx(0.0004, abs=1e-3)
    assert value(m.state_block.mole_frac['O2']) == \
        pytest.approx(0.2095, abs=1e-3)
