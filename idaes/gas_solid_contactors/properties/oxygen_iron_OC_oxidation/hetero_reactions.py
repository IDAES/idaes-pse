...
class _ReactionBlock(ReactionBlockBase):
    ...
    def initialize(blk, outlvl=idaeslog.NOTSET,
        ...
        # Revart state variables of the primary (solid) state block
    ...
    def _OC_conv(self):
        ...
        def OC_conv_eqn(b):
            return 1e6 * b.OC_conv * \
                   ...
                   1e6 * b.solid_state_ref.mass_frac['Fe2O3']
        try:
            # Try to build constraint
    ...
    def _reaction_rate(self):
        ...
        def rate_rule(b, r):
            return b.reaction_rate[r]*1e4 == b._params._scale_factor_rxn*1e4*(
                ...
                (b._params.a_vol/(b.solid_state_ref._params.mw['Fe3O4'])) *
                3*b._params.rxn_stoich_coeff[r]*b.k_rxn[r] *
                (((b.gas_state_ref.dens_mole_comp_vap['O2']**2 +
