...
from idaes.logger import getInitLogger, init_tee, condition
...
#                has_equilibrium=self.config.has_equilibrium_reactions)
        ...
        # Create total volume variable and constraint
        self.volume_reactor = Var(initialize=0.1,
                                  doc="Total reactor volume")

        @self.Constraint(self.flowsheet().config.time,
                         doc="Total recator volume constraint")
        def reactor_volume_constraint(b, t):
