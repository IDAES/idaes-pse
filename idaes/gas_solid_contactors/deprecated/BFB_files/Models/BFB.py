...
see reaction package for documentation.}"""))
    ...
    # TODO - additional documentation needed to capture changes in template
    ...
    # =========================================================================
        ...
        self.bubble_region.add_state_blocks(
            ...
            has_phase_equilibrium=self.config.bubble_config.
    ...
    # =========================================================================
        ...
        self.gas_emulsion_region.add_state_blocks(
            information_flow=set_direction_gas,
            has_phase_equilibrium=self.config.gas_emulsion_config.
            has_phase_equilibrium)
    ...
    # =========================================================================
        ...
        # TODO - a hack to pass the gas state block into the reaction block
        # for heterogeneous reactions is currently used. This requires
        # that the gas cv1D be built first (see "reaction_package_args")

        # Set argument for heterogeneous reaction block
        if self.config.solid_emulsion_config.reaction_package is not None:
            self.config.solid_emulsion_config.reaction_package_args = \
                    {"gas_state_block": self.gas_emulsion_region.properties}
        ...
        self.solid_emulsion_region.add_state_blocks(
            ...
            has_phase_equilibrium=self.config.solid_emulsion_config.
            has_phase_equilibrium)

        if self.config.solid_emulsion_config.reaction_package is not None:
            self.solid_emulsion_region.add_reaction_blocks(
                    has_equilibrium=self.config.solid_emulsion_config.
                    has_equilibrium_reactions)
    ...
    def _make_performance(self):
        ...
        def bubble_cloud_bulk_mass_trans(b, t, x, j):
            b.conc_diff = (b.gas_emulsion_region.properties[t, x].
                           dens_mole_vap -
            ...
            return (b.Kgbulk_c[t, x, j] * b.bubble_diameter[t, x] ==
                    ...
                    (0.5 *
                     ...
                     (b.conc_diff + (b.conc_diff**2 + b._eps**2)**0.5) +
        ...
        if self.config.energy_balance_type != EnergyBalanceType.none:
            ...
            def bubble_cloud_bulk_heat_trans(b, t, x):
                b.temp_diff = (b.gas_emulsion_region.properties[t, x].
                               dens_mole_vap
        ...
        # TODO - placeholder for solid emulsion mass transfer term
        # Solid_emulsion mass transfer
        @self.Constraint(self.flowsheet().config.time,
                         self.length_domain,
                         self.solid_phase_list_ref,
                         self.solid_component_list_ref,
                         doc="Solid emulsion mass transfer")
        def solid_emulsion_mass_transfer(b, t, x, p, j):
            return (b.solid_emulsion_region.
                    mass_transfer_term[t, x, p, j] == 0)
...
#        """
        ...
        # property block constraints, and reaction block constraints active
        # Exceptions:
        ...
        def constraints_generator(obj):
            for c in obj.component_objects(Constraint, descend_into=True):
                if not c.local_name.endswith("_disc_eq") \
                    ...
                    and not \
                        c.parent_block().local_name.startswith("reactions") \
                    and not \
        ...
        blk.solid_emulsion_mass_transfer.activate()
        ...
        if blk.config.bubble_config.reaction_package is not None:
            ...
            blk.bubble_region.reactions.initialize(outlvl=outlvl - 1,
                                                   optarg=optarg,
                                                   solver=solver)
        ...
        if blk.config.gas_emulsion_config.reaction_package is not None:
            ...
            # Initialize reaction property package
            blk.gas_emulsion_region.reactions.initialize(outlvl=outlvl - 1,
                                                         optarg=optarg,
                                                         solver=solver)
        ...
        if blk.config.solid_emulsion_config.reaction_package is not None:
            ...
            # Initialize reaction property package
            blk.solid_emulsion_region.reactions.initialize(outlvl=outlvl - 1,
                                                           optarg=optarg,
                                                           solver=solver)
