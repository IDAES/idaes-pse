...
##############################################################################
"""
This package provides the necessary constraints for solid phase properties for
the CLC of methane with an iron-based oxygen carrier
...
class PhysicalParameterData(PhysicalParameterBlock):
    ...
    def build(self):
        ...
        self.state_block_class = Solid_Phase_Thermo_StateBlock

        # List of valid phases in property package
        self.phase_list = Set(initialize=['Sol'])

        # Component list - a list of component identifiers
        self.component_list = Set(initialize=['Fe2O3', 'Fe3O4', 'Al2O3'])

        # List of components in each phase (optional)
        self.phase_comp = {"Sol": self.component_list}
...
#        # Bulk solid density of fresh oxygen carrier
        ...
        # TODO - look into finding better correlation for particle porosity
        # Particle porosity:
...
#        init_log = getInitLogger(blk.name, outlvl)
        ...
        for k in blk.keys():
            ...
            if hasattr(blk[k], "bulk_density_constraint"):
                calculate_variable_from_constraint(
                            blk[k].dens_mass_sol,
                            blk[k].bulk_density_constraint)
...
class Solid_Phase_Thermo_StateBlockData(StateBlockData):
    ...
    def _dens_mass_sol(self):
        # Bulk density of OC solid particles
        self.dens_mass_sol = Var(domain=Reals,
                                 initialize=3251.75,
                                 doc='Bulk density of OC solid particles'
                                 '[kg/m3]')

        def bulk_density_constraint(b):
            return b.dens_mass_sol == b._params.particle_porosity \
                                        * sum(b._params.dens_mass_comp_sol[j] *
                                              b.mass_frac[j]
                                              for j in b._params.component_list
                                              )
        try:
            # Try to build constraint
            self.bulk_density_constraint = Constraint(
                                            rule=bulk_density_constraint)
        except AttributeError:
            # If constraint fails, clean up so that DAE can try again later
            self.del_component(self.dens_mass_sol)
            self.del_component(self.bulk_density_constraint)
            raise
    ...
    def _enth_mass(self):
        ...
        try:
            ...
            self.mixture_enthalpy_eqn = Constraint(expr=(
                        self.enth_mass == sum(self.mass_frac[j] *
    ...
    def get_material_density_terms(b, p, j):
        return b.dens_mass_sol*b.mass_frac[j]

    def get_energy_density_terms(b, p):
        return b.dens_mass_sol*b.enth_mass
